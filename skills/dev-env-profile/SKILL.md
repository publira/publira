...
For a complete local stack, use `task dev-env:start`; it prints the profile-specific URLs and writes process logs under `~/.publira/dev-env/runs/<profile>/`. Those URLs are the profile's edge, a Traefik container Docker runs in front of its processes, and they are the only addresses that serve images: `/images…` belongs to the image server rather than to any app, so a page opened on the port an app listens on itself renders every eye-catch and logo as a broken image. That is about a browser, which asks for `/images…` on the origin of the page it is reading. A client that addresses each server itself keeps addressing their ports: the Flutter app does, and names the tenant in `X-Forwarded-Host`, which the edge would overwrite with the host name it was reached on. Use `task dev-env:stop` when finished; it takes the edge down with the processes. For a single app or command, use `eval "$(task --silent dev-env:env)"` in that shell before the normal command so it receives the same profile-scoped URLs and credentials.

A profile written before a change to what a profile holds is refused by every command that loads it, with a message naming its file, and recreating it with `task dev-env:destroy`, `task dev-env:create`, and `task dev-env:init` is the answer. That destroy is still the user's to ask for: report the refusal and what replacing the profile takes rather than running it. Never run `task dev-env:destroy` merely to make a profile reusable. It destroys that profile's database, Valkey DB, and bucket after confirmation, refuses profiles selected by any worktree, and must be requested explicitly by the user. It must never be used for the shared default development environment or an E2E profile.

When diagnosing an unexpected connection, run `task dev-env:show` and inspect the profile's URL, slot, and bucket before changing code. Keep `PNCH_CACHE_APP` as the per-application cache namespace; the Redis logical database is the worktree boundary.
