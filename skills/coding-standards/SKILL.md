...
| **Next.js cache: `cacheHandler` vs `cacheHandlers`** — wire both, backed by Redis through `@publira/next-cache-handlers` | Editing the cache settings in an app's `next.config.ts` | None. All three apps already wire both, so only a change to those files can break it | [`references/nextjs-cache.md`](references/nextjs-cache.md) |
