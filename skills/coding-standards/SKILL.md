...
| **React: Effects and `useEffectEvent`** — user actions go in handlers, derived values are computed during render, edit state resets by `key` remount, and Effects are for external systems only | Writing or reviewing any `useEffect` / `useEffectEvent`, or any component that mirrors props into state | Partial. `react/react-compiler` and `react-hooks/rules-of-hooks` catch some cases; no rule detects a props→state Effect ([#456](https://github.com/publira/publira/issues/456)) | [`references/react-effects.md`](references/react-effects.md) |
