...
A group of fields that is closed as one. `disabled` closes every control inside it: the controls this package ships, whether or not they sit in a `Field`, every native control (`<button>`, `<input>`, `<textarea>`), and the `ConfirmDialogAction` of a `ConfirmDialog` inside it, whose popup is portaled out of the `<fieldset>`. A nested `Fieldset` cannot reopen what an outer one closed.
