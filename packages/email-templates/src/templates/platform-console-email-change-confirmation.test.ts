...
import { platformConsoleEmailChangeConfirmationDataSchema } from "./platform-console-email-change-confirmation";

const data = {
  confirm_url: "https://platform.example.test/confirm-email?token=abc",
...
describe("platformConsoleEmailChangeConfirmationDataSchema", () => {
  it("accepts the variables the sender fills in", () => {
    expect(
      platformConsoleEmailChangeConfirmationDataSchema.parse(data)
    ).toEqual(data);
  });

  it("rejects a link that is not http(s)", () => {
    const parsed = platformConsoleEmailChangeConfirmationDataSchema.safeParse({
      ...data,
...
describe("PlatformConsoleEmailChangeConfirmationEmail", () => {
