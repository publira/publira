...
import { adminConsoleEmailChangeConfirmationDataSchema } from "./admin-console-email-change-confirmation";

const data = {
  confirm_url: "https://admin.example.test/confirm-email?token=abc",
...
describe("adminConsoleEmailChangeConfirmationDataSchema", () => {
  it("accepts the variables the sender fills in", () => {
    expect(adminConsoleEmailChangeConfirmationDataSchema.parse(data)).toEqual(
      data
    );
  });

  it("rejects a link that is not http(s)", () => {
    const parsed = adminConsoleEmailChangeConfirmationDataSchema.safeParse({
      ...data,
...
describe("AdminConsoleEmailChangeConfirmationEmail", () => {
  it("the ja mail carries the link and the expiry in the given time zone", async () => {
    ...
    expect(result.subject).toBe("青灯書房 管理画面メールアドレス変更確認");
  ...
  it("the en mail comes from the English catalog and its own time zone", async () => {
    ...
    expect(result.subject).toBe(
      "青灯書房 admin console email address change confirmation"
    );
    expect(result.html).toContain(
