...
- バケット: `publira`。`task dev` が `task storage:init` で冪等に作成し、同時にそれをプラットフォームのオブジェクトストレージとしてデータベースに保存します（サーバーは環境変数ではなくそこから読みます）。`task setup` はさらに `task storage:seed` で開発シードの画像を配置します
