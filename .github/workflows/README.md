...
Nightly full builds find cross-service drift that filters cannot catch. Host CI does not run nightly except **Test / Bootstrap**, which monitors rarely changed paths such as `compose.yaml` and `.devcontainer/**`.
...
| `Test / Bootstrap` | `compose.yaml`, Dev Container, db, bootstrap, apps, packages, server, Taskfile, build inputs, storage init |
