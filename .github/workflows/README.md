...
| `Docker / <target>` | `task docker:build:*`（web は続けて `task docker:smoke:web`、email-renderer は `task docker:smoke:email-renderer`） | [`infra/docker/README.md`](../../infra/docker/README.md) |
...
`Docker / <target>` は matrix ジョブで、`scripts/ci-plan-jobs.sh` が組み立てた行列（ロール・ターゲット・`task` 名・build arg）をそのまま実行する。コマンドはローカルと同一の `task docker:build:web|api|batch|email-renderer` で、web ロールは続けて `task docker:smoke:web`、email-renderer ロールは `task docker:smoke:email-renderer` を走らせる。
