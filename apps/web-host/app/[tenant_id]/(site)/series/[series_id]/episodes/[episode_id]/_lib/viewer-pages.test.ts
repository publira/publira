...
describe("toViewerPages", () => {
  it("配信順のまま連番のページ名を付ける", () => {
    const pages = toViewerPages("第1話", [
      image({ id: "IMG_001" }),
      image({ displayOrder: 2, id: "IMG_002" }),
    ]);
  ...
  it("寸法が記録されていない画像は寸法を伏せる", () => {
    const [page] = toViewerPages("第1話", [image({ height: 0, width: 0 })]);
  ...
  it("Content-Type が空の画像は mimeType を伏せる", () => {
    const [page] = toViewerPages("第1話", [image({ contentType: "" })]);
