...
- **A Client Component never loads a catalog.** An `import()` of a catalog from the client fetches, after hydration, copy the server already rendered, and a static import ships every locale to the browser. The group layout that resolves the request's locale seeds a provider with that locale's catalog, and a Client Component below it reads its copy from there — `<ClientMessage>` and `useClientMessages()` over `AdminLocaleProvider` in `web-admin` and `PlatformMessagesProvider` in `web-platform` — so the copy is present on the first render and needs no `<Suspense>` of its own. `PlatformMessagesProvider` takes no props and hands down the unresolved read, so the root layout can place it without reading anything itself: every route keeps its static shell, and a component that names a string waits, on the server, at the boundary its section already sits behind. Only a route-level `error.tsx` loads a catalog in the browser: `web-admin`'s sits above the layout that seeds its provider, and `web-platform`'s render when the platform API that read depends on is unreachable. Where no app provider carries the catalog yet (#2125), pass copy in as `ReactNode` through a `copy` prop (`LoginFormCopy`), so each string keeps its boundary
