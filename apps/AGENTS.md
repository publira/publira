...
- **A copy prop takes a `ReactNode`, never a catalog key.** `title="admin.audit.section_error"` is a contract that the component must resolve the key itself, and it hides where the `<Suspense>` lands. Type the prop `ReactNode` and pass `<Suspense><Message /></Suspense>` from the caller, so the shared frame — `ErrorScreen`, `SectionErrorBoundary` — reads no catalog at all
- **Do not wrap that in a helper.** A function whose whole body is one `<Suspense><Message /></Suspense>`, or a factory returning a bag of them to spread onto a component, buys a little less repetition and costs the reader a jump to its definition to learn what the call site does. Write the JSX where it is used, even when the same four strings appear at ten boundaries
