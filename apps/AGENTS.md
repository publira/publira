...
- **A Client Component never loads a catalog.** An `import()` of a catalog from the client fetches, after hydration, copy the server already rendered, and a static import ships every locale to the browser. The group layout that resolves the request's locale seeds a provider with that locale's catalog, and a Client Component below it reads its copy from there — `<ClientMessage>` and `useClientMessages()` over `AdminLocaleProvider` in `web-admin` — so the copy is present on the first render and needs no `<Suspense>`. Only an `error.tsx` above that layout loads a catalog in the browser, because the layout that would seed the provider is what failed. Where no app provider carries the catalog yet (#2125, #2127), pass copy in as `ReactNode` through a `copy` prop (`LoginFormCopy`), so each string keeps its boundary
