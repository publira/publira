...
Norm and enforcement: repository root [AGENTS.md](../AGENTS.md) (React: Effects and useEffectEvent). Decision flow and OK/NG rules: [`skills/coding-standards/references/react-effects.md`](../skills/coding-standards/references/react-effects.md).
...
Keep **both** enabled. Details and env (`PUBLIRA_REDIS_URL`, `PUBLIRA_CACHE_APP`): [`skills/coding-standards/references/nextjs-cache.md`](../skills/coding-standards/references/nextjs-cache.md) and `packages/next-cache-handlers/README.md`.
...
- **A genuine non-icon SVG** (decorative artwork, a chart, a generated image) is a real exception. Add its path to the grep step's exclusions in `.github/workflows/ci.yml`, with a comment saying why — the same way the `Date` boundary is handled in [`skills/coding-standards/references/date-and-time.md`](../skills/coding-standards/references/date-and-time.md).
