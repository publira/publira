...
- **A path with no locale redirects to the tenant's default locale, temporarily.** Bookmarks predate the prefix. `GetTenantByDomain` returns `default_locale` alongside the tenant id precisely so the proxy can pick that redirect in the round trip it already makes; a code this build serves no catalog for throws rather than redirecting somewhere else. The redirect is 307 rather than 308 so a browser caches neither a change to that setting nor a decision `Accept-Language` negotiation would later have to override
