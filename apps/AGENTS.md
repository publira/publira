...
## Shared components: compose slots, do not aggregate values in props

`@publira/layouts` and `@publira/ui-components` use the Compound Component Pattern. A shared component owns only its DOM structure; each independently resolved piece of copy, navigation, branding, or action is passed as a named child slot. Keep each async slot's `<Suspense>` boundary at the call site, where its fallback size is visible.

Props are limited to values that cannot be rendered as a node: HTML attributes such as `href`, `placeholder`, and `aria-label`, and a Server Action. Do not replace a slot with a bag prop (`navigation`, `currentUser`, `userMenuCopy`, `primaryAction`, `options: { label, locale }[]`) or a `T | Promise<T>` prop. A slot that renders a link receives the `href`; its label is the child node. Do not pass Tailwind classes through a layout data object, and do not add a pass-through `*ClassName` prop for a child element — that element is a slot the caller writes.
...
| A public service's own message — the `publira.v1` entities (`AnnouncementItem`, `MyFollow`, `MyPurchase`, `NotificationItem`, `PublishedCreator`) | `@publira/api-client/public/types` (`web-host`), beside the `publira.types.v1` messages |
...
| --- | --- |
| Locale parsing, catalog loading, `{$name}` interpolation | `@publira/i18n` (`parseLocale` / `parseLocaleCookie` / `loadMessages` / `bindMessages`) |
| One string, where a node cannot go | The app's `getMessages()` in `lib/get-messages.ts`, or `getMessagesFor(locale)` in `lib/messages.ts` |
...
- **A section that branches picks the `<Message>`, not the key.** The setup gate and the confirmation screens still decide what to render before anything appears, and each branch writes its own `<Message message="…" />` with the key spelled out where it is rendered. A key that reaches `<Message>` as a variable, a prop, a lookup in a table of keys, or a `condition ? "a.b" : "a.c"` is invisible to anything that reads the source for the strings a screen uses — listing the copy, catching a key missing from `locales/*.json`, finding the keys nothing renders any more. Duplicating the element in both branches is the cost of that; duplicating the key is not an option. Never pass a loaded catalog into a component either: a `messages` prop makes the key an implicit attribute of whatever the caller happened to load, and a key built by interpolation (`platform.auth.confirm_password.${status}`) is not checked against the catalog at all. A provider that carries the request's catalog is not such a component: it carries the catalog rather than chosen strings, and each key is still spelled out at the `<ClientMessage>` or `t("…")` that renders it
- **A localized attribute suspends the control, not the screen.** `placeholder`, `aria-label`, and `title` cannot be nodes, so the one control that carries one resolves the catalog itself and sits behind a `<Suspense>` the size of that control. Never drop the copy to avoid this, and never let it pull the rest of the form in with it. A control inside a `Field` must not wait on anything the browser loads, though: Base UI registers the label and description ids from the `Field`'s own Effects, so a control whose boundary hydrates after them renders an `aria-labelledby` the server HTML does not have, and React reports a hydration mismatch. In a Client Component such an attribute comes from the catalog provider instead, with no boundary at all
- **A string that cannot be a node at all comes from the accessor.** `const t = await getMessages();` answers one bound to the request's locale, and `t("admin.auth.login.title")` is the string — for `generateMetadata`'s `title`, an HTML attribute, a zod schema's wording, and anything crossing into a Server Action. Code that is handed a `Locale` instead of resolving one — a `lib/` mapper called from inside `"use cache"`, a Server Action (`getActionMessages` in `web-admin`) — calls `getMessagesFor(locale)` for the same accessor. `getMessage` is `@publira/i18n`'s own primitive and is not called from an app
- **A Client Component never loads a catalog.** An `import()` of a catalog from the client fetches, after hydration, copy the server already rendered, and a static import ships every locale to the browser. A provider carries the catalog of the request's locale, and a Client Component below it reads its copy from there — `<ClientMessage>` and `useClientMessages()` over `AdminLocaleProvider` in `web-admin`, `PlatformMessagesProvider` in `web-platform`, and `HostMessagesProvider` in `web-host` — so the copy is present on the first client render. Each provider takes no props and hands down the unresolved read, so the layout that places it reads nothing itself: every route keeps its static shell, and a component that names a string waits, on the server, at the boundary its section already sits behind. `web-platform` and `web-host` place theirs in the root layout, so it is not remounted when a navigation crosses route groups; `web-admin` places `AdminLocaleProvider` in `(protected)/layout.tsx`, the layout every signed-in screen shares, and the MFA page, which sits outside it, places its own. Each provider carries its own app's namespace alone — `admin`, `platform`, or `host` — because its value is written into the payload of every page below it, and copy a Client Component takes from another app's namespace is rendered on the server and passed down as a node instead. Only a route-level `error.tsx` loads a catalog in the browser, through `<ErrorBoundaryMessage>`: `web-admin`'s `app/[tenant_id]/error.tsx` sits above the layout that places its provider, and `web-platform`'s render when the platform API that read depends on is unreachable. `web-host`'s catalog read depends on nothing but the locale root parameter, so its error boundaries read the provider like any other Client Component
...
- **A copy prop is for copy that names the caller, and only where no slot can carry it.** `SectionErrorBoundary`'s `title` says which section is missing, so it differs per boundary and belongs to the page; and a boundary's `children` are the section it protects rather than its fallback, so the caller has no element inside the fallback to write that copy on. Everything else the fallback shows — what the reader can do about it, the retry button, the digest label — says the same thing at every boundary, so it is the frame rather than the section and the app's own component resolves it from the catalog instead of taking it as a prop. Judge that by what the copy is about, not by how many call sites happen to exist today. A shared package still takes everything from the caller: `@publira/ui-components` and `@publira/layouts` are used from both a URL-locale app and the cookie apps, so they cannot read a locale at all
- **A shared component holds no copy, not even as a default.** A component in `@publira/ui-components` or `@publira/layouts` cannot resolve a locale, so a default string there is a fixed language — one that reads as Japanese on an English console, and only where the caller forgot it, which is exactly where nobody looks. Every string the component renders therefore comes from the caller. The one exception is a string whose absence is itself a design: an omitted `placeholder` renders nothing, in every language. Autonyms — the language names in a switcher — come from `getLocaleLabel` rather than a catalog, because they are the same in every locale
- **Copy reaches a component through the element that renders it, never through a prop named after one.** This holds for every component that owns its DOM structure: those in `@publira/ui-components` and `@publira/layouts`, and an app's own under `components/` and `app/**/_components/` alike. Such a component follows the Compound Component Pattern: copy that comes from the caller lands in a named child slot the caller writes — visible text as that slot's `children`, an accessible name as a standard `aria-label` on it. An app's component still resolves its frame copy itself, as above. A prop whose only job is to carry a string into an element the call site cannot address (`removeLabel`, `retryLabel`, `digestLabel`, `submitLabel`, `actionText`, `triggerLabel`) says nothing about where it lands, so the caller guesses from the name or reads the component's source; `<MultiComboboxChipRemove aria-label={…} />` and `<ConfirmDialogCancel>{…}</ConfirmDialogCancel>` say it outright. A bag of strings (`copy: { ariaLabel, pending, submit }`) is several such props behind one name. What stays a prop is a value that cannot be a node at all, and it sits on the element that carries it: `href`, `placeholder`, `alt`, a Server Action, and an `aria-label` on the component's own root — spelled `aria-label` and passed through, never renamed `ariaLabel`. When a slot needs something only the framework knows — the digest and the retry callback of a caught error — the component hands that to its own slots through a context (`SectionErrorDigest`, `SectionErrorRetry`) rather than taking the copy instead
- **Wording that changes while a Server Action is in flight is two children of the submit control, not two strings.** Write `<ActionFormIdle>` and `<ActionFormPending>` from `@publira/ui-components/action-form` inside the control, each around the copy it shows. Both read the pending state of the form they sit in through `useFormStatus()`, so neither the component nor its caller picks a string on `isPending`
...
4. The script runs once per document, so every app writes `document.documentElement.lang` itself for whatever changes the language without loading one. A cookie console's switcher does it **after its Action resolves** — writing it in the click handler would leave the document claiming a language that neither the cookie nor the copy on screen agrees with whenever the Action fails. In `web-host` the switcher is a `next/link`, so the choice is a client-side navigation: React re-renders the attribute-less root element and takes the attribute away with it, and a click handler would be overwritten by the very render that follows it. `<DocumentLocale>` sits beside `<LocaleProvider>` in the `(site)` and `(auth)` layouts and synchronizes the attribute from an Effect, which runs after that commit
...
- **The locale is stripped before a path is classified.** `buildTenantRewritePathname` decides "published page or app route" on the locale-less remainder, so a tenant page whose slug happens to be `ja` still resolves under another locale's prefix
- **A published page takes the place of an app route at the same path.** The proxy rewrites a path to `/page/[...slug]` only when the tenant has a published page with that slug (`lib/published-page-slugs.ts`), so a new route under `app/[tenant_id]/[locale]` is served without being registered anywhere. The screens a reader cannot use the site without — signing in and up, a path a mail links to, the account settings — are kept out of a slug's reach by `server/internal/pageslug`, which the admin API refuses a slug by and the public API filters the published slugs by; a new route of that kind is added there
...
- **The root layout is the document shell and nothing else.** It is synchronous and reads nothing, so both locales enter the tree one level down, at the `(site)` and `(auth)` layouts. That is also what puts the tenant read behind `app/[tenant_id]/[locale]/error.tsx`: a stored default that cannot be read brings up that boundary instead of a bare 500 no boundary catches. The boundary stands in for those layouts and has no locale context, but its copy comes from `HostMessagesProvider`, which the root layout places because that read needs no tenant
