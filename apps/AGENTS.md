...
- **A Client Component never loads a catalog.** An `import()` of a catalog from the client fetches, after hydration, copy the server already rendered, and a static import ships every locale to the browser. The group layout that resolves the request's locale seeds a provider with that locale's catalog, and a Client Component below it reads its copy from there — `<ClientMessage>` and `useClientMessages()` over `AdminLocaleProvider` in `web-admin`, `PlatformMessagesProvider` in `web-platform`, and `HostMessagesProvider` in `web-host` — so the copy is present on the first render and needs no `<Suspense>` of its own. `PlatformMessagesProvider` and `HostMessagesProvider` take no props and hand down the unresolved read, so the root layout can place them without reading anything itself: every route keeps its static shell, a component that names a string waits, on the server, at the boundary its section already sits behind, and the provider is not remounted when a navigation crosses route groups. Each provider carries its own app's namespace alone — `admin`, `platform`, or `host` — because its value is written into the payload of every page below it, and copy a Client Component takes from another app's namespace is rendered on the server and passed down as a node instead. Only a route-level `error.tsx` loads a catalog in the browser, through `<ErrorBoundaryMessage>`: `web-admin`'s `app/[tenant_id]/error.tsx` sits above the layout that seeds its provider, and `web-platform`'s render when the platform API that read depends on is unreachable. `web-host`'s catalog read depends on nothing but the locale root parameter, so its error boundaries read the provider like any other Client Component
