...
- **Copy reaches a shared component through the element that renders it, never through a prop named after one.** Both packages follow the Compound Component Pattern: the component owns its DOM structure, and every part a reader sees is a named child slot — so visible text is `children` and an accessible name is a standard `aria-label`, both written on the slot itself. A prop whose only job is to carry a string into an element the call site cannot address (`removeLabel`, `retryLabel`, `digestLabel`, `submitLabel`, `actionText`, a renamed `ariaLabel`) says nothing about where it lands, so the caller guesses from the name or reads the package source; `<MultiComboboxChipRemove aria-label={…} />` and `<ConfirmDialogCancel>{…}</ConfirmDialogCancel>` say it outright. What stays a prop is a value that cannot be a node at all, and it sits on the element that carries it: `href`, `placeholder`, `alt`, a Server Action, and an `aria-label` on a slot's own root. When a slot needs something only the framework knows — the digest and the retry callback of a caught error — the component hands that to its own slots through a context (`SectionErrorDigest`, `SectionErrorRetry`) rather than taking the copy instead
