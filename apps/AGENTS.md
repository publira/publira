...
- **`getMessage` is for values that cannot be nodes.** In practice that is `generateMetadata`'s `title` and anything crossing into a Server Action. An attribute such as `placeholder` cannot stream either — drop the copy or move it into the label rather than resolving a catalog just for it
