...
- **A copy prop takes a `ReactNode`, never a catalog key.** `title="admin.audit.section_error"` is a contract that the component must resolve the key itself, and it hides where the `<Suspense>` lands. Type the prop `ReactNode` and pass `<Suspense><Message /></Suspense>` from the caller, so the shared frame — `ErrorScreen`, `SectionErrorBoundary` — reads no catalog at all. Where one component needs several of these, a local factory that returns the nodes (`sectionErrorCopy(key)`) is spread onto it; the `<Suspense>` is still written out, once, next to the fallback size it belongs to
