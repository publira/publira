...
import { ThemePreviewFrame } from "./theme-preview-frame";
...
const WorkRow = ({
  ...
  <div className="flex items-center gap-3 py-3">
    ...
    <Badge tone="success">
      <Suspense fallback={<SkeletonLine className="h-3 w-8" />}>
 ...
 *
 * The copy is rendered here on the server so that the site's `host.*` strings
 * never join the catalog `AdminLocaleProvider` writes into every console page;
 * only the frame reads the form's colors.
 */
    ...
    <div className="flex flex-wrap items-center gap-3 border-b border-border bg-surface px-4 py-3 text-surface-foreground">
      <span className="font-serif text-lg font-medium">
        <Suspense fallback={<SkeletonLine className="h-5 w-28" />}>
          <Message message="admin.settings.theme.preview.site_name" />
        </Suspense>
      </span>
    ...
    <div className="grid gap-6 px-4 py-5">
      <div className="flex flex-wrap gap-x-4 gap-y-1 text-sm text-muted-foreground">
        <span className="font-serif">
          <Suspense fallback={<SkeletonLine className="h-4 w-40" />}>
            <Message message="admin.settings.theme.preview.type_sample" />
          </Suspense>
        </span>
        <span>
          <Suspense fallback={<SkeletonLine className="h-4 w-40" />}>
        ...
        <div className="flex aspect-16/7 items-center justify-center rounded-surface bg-muted p-3 text-center">
          <span className="font-serif text-lg leading-tight text-muted-foreground">
            <Suspense fallback={<SkeletonLine className="h-5 w-48" />}>
              <Message message="admin.settings.theme.preview.works.first.title" />
            </Suspense>
          </span>
        </div>
        <p className="mt-3 font-serif text-xl leading-tight">
          <Suspense fallback={<SkeletonLine className="h-6 w-56" />}>
            <Message message="admin.settings.theme.preview.works.first.title" />
          </Suspense>
        </p>
        <p className="mt-1 text-sm text-muted-foreground">
          <Suspense fallback={<SkeletonLine className="h-4 w-24" />}>
            <Message message="admin.settings.theme.preview.works.first.author" />
          </Suspense>
        </p>
        <div className="mt-3 flex flex-wrap items-baseline justify-between gap-3">
          <p className="min-w-0 flex-1 text-xs text-muted-foreground">
            <Suspense fallback={<SkeletonLine className="h-3 w-full" />}>
              <Message message="admin.settings.theme.preview.works.first.synopsis" />
            </Suspense>
          </p>
          <div className="flex shrink-0 items-center gap-2">
            <span className={buttonVariants({ size: "sm" })}>
              <Suspense fallback={<SkeletonLine className="h-4 w-20" />}>
                <Message message="host.common.view_series_detail" />
              </Suspense>
            </span>
            <span
      ...
      <section>
        ...
        <div className="divide-y divide-border">
          ...
          <WorkRow
            author={
              <Suspense fallback={<SkeletonLine className="h-3 w-20" />}>
                <Message message="admin.settings.theme.preview.works.second.author" />
              </Suspense>
            }
            title={
              <Suspense fallback={<SkeletonLine className="h-4 w-40" />}>
                <Message message="admin.settings.theme.preview.works.second.title" />
              </Suspense>
            }
          />
        </div>
      ...
      <div className="flex flex-wrap gap-2">
        <Badge tone="success" variant="solid">
          <Suspense fallback={<SkeletonLine className="h-3 w-12" />}>
            <Message message="admin.settings.theme.preview.status.success" />
          </Suspense>
        </Badge>
        <Badge tone="warning" variant="solid">
          <Suspense fallback={<SkeletonLine className="h-3 w-12" />}>
            <Message message="admin.settings.theme.preview.status.warning" />
          </Suspense>
        </Badge>
        <Badge tone="destructive" variant="solid">
          <Suspense fallback={<SkeletonLine className="h-3 w-12" />}>
            <Message message="admin.settings.theme.preview.status.destructive" />
          </Suspense>
        </Badge>
        <Badge tone="info" variant="solid">
          <Suspense fallback={<SkeletonLine className="h-3 w-12" />}>
            <Message message="admin.settings.theme.preview.status.info" />
          </Suspense>
        </Badge>
      </div>
    </div>
