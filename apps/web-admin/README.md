...
- `proxy.ts` publishes the tenant's saved default as `publira_resolved_locale` on every response, from the Host-to-tenant resolution it makes to route the request anyway (`@publira/utils/resolved-locale`). It is the only way the browser learns that value, and both the `<head>` script and the client error boundary read it
