...
| One string in a Client Component | `<ClientMessage>` / `useClientMessages()` in `components/client-message.tsx`, over the `admin` namespace `AdminLocaleProvider` (`components/admin-locale-context.tsx`) carries from the layout that resolved the locale, loaded by `loadAdminClientMessages(locale)` |
