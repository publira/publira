...
export const authTokenFormSchema = async (locale: Locale) => {
  ...
  return z
    ...
    .refine((value) => AUTH_TOKEN_PATTERN.test(value), {
      error: t("platform.auth.fields.invalid_token"),
    });
};
...
export const emailFormSchema = async (locale: Locale) => {
  const t = await getMessagesFor(locale);
  const required = t("platform.auth.fields.email_required");

  return z
    .string({ error: required })
    .trim()
    .min(1, required)
    .pipe(z.email(t("platform.auth.fields.email_invalid")));
};

export const passwordFormSchema = async (locale: Locale) => {
  const t = await getMessagesFor(locale);
  const required = t("platform.auth.fields.password_required");
