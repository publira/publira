 ...
 * they are serving. The proxy is the one place that sees it on every request,
 * protected route and Server Action POST alike, so it writes the path once and
 * everything downstream reads it instead of hard-coding a route it happens to
 * sit on.
 *
 ...
 * elapsed — the same two checks `resolveAccessToken()` makes before it hands the
 * token to an RPC, so the proxy never admits a request the console will reject.
 *
 * The raw-JWT fallback in `lib/api-client.ts` is deliberately not honoured here:
 * it is transitional, and no cookie the login Action writes today has that shape.
 */
export const hasActivePlatformSessionCookie = async (
  value?: string | null
): Promise<boolean> => {
