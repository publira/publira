...
const EpisodeDetailData = async (
  props: PageProps<"/[tenant_public_id]/series/[series_id]/episodes/[episode_id]">
) => {
  const { episode_id, series_id, tenant_public_id } = await props.params;
  guardPlaceholders({ episode_id, series_id, tenant_public_id });

  let result;
  try {
    result = await getEpisodeDetail(tenant_public_id, series_id, episode_id);
  } catch (error) {
    const message =
