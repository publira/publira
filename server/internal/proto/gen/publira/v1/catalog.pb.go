// Code generated by protoc-gen-go. DO NOT EDIT.
// versions:
// 	protoc-gen-go v1.36.12
// 	protoc        (unknown)
// source: publira/v1/catalog.proto

package publirav1

import (
	v1 "github.com/publira/publira/server/internal/proto/gen/publira/types/v1"
	protoreflect "google.golang.org/protobuf/reflect/protoreflect"
	protoimpl "google.golang.org/protobuf/runtime/protoimpl"
	reflect "reflect"
	sync "sync"
	unsafe "unsafe"
)

const (
	// Verify that this generated code is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(20 - protoimpl.MinVersion)
	// Verify that runtime/protoimpl is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(protoimpl.MaxVersion - 20)
)

// Sort order of a published series list.
type SeriesOrder int32

const (
	// Same as SERIES_ORDER_PUBLISHED_AT_DESC.
	SeriesOrder_SERIES_ORDER_UNSPECIFIED SeriesOrder = 0
	// Newest published first.
	SeriesOrder_SERIES_ORDER_PUBLISHED_AT_DESC SeriesOrder = 1
	// Oldest published first.
	SeriesOrder_SERIES_ORDER_PUBLISHED_AT_ASC SeriesOrder = 2
	// By title, ascending.
	SeriesOrder_SERIES_ORDER_TITLE_ASC SeriesOrder = 3
	// By title, descending.
	SeriesOrder_SERIES_ORDER_TITLE_DESC SeriesOrder = 4
	// By the most recent episode of each series, newest first. A series whose
	// episodes are all still unpublished sorts by its own publication instant,
	// which is the last thing that happened to it.
	SeriesOrder_SERIES_ORDER_LATEST_EPISODE_AT_DESC SeriesOrder = 5
)

// Enum value maps for SeriesOrder.
var (
	SeriesOrder_name = map[int32]string{
		0: "SERIES_ORDER_UNSPECIFIED",
		1: "SERIES_ORDER_PUBLISHED_AT_DESC",
		2: "SERIES_ORDER_PUBLISHED_AT_ASC",
		3: "SERIES_ORDER_TITLE_ASC",
		4: "SERIES_ORDER_TITLE_DESC",
		5: "SERIES_ORDER_LATEST_EPISODE_AT_DESC",
	}
	SeriesOrder_value = map[string]int32{
		"SERIES_ORDER_UNSPECIFIED":            0,
		"SERIES_ORDER_PUBLISHED_AT_DESC":      1,
		"SERIES_ORDER_PUBLISHED_AT_ASC":       2,
		"SERIES_ORDER_TITLE_ASC":              3,
		"SERIES_ORDER_TITLE_DESC":             4,
		"SERIES_ORDER_LATEST_EPISODE_AT_DESC": 5,
	}
)

func (x SeriesOrder) Enum() *SeriesOrder {
	p := new(SeriesOrder)
	*p = x
	return p
}

func (x SeriesOrder) String() string {
	return protoimpl.X.EnumStringOf(x.Descriptor(), protoreflect.EnumNumber(x))
}

func (SeriesOrder) Descriptor() protoreflect.EnumDescriptor {
	return file_publira_v1_catalog_proto_enumTypes[0].Descriptor()
}

func (SeriesOrder) Type() protoreflect.EnumType {
	return &file_publira_v1_catalog_proto_enumTypes[0]
}

func (x SeriesOrder) Number() protoreflect.EnumNumber {
	return protoreflect.EnumNumber(x)
}

// Deprecated: Use SeriesOrder.Descriptor instead.
func (SeriesOrder) EnumDescriptor() ([]byte, []int) {
	return file_publira_v1_catalog_proto_rawDescGZIP(), []int{0}
}

// Viewer access state for an episode body on GetEpisodeDetail.
type EpisodeAccess int32

const (
	EpisodeAccess_EPISODE_ACCESS_UNSPECIFIED EpisodeAccess = 0
	// The body is public: price is 0, or a scheduled free window is open on it.
	EpisodeAccess_EPISODE_ACCESS_FREE EpisodeAccess = 1
	// paid and no valid purchase/ticket grant.
	EpisodeAccess_EPISODE_ACCESS_LOCKED EpisodeAccess = 2
	// valid purchase or active access ticket.
	EpisodeAccess_EPISODE_ACCESS_ENTITLED EpisodeAccess = 3
	// The tenant makes a reader prove an age for this series' rating and this
	// reader has not: they are reading as a guest, have given no birth date, or
	// are younger than the rating asks for. It outranks the three above, so a
	// free body and a purchased one are both withheld, and it is answered the
	// same way as EPISODE_ACCESS_LOCKED — no images and no media token.
	EpisodeAccess_EPISODE_ACCESS_AGE_RESTRICTED EpisodeAccess = 4
)

// Enum value maps for EpisodeAccess.
var (
	EpisodeAccess_name = map[int32]string{
		0: "EPISODE_ACCESS_UNSPECIFIED",
		1: "EPISODE_ACCESS_FREE",
		2: "EPISODE_ACCESS_LOCKED",
		3: "EPISODE_ACCESS_ENTITLED",
		4: "EPISODE_ACCESS_AGE_RESTRICTED",
	}
	EpisodeAccess_value = map[string]int32{
		"EPISODE_ACCESS_UNSPECIFIED":    0,
		"EPISODE_ACCESS_FREE":           1,
		"EPISODE_ACCESS_LOCKED":         2,
		"EPISODE_ACCESS_ENTITLED":       3,
		"EPISODE_ACCESS_AGE_RESTRICTED": 4,
	}
)

func (x EpisodeAccess) Enum() *EpisodeAccess {
	p := new(EpisodeAccess)
	*p = x
	return p
}

func (x EpisodeAccess) String() string {
	return protoimpl.X.EnumStringOf(x.Descriptor(), protoreflect.EnumNumber(x))
}

func (EpisodeAccess) Descriptor() protoreflect.EnumDescriptor {
	return file_publira_v1_catalog_proto_enumTypes[1].Descriptor()
}

func (EpisodeAccess) Type() protoreflect.EnumType {
	return &file_publira_v1_catalog_proto_enumTypes[1]
}

func (x EpisodeAccess) Number() protoreflect.EnumNumber {
	return protoreflect.EnumNumber(x)
}

// Deprecated: Use EpisodeAccess.Descriptor instead.
func (EpisodeAccess) EnumDescriptor() ([]byte, []int) {
	return file_publira_v1_catalog_proto_rawDescGZIP(), []int{1}
}

// Whether behavioural signals are behind a recommendation list at all. It
// describes the tenant rather than the page: a later page of a ranked list is
// all new arrivals, and still says RANKING. The list itself cannot be read for
// this — a ranked list and its fallback are both just series.
type RecommendationSource int32

const (
	RecommendationSource_RECOMMENDATION_SOURCE_UNSPECIFIED RecommendationSource = 0
	// A ranking snapshot decided the front of the list.
	RecommendationSource_RECOMMENDATION_SOURCE_RANKING RecommendationSource = 1
	// The tenant has no usable ranking snapshot, so the whole list is ordered
	// newest first.
	RecommendationSource_RECOMMENDATION_SOURCE_NEW_ARRIVALS RecommendationSource = 2
)

// Enum value maps for RecommendationSource.
var (
	RecommendationSource_name = map[int32]string{
		0: "RECOMMENDATION_SOURCE_UNSPECIFIED",
		1: "RECOMMENDATION_SOURCE_RANKING",
		2: "RECOMMENDATION_SOURCE_NEW_ARRIVALS",
	}
	RecommendationSource_value = map[string]int32{
		"RECOMMENDATION_SOURCE_UNSPECIFIED":  0,
		"RECOMMENDATION_SOURCE_RANKING":      1,
		"RECOMMENDATION_SOURCE_NEW_ARRIVALS": 2,
	}
)

func (x RecommendationSource) Enum() *RecommendationSource {
	p := new(RecommendationSource)
	*p = x
	return p
}

func (x RecommendationSource) String() string {
	return protoimpl.X.EnumStringOf(x.Descriptor(), protoreflect.EnumNumber(x))
}

func (RecommendationSource) Descriptor() protoreflect.EnumDescriptor {
	return file_publira_v1_catalog_proto_enumTypes[2].Descriptor()
}

func (RecommendationSource) Type() protoreflect.EnumType {
	return &file_publira_v1_catalog_proto_enumTypes[2]
}

func (x RecommendationSource) Number() protoreflect.EnumNumber {
	return protoreflect.EnumNumber(x)
}

// Deprecated: Use RecommendationSource.Descriptor instead.
func (RecommendationSource) EnumDescriptor() ([]byte, []int) {
	return file_publira_v1_catalog_proto_rawDescGZIP(), []int{2}
}

// The window a ranking snapshot covers. The batch writes one snapshot per
// period on every run, so a tenant that has one has both.
type RankingPeriod int32

const (
	// Same as RANKING_PERIOD_DAILY.
	RankingPeriod_RANKING_PERIOD_UNSPECIFIED RankingPeriod = 0
	// The single day the last run ranked.
	RankingPeriod_RANKING_PERIOD_DAILY RankingPeriod = 1
	// The seven days ending on that same day.
	RankingPeriod_RANKING_PERIOD_WEEKLY RankingPeriod = 2
)

// Enum value maps for RankingPeriod.
var (
	RankingPeriod_name = map[int32]string{
		0: "RANKING_PERIOD_UNSPECIFIED",
		1: "RANKING_PERIOD_DAILY",
		2: "RANKING_PERIOD_WEEKLY",
	}
	RankingPeriod_value = map[string]int32{
		"RANKING_PERIOD_UNSPECIFIED": 0,
		"RANKING_PERIOD_DAILY":       1,
		"RANKING_PERIOD_WEEKLY":      2,
	}
)

func (x RankingPeriod) Enum() *RankingPeriod {
	p := new(RankingPeriod)
	*p = x
	return p
}

func (x RankingPeriod) String() string {
	return protoimpl.X.EnumStringOf(x.Descriptor(), protoreflect.EnumNumber(x))
}

func (RankingPeriod) Descriptor() protoreflect.EnumDescriptor {
	return file_publira_v1_catalog_proto_enumTypes[3].Descriptor()
}

func (RankingPeriod) Type() protoreflect.EnumType {
	return &file_publira_v1_catalog_proto_enumTypes[3]
}

func (x RankingPeriod) Number() protoreflect.EnumNumber {
	return protoreflect.EnumNumber(x)
}

// Deprecated: Use RankingPeriod.Descriptor instead.
func (RankingPeriod) EnumDescriptor() ([]byte, []int) {
	return file_publira_v1_catalog_proto_rawDescGZIP(), []int{3}
}

// Kinds of public catalog targets a member can follow.
type FollowTargetType int32

const (
	FollowTargetType_FOLLOW_TARGET_TYPE_UNSPECIFIED FollowTargetType = 0
	FollowTargetType_FOLLOW_TARGET_TYPE_EPISODE     FollowTargetType = 1
	FollowTargetType_FOLLOW_TARGET_TYPE_CREATOR     FollowTargetType = 2
	FollowTargetType_FOLLOW_TARGET_TYPE_SERIES      FollowTargetType = 3
)

// Enum value maps for FollowTargetType.
var (
	FollowTargetType_name = map[int32]string{
		0: "FOLLOW_TARGET_TYPE_UNSPECIFIED",
		1: "FOLLOW_TARGET_TYPE_EPISODE",
		2: "FOLLOW_TARGET_TYPE_CREATOR",
		3: "FOLLOW_TARGET_TYPE_SERIES",
	}
	FollowTargetType_value = map[string]int32{
		"FOLLOW_TARGET_TYPE_UNSPECIFIED": 0,
		"FOLLOW_TARGET_TYPE_EPISODE":     1,
		"FOLLOW_TARGET_TYPE_CREATOR":     2,
		"FOLLOW_TARGET_TYPE_SERIES":      3,
	}
)

func (x FollowTargetType) Enum() *FollowTargetType {
	p := new(FollowTargetType)
	*p = x
	return p
}

func (x FollowTargetType) String() string {
	return protoimpl.X.EnumStringOf(x.Descriptor(), protoreflect.EnumNumber(x))
}

func (FollowTargetType) Descriptor() protoreflect.EnumDescriptor {
	return file_publira_v1_catalog_proto_enumTypes[4].Descriptor()
}

func (FollowTargetType) Type() protoreflect.EnumType {
	return &file_publira_v1_catalog_proto_enumTypes[4]
}

func (x FollowTargetType) Number() protoreflect.EnumNumber {
	return protoreflect.EnumNumber(x)
}

// Deprecated: Use FollowTargetType.Descriptor instead.
func (FollowTargetType) EnumDescriptor() ([]byte, []int) {
	return file_publira_v1_catalog_proto_rawDescGZIP(), []int{4}
}

// How expressive an episode rating is, which a tenant decides and a series may
// override. It reaches the client so the control knows whether to render a
// plain button or one that can be pressed again.
type EpisodeRatingMode int32

const (
	EpisodeRatingMode_EPISODE_RATING_MODE_UNSPECIFIED EpisodeRatingMode = 0
	// One press says everything the reader can say, and stores the full 5.
	EpisodeRatingMode_EPISODE_RATING_MODE_SINGLE EpisodeRatingMode = 1
	// The reader presses their way up to 5.
	EpisodeRatingMode_EPISODE_RATING_MODE_MULTIPLE EpisodeRatingMode = 2
)

// Enum value maps for EpisodeRatingMode.
var (
	EpisodeRatingMode_name = map[int32]string{
		0: "EPISODE_RATING_MODE_UNSPECIFIED",
		1: "EPISODE_RATING_MODE_SINGLE",
		2: "EPISODE_RATING_MODE_MULTIPLE",
	}
	EpisodeRatingMode_value = map[string]int32{
		"EPISODE_RATING_MODE_UNSPECIFIED": 0,
		"EPISODE_RATING_MODE_SINGLE":      1,
		"EPISODE_RATING_MODE_MULTIPLE":    2,
	}
)

func (x EpisodeRatingMode) Enum() *EpisodeRatingMode {
	p := new(EpisodeRatingMode)
	*p = x
	return p
}

func (x EpisodeRatingMode) String() string {
	return protoimpl.X.EnumStringOf(x.Descriptor(), protoreflect.EnumNumber(x))
}

func (EpisodeRatingMode) Descriptor() protoreflect.EnumDescriptor {
	return file_publira_v1_catalog_proto_enumTypes[5].Descriptor()
}

func (EpisodeRatingMode) Type() protoreflect.EnumType {
	return &file_publira_v1_catalog_proto_enumTypes[5]
}

func (x EpisodeRatingMode) Number() protoreflect.EnumNumber {
	return protoreflect.EnumNumber(x)
}

// Deprecated: Use EpisodeRatingMode.Descriptor instead.
func (EpisodeRatingMode) EnumDescriptor() ([]byte, []int) {
	return file_publira_v1_catalog_proto_rawDescGZIP(), []int{5}
}

// Kinds of public catalog targets a page view can be recorded for.
type ContentViewTargetType int32

const (
	ContentViewTargetType_CONTENT_VIEW_TARGET_TYPE_UNSPECIFIED ContentViewTargetType = 0
	ContentViewTargetType_CONTENT_VIEW_TARGET_TYPE_SERIES      ContentViewTargetType = 1
	ContentViewTargetType_CONTENT_VIEW_TARGET_TYPE_EPISODE     ContentViewTargetType = 2
)

// Enum value maps for ContentViewTargetType.
var (
	ContentViewTargetType_name = map[int32]string{
		0: "CONTENT_VIEW_TARGET_TYPE_UNSPECIFIED",
		1: "CONTENT_VIEW_TARGET_TYPE_SERIES",
		2: "CONTENT_VIEW_TARGET_TYPE_EPISODE",
	}
	ContentViewTargetType_value = map[string]int32{
		"CONTENT_VIEW_TARGET_TYPE_UNSPECIFIED": 0,
		"CONTENT_VIEW_TARGET_TYPE_SERIES":      1,
		"CONTENT_VIEW_TARGET_TYPE_EPISODE":     2,
	}
)

func (x ContentViewTargetType) Enum() *ContentViewTargetType {
	p := new(ContentViewTargetType)
	*p = x
	return p
}

func (x ContentViewTargetType) String() string {
	return protoimpl.X.EnumStringOf(x.Descriptor(), protoreflect.EnumNumber(x))
}

func (ContentViewTargetType) Descriptor() protoreflect.EnumDescriptor {
	return file_publira_v1_catalog_proto_enumTypes[6].Descriptor()
}

func (ContentViewTargetType) Type() protoreflect.EnumType {
	return &file_publira_v1_catalog_proto_enumTypes[6]
}

func (x ContentViewTargetType) Number() protoreflect.EnumNumber {
	return protoreflect.EnumNumber(x)
}

// Deprecated: Use ContentViewTargetType.Descriptor instead.
func (ContentViewTargetType) EnumDescriptor() ([]byte, []int) {
	return file_publira_v1_catalog_proto_rawDescGZIP(), []int{6}
}

// The store an app buys through with the store's own in-app purchase.
type InAppPurchaseStore int32

const (
	InAppPurchaseStore_IN_APP_PURCHASE_STORE_UNSPECIFIED InAppPurchaseStore = 0
	InAppPurchaseStore_IN_APP_PURCHASE_STORE_APP_STORE   InAppPurchaseStore = 1
	InAppPurchaseStore_IN_APP_PURCHASE_STORE_GOOGLE_PLAY InAppPurchaseStore = 2
)

// Enum value maps for InAppPurchaseStore.
var (
	InAppPurchaseStore_name = map[int32]string{
		0: "IN_APP_PURCHASE_STORE_UNSPECIFIED",
		1: "IN_APP_PURCHASE_STORE_APP_STORE",
		2: "IN_APP_PURCHASE_STORE_GOOGLE_PLAY",
	}
	InAppPurchaseStore_value = map[string]int32{
		"IN_APP_PURCHASE_STORE_UNSPECIFIED": 0,
		"IN_APP_PURCHASE_STORE_APP_STORE":   1,
		"IN_APP_PURCHASE_STORE_GOOGLE_PLAY": 2,
	}
)

func (x InAppPurchaseStore) Enum() *InAppPurchaseStore {
	p := new(InAppPurchaseStore)
	*p = x
	return p
}

func (x InAppPurchaseStore) String() string {
	return protoimpl.X.EnumStringOf(x.Descriptor(), protoreflect.EnumNumber(x))
}

func (InAppPurchaseStore) Descriptor() protoreflect.EnumDescriptor {
	return file_publira_v1_catalog_proto_enumTypes[7].Descriptor()
}

func (InAppPurchaseStore) Type() protoreflect.EnumType {
	return &file_publira_v1_catalog_proto_enumTypes[7]
}

func (x InAppPurchaseStore) Number() protoreflect.EnumNumber {
	return protoreflect.EnumNumber(x)
}

// Deprecated: Use InAppPurchaseStore.Descriptor instead.
func (InAppPurchaseStore) EnumDescriptor() ([]byte, []int) {
	return file_publira_v1_catalog_proto_rawDescGZIP(), []int{7}
}

type StartEpisodeCheckoutRequest_Client int32

const (
	StartEpisodeCheckoutRequest_CLIENT_UNSPECIFIED StartEpisodeCheckoutRequest_Client = 0
	StartEpisodeCheckoutRequest_CLIENT_WEB         StartEpisodeCheckoutRequest_Client = 1
	StartEpisodeCheckoutRequest_CLIENT_MOBILE      StartEpisodeCheckoutRequest_Client = 2
)

// Enum value maps for StartEpisodeCheckoutRequest_Client.
var (
	StartEpisodeCheckoutRequest_Client_name = map[int32]string{
		0: "CLIENT_UNSPECIFIED",
		1: "CLIENT_WEB",
		2: "CLIENT_MOBILE",
	}
	StartEpisodeCheckoutRequest_Client_value = map[string]int32{
		"CLIENT_UNSPECIFIED": 0,
		"CLIENT_WEB":         1,
		"CLIENT_MOBILE":      2,
	}
)

func (x StartEpisodeCheckoutRequest_Client) Enum() *StartEpisodeCheckoutRequest_Client {
	p := new(StartEpisodeCheckoutRequest_Client)
	*p = x
	return p
}

func (x StartEpisodeCheckoutRequest_Client) String() string {
	return protoimpl.X.EnumStringOf(x.Descriptor(), protoreflect.EnumNumber(x))
}

func (StartEpisodeCheckoutRequest_Client) Descriptor() protoreflect.EnumDescriptor {
	return file_publira_v1_catalog_proto_enumTypes[8].Descriptor()
}

func (StartEpisodeCheckoutRequest_Client) Type() protoreflect.EnumType {
	return &file_publira_v1_catalog_proto_enumTypes[8]
}

func (x StartEpisodeCheckoutRequest_Client) Number() protoreflect.EnumNumber {
	return protoreflect.EnumNumber(x)
}

// Deprecated: Use StartEpisodeCheckoutRequest_Client.Descriptor instead.
func (StartEpisodeCheckoutRequest_Client) EnumDescriptor() ([]byte, []int) {
	return file_publira_v1_catalog_proto_rawDescGZIP(), []int{77, 0}
}

// Cursor pagination. Field shape and token rules: proto/README.md.
type ListPublishedLabelsRequest struct {
	state  protoimpl.MessageState `protogen:"open.v1"`
	Tenant *v1.TenantContext      `protobuf:"bytes,1,opt,name=tenant,proto3" json:"tenant,omitempty"`
	// Max items in one page. <= 0 or > 100 falls back to 20.
	Limit int32 `protobuf:"varint,2,opt,name=limit,proto3" json:"limit,omitempty"`
	// Opaque token from a previous response. Empty for the first page.
	Token string `protobuf:"bytes,4,opt,name=token,proto3" json:"token,omitempty"`
	// The client making the read. Unspecified is answered as the web storefront.
	Surface       v1.ClientSurface `protobuf:"varint,5,opt,name=surface,proto3,enum=publira.types.v1.ClientSurface" json:"surface,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ListPublishedLabelsRequest) Reset() {
	*x = ListPublishedLabelsRequest{}
	mi := &file_publira_v1_catalog_proto_msgTypes[0]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ListPublishedLabelsRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ListPublishedLabelsRequest) ProtoMessage() {}

func (x *ListPublishedLabelsRequest) ProtoReflect() protoreflect.Message {
	mi := &file_publira_v1_catalog_proto_msgTypes[0]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ListPublishedLabelsRequest.ProtoReflect.Descriptor instead.
func (*ListPublishedLabelsRequest) Descriptor() ([]byte, []int) {
	return file_publira_v1_catalog_proto_rawDescGZIP(), []int{0}
}

func (x *ListPublishedLabelsRequest) GetTenant() *v1.TenantContext {
	if x != nil {
		return x.Tenant
	}
	return nil
}

func (x *ListPublishedLabelsRequest) GetLimit() int32 {
	if x != nil {
		return x.Limit
	}
	return 0
}

func (x *ListPublishedLabelsRequest) GetToken() string {
	if x != nil {
		return x.Token
	}
	return ""
}

func (x *ListPublishedLabelsRequest) GetSurface() v1.ClientSurface {
	if x != nil {
		return x.Surface
	}
	return v1.ClientSurface(0)
}

type ListPublishedLabelsResponse struct {
	state  protoimpl.MessageState `protogen:"open.v1"`
	Labels []*v1.Label            `protobuf:"bytes,1,rep,name=labels,proto3" json:"labels,omitempty"`
	// Token for the previous page. Empty on the first page.
	PreviousToken string `protobuf:"bytes,2,opt,name=previous_token,json=previousToken,proto3" json:"previous_token,omitempty"`
	// Token for the next page. Empty on the last page.
	NextToken     string `protobuf:"bytes,3,opt,name=next_token,json=nextToken,proto3" json:"next_token,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ListPublishedLabelsResponse) Reset() {
	*x = ListPublishedLabelsResponse{}
	mi := &file_publira_v1_catalog_proto_msgTypes[1]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ListPublishedLabelsResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ListPublishedLabelsResponse) ProtoMessage() {}

func (x *ListPublishedLabelsResponse) ProtoReflect() protoreflect.Message {
	mi := &file_publira_v1_catalog_proto_msgTypes[1]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ListPublishedLabelsResponse.ProtoReflect.Descriptor instead.
func (*ListPublishedLabelsResponse) Descriptor() ([]byte, []int) {
	return file_publira_v1_catalog_proto_rawDescGZIP(), []int{1}
}

func (x *ListPublishedLabelsResponse) GetLabels() []*v1.Label {
	if x != nil {
		return x.Labels
	}
	return nil
}

func (x *ListPublishedLabelsResponse) GetPreviousToken() string {
	if x != nil {
		return x.PreviousToken
	}
	return ""
}

func (x *ListPublishedLabelsResponse) GetNextToken() string {
	if x != nil {
		return x.NextToken
	}
	return ""
}

// Cursor pagination. Field shape and token rules: proto/README.md.
type ListPublishedSeriesRequest struct {
	state  protoimpl.MessageState `protogen:"open.v1"`
	Tenant *v1.TenantContext      `protobuf:"bytes,1,opt,name=tenant,proto3" json:"tenant,omitempty"`
	// Max items in one page. <= 0 or > 100 falls back to 20.
	Limit int32 `protobuf:"varint,2,opt,name=limit,proto3" json:"limit,omitempty"`
	// Opaque token from a previous response. Empty for the first page.
	Token string `protobuf:"bytes,4,opt,name=token,proto3" json:"token,omitempty"`
	// Sort order. A token carries the order it was built for; sending it with a
	// different order is invalid_argument, because the page it points at no
	// longer exists in the new order. Changing the order restarts at page 1.
	Order SeriesOrder `protobuf:"varint,5,opt,name=order,proto3,enum=publira.v1.SeriesOrder" json:"order,omitempty"`
	// Keep only the series a reader can start without paying: those whose
	// free_episode_count is above zero right now. A token carries the filters it
	// was built for next to the order, and for the same reason — the row it names
	// sits somewhere else in a differently filtered list — so sending it with a
	// different filter is invalid_argument and changing the filter restarts at
	// page 1. Every filter below is bound to the token the same way.
	HasFreeEpisodes bool `protobuf:"varint,6,opt,name=has_free_episodes,json=hasFreeEpisodes,proto3" json:"has_free_episodes,omitempty"`
	// Keep only the series carrying this genre, named by the public ID
	// ListPublishedGenres reports. Empty applies no genre filter. A public ID
	// naming no genre of the tenant is not_found rather than an empty list: the
	// storefront asked for a page that does not exist, and a genre of another
	// tenant has to be indistinguishable from one that was never there.
	GenrePublicId string `protobuf:"bytes,7,opt,name=genre_public_id,json=genrePublicId,proto3" json:"genre_public_id,omitempty"`
	// Keep only the series carrying this tag, named by the slug
	// ListPublishedTags reports. Empty applies no tag filter. A slug naming no
	// tag of the tenant is not_found, for the reason genre_public_id is.
	TagSlug string `protobuf:"bytes,8,opt,name=tag_slug,json=tagSlug,proto3" json:"tag_slug,omitempty"`
	// Keep only the series in this serialization state. Unlike the status a
	// series is saved with, where unspecified means the column default, an
	// unspecified status here applies no filter: a list nobody narrowed holds
	// every state.
	Status v1.SeriesStatus `protobuf:"varint,9,opt,name=status,proto3,enum=publira.types.v1.SeriesStatus" json:"status,omitempty"`
	// Keep only the series expecting an episode on this weekday, as an
	// EXTRACT(DOW) number: 0 is Sunday and 6 is Saturday. It is optional because
	// 0 is Sunday rather than "no weekday"; a weekday outside 0 to 6 is
	// invalid_argument.
	Weekday *int32 `protobuf:"varint,10,opt,name=weekday,proto3,oneof" json:"weekday,omitempty"`
	// The client making the read. Unspecified is answered as the web storefront.
	Surface       v1.ClientSurface `protobuf:"varint,11,opt,name=surface,proto3,enum=publira.types.v1.ClientSurface" json:"surface,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ListPublishedSeriesRequest) Reset() {
	*x = ListPublishedSeriesRequest{}
	mi := &file_publira_v1_catalog_proto_msgTypes[2]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ListPublishedSeriesRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ListPublishedSeriesRequest) ProtoMessage() {}

func (x *ListPublishedSeriesRequest) ProtoReflect() protoreflect.Message {
	mi := &file_publira_v1_catalog_proto_msgTypes[2]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ListPublishedSeriesRequest.ProtoReflect.Descriptor instead.
func (*ListPublishedSeriesRequest) Descriptor() ([]byte, []int) {
	return file_publira_v1_catalog_proto_rawDescGZIP(), []int{2}
}

func (x *ListPublishedSeriesRequest) GetTenant() *v1.TenantContext {
	if x != nil {
		return x.Tenant
	}
	return nil
}

func (x *ListPublishedSeriesRequest) GetLimit() int32 {
	if x != nil {
		return x.Limit
	}
	return 0
}

func (x *ListPublishedSeriesRequest) GetToken() string {
	if x != nil {
		return x.Token
	}
	return ""
}

func (x *ListPublishedSeriesRequest) GetOrder() SeriesOrder {
	if x != nil {
		return x.Order
	}
	return SeriesOrder_SERIES_ORDER_UNSPECIFIED
}

func (x *ListPublishedSeriesRequest) GetHasFreeEpisodes() bool {
	if x != nil {
		return x.HasFreeEpisodes
	}
	return false
}

func (x *ListPublishedSeriesRequest) GetGenrePublicId() string {
	if x != nil {
		return x.GenrePublicId
	}
	return ""
}

func (x *ListPublishedSeriesRequest) GetTagSlug() string {
	if x != nil {
		return x.TagSlug
	}
	return ""
}

func (x *ListPublishedSeriesRequest) GetStatus() v1.SeriesStatus {
	if x != nil {
		return x.Status
	}
	return v1.SeriesStatus(0)
}

func (x *ListPublishedSeriesRequest) GetWeekday() int32 {
	if x != nil && x.Weekday != nil {
		return *x.Weekday
	}
	return 0
}

func (x *ListPublishedSeriesRequest) GetSurface() v1.ClientSurface {
	if x != nil {
		return x.Surface
	}
	return v1.ClientSurface(0)
}

type ListPublishedSeriesResponse struct {
	state  protoimpl.MessageState `protogen:"open.v1"`
	Series []*v1.Series           `protobuf:"bytes,1,rep,name=series,proto3" json:"series,omitempty"`
	// Token for the previous page. Empty on the first page.
	PreviousToken string `protobuf:"bytes,2,opt,name=previous_token,json=previousToken,proto3" json:"previous_token,omitempty"`
	// Token for the next page. Empty on the last page.
	NextToken     string `protobuf:"bytes,3,opt,name=next_token,json=nextToken,proto3" json:"next_token,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ListPublishedSeriesResponse) Reset() {
	*x = ListPublishedSeriesResponse{}
	mi := &file_publira_v1_catalog_proto_msgTypes[3]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ListPublishedSeriesResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ListPublishedSeriesResponse) ProtoMessage() {}

func (x *ListPublishedSeriesResponse) ProtoReflect() protoreflect.Message {
	mi := &file_publira_v1_catalog_proto_msgTypes[3]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ListPublishedSeriesResponse.ProtoReflect.Descriptor instead.
func (*ListPublishedSeriesResponse) Descriptor() ([]byte, []int) {
	return file_publira_v1_catalog_proto_rawDescGZIP(), []int{3}
}

func (x *ListPublishedSeriesResponse) GetSeries() []*v1.Series {
	if x != nil {
		return x.Series
	}
	return nil
}

func (x *ListPublishedSeriesResponse) GetPreviousToken() string {
	if x != nil {
		return x.PreviousToken
	}
	return ""
}

func (x *ListPublishedSeriesResponse) GetNextToken() string {
	if x != nil {
		return x.NextToken
	}
	return ""
}

type GetSeriesDetailRequest struct {
	state    protoimpl.MessageState `protogen:"open.v1"`
	Tenant   *v1.TenantContext      `protobuf:"bytes,1,opt,name=tenant,proto3" json:"tenant,omitempty"`
	PublicId string                 `protobuf:"bytes,2,opt,name=public_id,json=publicId,proto3" json:"public_id,omitempty"`
	// The client making the read. Unspecified is answered as the web storefront.
	Surface       v1.ClientSurface `protobuf:"varint,3,opt,name=surface,proto3,enum=publira.types.v1.ClientSurface" json:"surface,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetSeriesDetailRequest) Reset() {
	*x = GetSeriesDetailRequest{}
	mi := &file_publira_v1_catalog_proto_msgTypes[4]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetSeriesDetailRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetSeriesDetailRequest) ProtoMessage() {}

func (x *GetSeriesDetailRequest) ProtoReflect() protoreflect.Message {
	mi := &file_publira_v1_catalog_proto_msgTypes[4]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetSeriesDetailRequest.ProtoReflect.Descriptor instead.
func (*GetSeriesDetailRequest) Descriptor() ([]byte, []int) {
	return file_publira_v1_catalog_proto_rawDescGZIP(), []int{4}
}

func (x *GetSeriesDetailRequest) GetTenant() *v1.TenantContext {
	if x != nil {
		return x.Tenant
	}
	return nil
}

func (x *GetSeriesDetailRequest) GetPublicId() string {
	if x != nil {
		return x.PublicId
	}
	return ""
}

func (x *GetSeriesDetailRequest) GetSurface() v1.ClientSurface {
	if x != nil {
		return x.Surface
	}
	return v1.ClientSurface(0)
}

type GetSeriesDetailResponse struct {
	state    protoimpl.MessageState `protogen:"open.v1"`
	Series   *v1.Series             `protobuf:"bytes,1,opt,name=series,proto3" json:"series,omitempty"`
	Episodes []*v1.Episode          `protobuf:"bytes,2,rep,name=episodes,proto3" json:"episodes,omitempty"`
	// How old the tenant's age rule makes a reader prove they are before it will
	// open this series' bodies, and 0 when the rule asks nothing of it. It is
	// the rating and the tenant setting resolved together, so a client applies
	// one number to the reader in front of it instead of carrying its own copy
	// of which rule covers which rating.
	//
	// It says nothing about the reader, which is what keeps this response the
	// same bytes for everyone: whether a particular reader clears the number is
	// decided where the body is asked for, by GetEpisodeDetail.
	RequiredMinimumAge int32 `protobuf:"varint,3,opt,name=required_minimum_age,json=requiredMinimumAge,proto3" json:"required_minimum_age,omitempty"`
	// How comments on this series' episodes are published: the series' own mode
	// when it states one, and the tenant's when it does not. It is the resolved
	// answer rather than the override, so a client hides or shows the comment
	// section from this field alone.
	//
	// Never COMMENT_MODE_UNSPECIFIED: a series that states nothing follows a
	// tenant setting that always names a mode, and a stored value naming none
	// fails this RPC.
	CommentMode   v1.CommentMode `protobuf:"varint,4,opt,name=comment_mode,json=commentMode,proto3,enum=publira.types.v1.CommentMode" json:"comment_mode,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetSeriesDetailResponse) Reset() {
	*x = GetSeriesDetailResponse{}
	mi := &file_publira_v1_catalog_proto_msgTypes[5]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetSeriesDetailResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetSeriesDetailResponse) ProtoMessage() {}

func (x *GetSeriesDetailResponse) ProtoReflect() protoreflect.Message {
	mi := &file_publira_v1_catalog_proto_msgTypes[5]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetSeriesDetailResponse.ProtoReflect.Descriptor instead.
func (*GetSeriesDetailResponse) Descriptor() ([]byte, []int) {
	return file_publira_v1_catalog_proto_rawDescGZIP(), []int{5}
}

func (x *GetSeriesDetailResponse) GetSeries() *v1.Series {
	if x != nil {
		return x.Series
	}
	return nil
}

func (x *GetSeriesDetailResponse) GetEpisodes() []*v1.Episode {
	if x != nil {
		return x.Episodes
	}
	return nil
}

func (x *GetSeriesDetailResponse) GetRequiredMinimumAge() int32 {
	if x != nil {
		return x.RequiredMinimumAge
	}
	return 0
}

func (x *GetSeriesDetailResponse) GetCommentMode() v1.CommentMode {
	if x != nil {
		return x.CommentMode
	}
	return v1.CommentMode(0)
}

type GetSeriesEpisodeAccessRequest struct {
	state          protoimpl.MessageState `protogen:"open.v1"`
	Tenant         *v1.TenantContext      `protobuf:"bytes,1,opt,name=tenant,proto3" json:"tenant,omitempty"`
	SeriesPublicId string                 `protobuf:"bytes,2,opt,name=series_public_id,json=seriesPublicId,proto3" json:"series_public_id,omitempty"`
	// The client making the read. Unspecified is answered as the web storefront.
	Surface       v1.ClientSurface `protobuf:"varint,3,opt,name=surface,proto3,enum=publira.types.v1.ClientSurface" json:"surface,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetSeriesEpisodeAccessRequest) Reset() {
	*x = GetSeriesEpisodeAccessRequest{}
	mi := &file_publira_v1_catalog_proto_msgTypes[6]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetSeriesEpisodeAccessRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetSeriesEpisodeAccessRequest) ProtoMessage() {}

func (x *GetSeriesEpisodeAccessRequest) ProtoReflect() protoreflect.Message {
	mi := &file_publira_v1_catalog_proto_msgTypes[6]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetSeriesEpisodeAccessRequest.ProtoReflect.Descriptor instead.
func (*GetSeriesEpisodeAccessRequest) Descriptor() ([]byte, []int) {
	return file_publira_v1_catalog_proto_rawDescGZIP(), []int{6}
}

func (x *GetSeriesEpisodeAccessRequest) GetTenant() *v1.TenantContext {
	if x != nil {
		return x.Tenant
	}
	return nil
}

func (x *GetSeriesEpisodeAccessRequest) GetSeriesPublicId() string {
	if x != nil {
		return x.SeriesPublicId
	}
	return ""
}

func (x *GetSeriesEpisodeAccessRequest) GetSurface() v1.ClientSurface {
	if x != nil {
		return x.Surface
	}
	return v1.ClientSurface(0)
}

// What the caller may do with one published episode's body.
type SeriesEpisodeAccess struct {
	state           protoimpl.MessageState `protogen:"open.v1"`
	EpisodePublicId string                 `protobuf:"bytes,1,opt,name=episode_public_id,json=episodePublicId,proto3" json:"episode_public_id,omitempty"`
	// Never EPISODE_ACCESS_UNSPECIFIED.
	Access        EpisodeAccess `protobuf:"varint,2,opt,name=access,proto3,enum=publira.v1.EpisodeAccess" json:"access,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *SeriesEpisodeAccess) Reset() {
	*x = SeriesEpisodeAccess{}
	mi := &file_publira_v1_catalog_proto_msgTypes[7]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *SeriesEpisodeAccess) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*SeriesEpisodeAccess) ProtoMessage() {}

func (x *SeriesEpisodeAccess) ProtoReflect() protoreflect.Message {
	mi := &file_publira_v1_catalog_proto_msgTypes[7]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use SeriesEpisodeAccess.ProtoReflect.Descriptor instead.
func (*SeriesEpisodeAccess) Descriptor() ([]byte, []int) {
	return file_publira_v1_catalog_proto_rawDescGZIP(), []int{7}
}

func (x *SeriesEpisodeAccess) GetEpisodePublicId() string {
	if x != nil {
		return x.EpisodePublicId
	}
	return ""
}

func (x *SeriesEpisodeAccess) GetAccess() EpisodeAccess {
	if x != nil {
		return x.Access
	}
	return EpisodeAccess_EPISODE_ACCESS_UNSPECIFIED
}

type GetSeriesEpisodeAccessResponse struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Every published episode of the series, in the order GetSeriesDetail lists
	// them.
	Episodes      []*SeriesEpisodeAccess `protobuf:"bytes,1,rep,name=episodes,proto3" json:"episodes,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetSeriesEpisodeAccessResponse) Reset() {
	*x = GetSeriesEpisodeAccessResponse{}
	mi := &file_publira_v1_catalog_proto_msgTypes[8]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetSeriesEpisodeAccessResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetSeriesEpisodeAccessResponse) ProtoMessage() {}

func (x *GetSeriesEpisodeAccessResponse) ProtoReflect() protoreflect.Message {
	mi := &file_publira_v1_catalog_proto_msgTypes[8]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetSeriesEpisodeAccessResponse.ProtoReflect.Descriptor instead.
func (*GetSeriesEpisodeAccessResponse) Descriptor() ([]byte, []int) {
	return file_publira_v1_catalog_proto_rawDescGZIP(), []int{8}
}

func (x *GetSeriesEpisodeAccessResponse) GetEpisodes() []*SeriesEpisodeAccess {
	if x != nil {
		return x.Episodes
	}
	return nil
}

type GetEpisodeDetailRequest struct {
	state    protoimpl.MessageState `protogen:"open.v1"`
	Tenant   *v1.TenantContext      `protobuf:"bytes,1,opt,name=tenant,proto3" json:"tenant,omitempty"`
	PublicId string                 `protobuf:"bytes,2,opt,name=public_id,json=publicId,proto3" json:"public_id,omitempty"`
	// The client making the read. Unspecified is answered as the web storefront.
	Surface       v1.ClientSurface `protobuf:"varint,3,opt,name=surface,proto3,enum=publira.types.v1.ClientSurface" json:"surface,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetEpisodeDetailRequest) Reset() {
	*x = GetEpisodeDetailRequest{}
	mi := &file_publira_v1_catalog_proto_msgTypes[9]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetEpisodeDetailRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetEpisodeDetailRequest) ProtoMessage() {}

func (x *GetEpisodeDetailRequest) ProtoReflect() protoreflect.Message {
	mi := &file_publira_v1_catalog_proto_msgTypes[9]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetEpisodeDetailRequest.ProtoReflect.Descriptor instead.
func (*GetEpisodeDetailRequest) Descriptor() ([]byte, []int) {
	return file_publira_v1_catalog_proto_rawDescGZIP(), []int{9}
}

func (x *GetEpisodeDetailRequest) GetTenant() *v1.TenantContext {
	if x != nil {
		return x.Tenant
	}
	return nil
}

func (x *GetEpisodeDetailRequest) GetPublicId() string {
	if x != nil {
		return x.PublicId
	}
	return ""
}

func (x *GetEpisodeDetailRequest) GetSurface() v1.ClientSurface {
	if x != nil {
		return x.Surface
	}
	return v1.ClientSurface(0)
}

// A published episode next to the one being read, in its own series. It holds
// what a link to that episode needs and nothing about the reader, so an
// episode detail carrying it stays as cacheable as it was; what the reader may
// do with the neighbour's body is decided when they open it.
type EpisodeNeighbor struct {
	state      protoimpl.MessageState `protogen:"open.v1"`
	PublicId   string                 `protobuf:"bytes,1,opt,name=public_id,json=publicId,proto3" json:"public_id,omitempty"`
	Title      string                 `protobuf:"bytes,2,opt,name=title,proto3" json:"title,omitempty"`
	OrderIndex int32                  `protobuf:"varint,3,opt,name=order_index,json=orderIndex,proto3" json:"order_index,omitempty"`
	// What the episode costs. It stays the stored price while a free window is
	// open, because that is what the episode costs again once the window closes.
	Price int32 `protobuf:"varint,4,opt,name=price,proto3" json:"price,omitempty"`
	// Whether the body is public right now: price is 0, or a free window is open
	// on it. It is resolved here rather than read off the price, so a link
	// cannot say "paid" about an episode that is free at the moment the reader
	// follows it.
	IsFree bool `protobuf:"varint,5,opt,name=is_free,json=isFree,proto3" json:"is_free,omitempty"`
	// Who the neighbouring episode is credited to, in role priority order. A
	// series whose artist changed part way through credits its episodes
	// differently, so a link to the next one names the team the reader will
	// actually find on it.
	Creators []*v1.Creator `protobuf:"bytes,6,rep,name=creators,proto3" json:"creators,omitempty"`
	// Which surfaces the episode may be bought on, resolved as
	// Episode.purchase_availability is, so a link to a paid episode sold on the
	// other surface alone does not quote a price this surface cannot take.
	PurchaseAvailability v1.SurfaceAvailability `protobuf:"varint,7,opt,name=purchase_availability,json=purchaseAvailability,proto3,enum=publira.types.v1.SurfaceAvailability" json:"purchase_availability,omitempty"`
	unknownFields        protoimpl.UnknownFields
	sizeCache            protoimpl.SizeCache
}

func (x *EpisodeNeighbor) Reset() {
	*x = EpisodeNeighbor{}
	mi := &file_publira_v1_catalog_proto_msgTypes[10]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *EpisodeNeighbor) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*EpisodeNeighbor) ProtoMessage() {}

func (x *EpisodeNeighbor) ProtoReflect() protoreflect.Message {
	mi := &file_publira_v1_catalog_proto_msgTypes[10]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use EpisodeNeighbor.ProtoReflect.Descriptor instead.
func (*EpisodeNeighbor) Descriptor() ([]byte, []int) {
	return file_publira_v1_catalog_proto_rawDescGZIP(), []int{10}
}

func (x *EpisodeNeighbor) GetPublicId() string {
	if x != nil {
		return x.PublicId
	}
	return ""
}

func (x *EpisodeNeighbor) GetTitle() string {
	if x != nil {
		return x.Title
	}
	return ""
}

func (x *EpisodeNeighbor) GetOrderIndex() int32 {
	if x != nil {
		return x.OrderIndex
	}
	return 0
}

func (x *EpisodeNeighbor) GetPrice() int32 {
	if x != nil {
		return x.Price
	}
	return 0
}

func (x *EpisodeNeighbor) GetIsFree() bool {
	if x != nil {
		return x.IsFree
	}
	return false
}

func (x *EpisodeNeighbor) GetCreators() []*v1.Creator {
	if x != nil {
		return x.Creators
	}
	return nil
}

func (x *EpisodeNeighbor) GetPurchaseAvailability() v1.SurfaceAvailability {
	if x != nil {
		return x.PurchaseAvailability
	}
	return v1.SurfaceAvailability(0)
}

// Episode detail for public viewing.
// Body/content is represented as ordered images for the reader UI.
type GetEpisodeDetailResponse struct {
	state   protoimpl.MessageState `protogen:"open.v1"`
	Episode *v1.Episode            `protobuf:"bytes,1,opt,name=episode,proto3" json:"episode,omitempty"`
	Series  *v1.Series             `protobuf:"bytes,2,opt,name=series,proto3" json:"series,omitempty"`
	// Body images. Empty when access is locked (paid, no purchase/ticket).
	Images []*v1.EpisodeImage `protobuf:"bytes,3,rep,name=images,proto3" json:"images,omitempty"`
	// Viewer access state for this episode.
	Access EpisodeAccess `protobuf:"varint,4,opt,name=access,proto3,enum=publira.v1.EpisodeAccess" json:"access,omitempty"`
	// RFC3339 end of the free window open on this episode, empty when none is.
	// A priced episode reads as free until this instant and is locked again
	// afterwards, which is what a countdown on the reader's screen counts down
	// to. An episode whose price is 0 carries no value here: it is not free
	// until anything.
	FreeUntil string `protobuf:"bytes,5,opt,name=free_until,json=freeUntil,proto3" json:"free_until,omitempty"`
	// The published episodes either side of this one in the same series, by
	// order_index and then id, which is the order the series detail lists them
	// in. Unset at the ends of the series, and both unset when the series has
	// this episode alone. A draft or scheduled episode is never one of them, so
	// the pair moves as episodes are published, reordered, or taken down.
	PreviousEpisode *EpisodeNeighbor `protobuf:"bytes,6,opt,name=previous_episode,json=previousEpisode,proto3" json:"previous_episode,omitempty"`
	NextEpisode     *EpisodeNeighbor `protobuf:"bytes,7,opt,name=next_episode,json=nextEpisode,proto3" json:"next_episode,omitempty"`
	unknownFields   protoimpl.UnknownFields
	sizeCache       protoimpl.SizeCache
}

func (x *GetEpisodeDetailResponse) Reset() {
	*x = GetEpisodeDetailResponse{}
	mi := &file_publira_v1_catalog_proto_msgTypes[11]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetEpisodeDetailResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetEpisodeDetailResponse) ProtoMessage() {}

func (x *GetEpisodeDetailResponse) ProtoReflect() protoreflect.Message {
	mi := &file_publira_v1_catalog_proto_msgTypes[11]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetEpisodeDetailResponse.ProtoReflect.Descriptor instead.
func (*GetEpisodeDetailResponse) Descriptor() ([]byte, []int) {
	return file_publira_v1_catalog_proto_rawDescGZIP(), []int{11}
}

func (x *GetEpisodeDetailResponse) GetEpisode() *v1.Episode {
	if x != nil {
		return x.Episode
	}
	return nil
}

func (x *GetEpisodeDetailResponse) GetSeries() *v1.Series {
	if x != nil {
		return x.Series
	}
	return nil
}

func (x *GetEpisodeDetailResponse) GetImages() []*v1.EpisodeImage {
	if x != nil {
		return x.Images
	}
	return nil
}

func (x *GetEpisodeDetailResponse) GetAccess() EpisodeAccess {
	if x != nil {
		return x.Access
	}
	return EpisodeAccess_EPISODE_ACCESS_UNSPECIFIED
}

func (x *GetEpisodeDetailResponse) GetFreeUntil() string {
	if x != nil {
		return x.FreeUntil
	}
	return ""
}

func (x *GetEpisodeDetailResponse) GetPreviousEpisode() *EpisodeNeighbor {
	if x != nil {
		return x.PreviousEpisode
	}
	return nil
}

func (x *GetEpisodeDetailResponse) GetNextEpisode() *EpisodeNeighbor {
	if x != nil {
		return x.NextEpisode
	}
	return nil
}

// A creator that currently has at least one published series.
type PublishedCreator struct {
	state                  protoimpl.MessageState `protogen:"open.v1"`
	PublicId               string                 `protobuf:"bytes,1,opt,name=public_id,json=publicId,proto3" json:"public_id,omitempty"`
	Name                   string                 `protobuf:"bytes,2,opt,name=name,proto3" json:"name,omitempty"`
	ProfileText            string                 `protobuf:"bytes,3,opt,name=profile_text,json=profileText,proto3" json:"profile_text,omitempty"`
	IconImageUrl           string                 `protobuf:"bytes,4,opt,name=icon_image_url,json=iconImageUrl,proto3" json:"icon_image_url,omitempty"`
	IconImageFileSizeBytes int64                  `protobuf:"varint,5,opt,name=icon_image_file_size_bytes,json=iconImageFileSizeBytes,proto3" json:"icon_image_file_size_bytes,omitempty"`
	IconImageUpdatedAt     string                 `protobuf:"bytes,6,opt,name=icon_image_updated_at,json=iconImageUpdatedAt,proto3" json:"icon_image_updated_at,omitempty"`
	PublishedSeriesCount   int32                  `protobuf:"varint,7,opt,name=published_series_count,json=publishedSeriesCount,proto3" json:"published_series_count,omitempty"`
	unknownFields          protoimpl.UnknownFields
	sizeCache              protoimpl.SizeCache
}

func (x *PublishedCreator) Reset() {
	*x = PublishedCreator{}
	mi := &file_publira_v1_catalog_proto_msgTypes[12]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *PublishedCreator) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*PublishedCreator) ProtoMessage() {}

func (x *PublishedCreator) ProtoReflect() protoreflect.Message {
	mi := &file_publira_v1_catalog_proto_msgTypes[12]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use PublishedCreator.ProtoReflect.Descriptor instead.
func (*PublishedCreator) Descriptor() ([]byte, []int) {
	return file_publira_v1_catalog_proto_rawDescGZIP(), []int{12}
}

func (x *PublishedCreator) GetPublicId() string {
	if x != nil {
		return x.PublicId
	}
	return ""
}

func (x *PublishedCreator) GetName() string {
	if x != nil {
		return x.Name
	}
	return ""
}

func (x *PublishedCreator) GetProfileText() string {
	if x != nil {
		return x.ProfileText
	}
	return ""
}

func (x *PublishedCreator) GetIconImageUrl() string {
	if x != nil {
		return x.IconImageUrl
	}
	return ""
}

func (x *PublishedCreator) GetIconImageFileSizeBytes() int64 {
	if x != nil {
		return x.IconImageFileSizeBytes
	}
	return 0
}

func (x *PublishedCreator) GetIconImageUpdatedAt() string {
	if x != nil {
		return x.IconImageUpdatedAt
	}
	return ""
}

func (x *PublishedCreator) GetPublishedSeriesCount() int32 {
	if x != nil {
		return x.PublishedSeriesCount
	}
	return 0
}

// Cursor pagination. Field shape and token rules: proto/README.md.
// Sorted by creator name, then id. There is no order field: name order is the
// only list the storefront needs, and a later selectable order would have to
// carry its name in the token (see proto/README.md).
type ListPublishedCreatorsRequest struct {
	state  protoimpl.MessageState `protogen:"open.v1"`
	Tenant *v1.TenantContext      `protobuf:"bytes,1,opt,name=tenant,proto3" json:"tenant,omitempty"`
	// Max items in one page. <= 0 or > 100 falls back to 20.
	Limit int32 `protobuf:"varint,2,opt,name=limit,proto3" json:"limit,omitempty"`
	// Opaque token from a previous response. Empty for the first page.
	Token string `protobuf:"bytes,3,opt,name=token,proto3" json:"token,omitempty"`
	// The client making the read. Unspecified is answered as the web storefront.
	Surface       v1.ClientSurface `protobuf:"varint,4,opt,name=surface,proto3,enum=publira.types.v1.ClientSurface" json:"surface,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ListPublishedCreatorsRequest) Reset() {
	*x = ListPublishedCreatorsRequest{}
	mi := &file_publira_v1_catalog_proto_msgTypes[13]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ListPublishedCreatorsRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ListPublishedCreatorsRequest) ProtoMessage() {}

func (x *ListPublishedCreatorsRequest) ProtoReflect() protoreflect.Message {
	mi := &file_publira_v1_catalog_proto_msgTypes[13]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ListPublishedCreatorsRequest.ProtoReflect.Descriptor instead.
func (*ListPublishedCreatorsRequest) Descriptor() ([]byte, []int) {
	return file_publira_v1_catalog_proto_rawDescGZIP(), []int{13}
}

func (x *ListPublishedCreatorsRequest) GetTenant() *v1.TenantContext {
	if x != nil {
		return x.Tenant
	}
	return nil
}

func (x *ListPublishedCreatorsRequest) GetLimit() int32 {
	if x != nil {
		return x.Limit
	}
	return 0
}

func (x *ListPublishedCreatorsRequest) GetToken() string {
	if x != nil {
		return x.Token
	}
	return ""
}

func (x *ListPublishedCreatorsRequest) GetSurface() v1.ClientSurface {
	if x != nil {
		return x.Surface
	}
	return v1.ClientSurface(0)
}

type ListPublishedCreatorsResponse struct {
	state    protoimpl.MessageState `protogen:"open.v1"`
	Creators []*PublishedCreator    `protobuf:"bytes,1,rep,name=creators,proto3" json:"creators,omitempty"`
	// Token for the previous page. Empty on the first page.
	PreviousToken string `protobuf:"bytes,2,opt,name=previous_token,json=previousToken,proto3" json:"previous_token,omitempty"`
	// Token for the next page. Empty on the last page.
	NextToken     string `protobuf:"bytes,3,opt,name=next_token,json=nextToken,proto3" json:"next_token,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ListPublishedCreatorsResponse) Reset() {
	*x = ListPublishedCreatorsResponse{}
	mi := &file_publira_v1_catalog_proto_msgTypes[14]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ListPublishedCreatorsResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ListPublishedCreatorsResponse) ProtoMessage() {}

func (x *ListPublishedCreatorsResponse) ProtoReflect() protoreflect.Message {
	mi := &file_publira_v1_catalog_proto_msgTypes[14]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ListPublishedCreatorsResponse.ProtoReflect.Descriptor instead.
func (*ListPublishedCreatorsResponse) Descriptor() ([]byte, []int) {
	return file_publira_v1_catalog_proto_rawDescGZIP(), []int{14}
}

func (x *ListPublishedCreatorsResponse) GetCreators() []*PublishedCreator {
	if x != nil {
		return x.Creators
	}
	return nil
}

func (x *ListPublishedCreatorsResponse) GetPreviousToken() string {
	if x != nil {
		return x.PreviousToken
	}
	return ""
}

func (x *ListPublishedCreatorsResponse) GetNextToken() string {
	if x != nil {
		return x.NextToken
	}
	return ""
}

// Cursor pagination for the creator's published series. Field shape and token
// rules: proto/README.md. Sorted by title, then id. There is no order field:
// title order is the only series list the creator page needs, and a later
// selectable order would have to carry its name in the token.
type GetPublishedCreatorDetailRequest struct {
	state    protoimpl.MessageState `protogen:"open.v1"`
	Tenant   *v1.TenantContext      `protobuf:"bytes,1,opt,name=tenant,proto3" json:"tenant,omitempty"`
	PublicId string                 `protobuf:"bytes,2,opt,name=public_id,json=publicId,proto3" json:"public_id,omitempty"`
	// Max series in one page. <= 0 or > 100 falls back to 20.
	Limit int32 `protobuf:"varint,3,opt,name=limit,proto3" json:"limit,omitempty"`
	// Opaque token from a previous response. Empty for the first page.
	Token string `protobuf:"bytes,4,opt,name=token,proto3" json:"token,omitempty"`
	// The client making the read. Unspecified is answered as the web storefront.
	Surface       v1.ClientSurface `protobuf:"varint,5,opt,name=surface,proto3,enum=publira.types.v1.ClientSurface" json:"surface,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetPublishedCreatorDetailRequest) Reset() {
	*x = GetPublishedCreatorDetailRequest{}
	mi := &file_publira_v1_catalog_proto_msgTypes[15]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetPublishedCreatorDetailRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetPublishedCreatorDetailRequest) ProtoMessage() {}

func (x *GetPublishedCreatorDetailRequest) ProtoReflect() protoreflect.Message {
	mi := &file_publira_v1_catalog_proto_msgTypes[15]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetPublishedCreatorDetailRequest.ProtoReflect.Descriptor instead.
func (*GetPublishedCreatorDetailRequest) Descriptor() ([]byte, []int) {
	return file_publira_v1_catalog_proto_rawDescGZIP(), []int{15}
}

func (x *GetPublishedCreatorDetailRequest) GetTenant() *v1.TenantContext {
	if x != nil {
		return x.Tenant
	}
	return nil
}

func (x *GetPublishedCreatorDetailRequest) GetPublicId() string {
	if x != nil {
		return x.PublicId
	}
	return ""
}

func (x *GetPublishedCreatorDetailRequest) GetLimit() int32 {
	if x != nil {
		return x.Limit
	}
	return 0
}

func (x *GetPublishedCreatorDetailRequest) GetToken() string {
	if x != nil {
		return x.Token
	}
	return ""
}

func (x *GetPublishedCreatorDetailRequest) GetSurface() v1.ClientSurface {
	if x != nil {
		return x.Surface
	}
	return v1.ClientSurface(0)
}

type GetPublishedCreatorDetailResponse struct {
	state   protoimpl.MessageState `protogen:"open.v1"`
	Creator *PublishedCreator      `protobuf:"bytes,1,opt,name=creator,proto3" json:"creator,omitempty"`
	// One page of published series credited to this creator, title ascending.
	Series []*v1.Series `protobuf:"bytes,2,rep,name=series,proto3" json:"series,omitempty"`
	// Token for the previous series page. Empty on the first page.
	PreviousToken string `protobuf:"bytes,3,opt,name=previous_token,json=previousToken,proto3" json:"previous_token,omitempty"`
	// Token for the next series page. Empty on the last page.
	NextToken     string `protobuf:"bytes,4,opt,name=next_token,json=nextToken,proto3" json:"next_token,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetPublishedCreatorDetailResponse) Reset() {
	*x = GetPublishedCreatorDetailResponse{}
	mi := &file_publira_v1_catalog_proto_msgTypes[16]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetPublishedCreatorDetailResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetPublishedCreatorDetailResponse) ProtoMessage() {}

func (x *GetPublishedCreatorDetailResponse) ProtoReflect() protoreflect.Message {
	mi := &file_publira_v1_catalog_proto_msgTypes[16]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetPublishedCreatorDetailResponse.ProtoReflect.Descriptor instead.
func (*GetPublishedCreatorDetailResponse) Descriptor() ([]byte, []int) {
	return file_publira_v1_catalog_proto_rawDescGZIP(), []int{16}
}

func (x *GetPublishedCreatorDetailResponse) GetCreator() *PublishedCreator {
	if x != nil {
		return x.Creator
	}
	return nil
}

func (x *GetPublishedCreatorDetailResponse) GetSeries() []*v1.Series {
	if x != nil {
		return x.Series
	}
	return nil
}

func (x *GetPublishedCreatorDetailResponse) GetPreviousToken() string {
	if x != nil {
		return x.PreviousToken
	}
	return ""
}

func (x *GetPublishedCreatorDetailResponse) GetNextToken() string {
	if x != nil {
		return x.NextToken
	}
	return ""
}

// A label that belongs to the requested tenant, plus how many of its series
// are currently published. Labels have no unpublished state of their own; a
// label with zero published series is still returned so a shareable detail
// URL stays valid after its last series is taken down.
type PublishedLabel struct {
	state                  protoimpl.MessageState      `protogen:"open.v1"`
	PublicId               string                      `protobuf:"bytes,1,opt,name=public_id,json=publicId,proto3" json:"public_id,omitempty"`
	Name                   string                      `protobuf:"bytes,2,opt,name=name,proto3" json:"name,omitempty"`
	EyeCatchImageUpdatedAt string                      `protobuf:"bytes,3,opt,name=eye_catch_image_updated_at,json=eyeCatchImageUpdatedAt,proto3" json:"eye_catch_image_updated_at,omitempty"`
	EyeCatchImageVariants  []*v1.SeriesEyeCatchVariant `protobuf:"bytes,4,rep,name=eye_catch_image_variants,json=eyeCatchImageVariants,proto3" json:"eye_catch_image_variants,omitempty"`
	PublishedSeriesCount   int32                       `protobuf:"varint,5,opt,name=published_series_count,json=publishedSeriesCount,proto3" json:"published_series_count,omitempty"`
	unknownFields          protoimpl.UnknownFields
	sizeCache              protoimpl.SizeCache
}

func (x *PublishedLabel) Reset() {
	*x = PublishedLabel{}
	mi := &file_publira_v1_catalog_proto_msgTypes[17]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *PublishedLabel) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*PublishedLabel) ProtoMessage() {}

func (x *PublishedLabel) ProtoReflect() protoreflect.Message {
	mi := &file_publira_v1_catalog_proto_msgTypes[17]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use PublishedLabel.ProtoReflect.Descriptor instead.
func (*PublishedLabel) Descriptor() ([]byte, []int) {
	return file_publira_v1_catalog_proto_rawDescGZIP(), []int{17}
}

func (x *PublishedLabel) GetPublicId() string {
	if x != nil {
		return x.PublicId
	}
	return ""
}

func (x *PublishedLabel) GetName() string {
	if x != nil {
		return x.Name
	}
	return ""
}

func (x *PublishedLabel) GetEyeCatchImageUpdatedAt() string {
	if x != nil {
		return x.EyeCatchImageUpdatedAt
	}
	return ""
}

func (x *PublishedLabel) GetEyeCatchImageVariants() []*v1.SeriesEyeCatchVariant {
	if x != nil {
		return x.EyeCatchImageVariants
	}
	return nil
}

func (x *PublishedLabel) GetPublishedSeriesCount() int32 {
	if x != nil {
		return x.PublishedSeriesCount
	}
	return 0
}

// Cursor pagination for the label's published series. Field shape and token
// rules: proto/README.md. Sorted by title, then id. There is no order field:
// title order is the only series list the label page needs, and a later
// selectable order would have to carry its name in the token.
type GetPublishedLabelDetailRequest struct {
	state    protoimpl.MessageState `protogen:"open.v1"`
	Tenant   *v1.TenantContext      `protobuf:"bytes,1,opt,name=tenant,proto3" json:"tenant,omitempty"`
	PublicId string                 `protobuf:"bytes,2,opt,name=public_id,json=publicId,proto3" json:"public_id,omitempty"`
	// Max series in one page. <= 0 or > 100 falls back to 20.
	Limit int32 `protobuf:"varint,3,opt,name=limit,proto3" json:"limit,omitempty"`
	// Opaque token from a previous response. Empty for the first page.
	Token string `protobuf:"bytes,4,opt,name=token,proto3" json:"token,omitempty"`
	// The client making the read. Unspecified is answered as the web storefront.
	Surface       v1.ClientSurface `protobuf:"varint,5,opt,name=surface,proto3,enum=publira.types.v1.ClientSurface" json:"surface,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetPublishedLabelDetailRequest) Reset() {
	*x = GetPublishedLabelDetailRequest{}
	mi := &file_publira_v1_catalog_proto_msgTypes[18]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetPublishedLabelDetailRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetPublishedLabelDetailRequest) ProtoMessage() {}

func (x *GetPublishedLabelDetailRequest) ProtoReflect() protoreflect.Message {
	mi := &file_publira_v1_catalog_proto_msgTypes[18]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetPublishedLabelDetailRequest.ProtoReflect.Descriptor instead.
func (*GetPublishedLabelDetailRequest) Descriptor() ([]byte, []int) {
	return file_publira_v1_catalog_proto_rawDescGZIP(), []int{18}
}

func (x *GetPublishedLabelDetailRequest) GetTenant() *v1.TenantContext {
	if x != nil {
		return x.Tenant
	}
	return nil
}

func (x *GetPublishedLabelDetailRequest) GetPublicId() string {
	if x != nil {
		return x.PublicId
	}
	return ""
}

func (x *GetPublishedLabelDetailRequest) GetLimit() int32 {
	if x != nil {
		return x.Limit
	}
	return 0
}

func (x *GetPublishedLabelDetailRequest) GetToken() string {
	if x != nil {
		return x.Token
	}
	return ""
}

func (x *GetPublishedLabelDetailRequest) GetSurface() v1.ClientSurface {
	if x != nil {
		return x.Surface
	}
	return v1.ClientSurface(0)
}

type GetPublishedLabelDetailResponse struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	Label *PublishedLabel        `protobuf:"bytes,1,opt,name=label,proto3" json:"label,omitempty"`
	// One page of published series on this label, title ascending.
	Series []*v1.Series `protobuf:"bytes,2,rep,name=series,proto3" json:"series,omitempty"`
	// Token for the previous series page. Empty on the first page.
	PreviousToken string `protobuf:"bytes,3,opt,name=previous_token,json=previousToken,proto3" json:"previous_token,omitempty"`
	// Token for the next series page. Empty on the last page.
	NextToken     string `protobuf:"bytes,4,opt,name=next_token,json=nextToken,proto3" json:"next_token,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetPublishedLabelDetailResponse) Reset() {
	*x = GetPublishedLabelDetailResponse{}
	mi := &file_publira_v1_catalog_proto_msgTypes[19]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetPublishedLabelDetailResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetPublishedLabelDetailResponse) ProtoMessage() {}

func (x *GetPublishedLabelDetailResponse) ProtoReflect() protoreflect.Message {
	mi := &file_publira_v1_catalog_proto_msgTypes[19]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetPublishedLabelDetailResponse.ProtoReflect.Descriptor instead.
func (*GetPublishedLabelDetailResponse) Descriptor() ([]byte, []int) {
	return file_publira_v1_catalog_proto_rawDescGZIP(), []int{19}
}

func (x *GetPublishedLabelDetailResponse) GetLabel() *PublishedLabel {
	if x != nil {
		return x.Label
	}
	return nil
}

func (x *GetPublishedLabelDetailResponse) GetSeries() []*v1.Series {
	if x != nil {
		return x.Series
	}
	return nil
}

func (x *GetPublishedLabelDetailResponse) GetPreviousToken() string {
	if x != nil {
		return x.PreviousToken
	}
	return ""
}

func (x *GetPublishedLabelDetailResponse) GetNextToken() string {
	if x != nil {
		return x.NextToken
	}
	return ""
}

// A genre the tenant curates, next to how many of its series are published
// right now. The count is what a browse page puts beside the name; it is
// counted per read rather than stored, so a series taken down stops counting
// the moment it does.
type PublishedGenre struct {
	state                protoimpl.MessageState `protogen:"open.v1"`
	PublicId             string                 `protobuf:"bytes,1,opt,name=public_id,json=publicId,proto3" json:"public_id,omitempty"`
	Name                 string                 `protobuf:"bytes,2,opt,name=name,proto3" json:"name,omitempty"`
	Slug                 string                 `protobuf:"bytes,3,opt,name=slug,proto3" json:"slug,omitempty"`
	PublishedSeriesCount int32                  `protobuf:"varint,4,opt,name=published_series_count,json=publishedSeriesCount,proto3" json:"published_series_count,omitempty"`
	unknownFields        protoimpl.UnknownFields
	sizeCache            protoimpl.SizeCache
}

func (x *PublishedGenre) Reset() {
	*x = PublishedGenre{}
	mi := &file_publira_v1_catalog_proto_msgTypes[20]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *PublishedGenre) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*PublishedGenre) ProtoMessage() {}

func (x *PublishedGenre) ProtoReflect() protoreflect.Message {
	mi := &file_publira_v1_catalog_proto_msgTypes[20]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use PublishedGenre.ProtoReflect.Descriptor instead.
func (*PublishedGenre) Descriptor() ([]byte, []int) {
	return file_publira_v1_catalog_proto_rawDescGZIP(), []int{20}
}

func (x *PublishedGenre) GetPublicId() string {
	if x != nil {
		return x.PublicId
	}
	return ""
}

func (x *PublishedGenre) GetName() string {
	if x != nil {
		return x.Name
	}
	return ""
}

func (x *PublishedGenre) GetSlug() string {
	if x != nil {
		return x.Slug
	}
	return ""
}

func (x *PublishedGenre) GetPublishedSeriesCount() int32 {
	if x != nil {
		return x.PublishedSeriesCount
	}
	return 0
}

// Cursor pagination. Field shape and token rules: proto/README.md.
// Sorted by the tenant's own genre order, then id. There is no order field:
// the console decides where a genre sits, and a browse page that reordered
// them would be showing a list the tenant did not arrange.
// A genre no published series carries is still listed, for the reason a label
// with no published series is: the URL of its page has to keep working after
// its last series is taken down.
type ListPublishedGenresRequest struct {
	state  protoimpl.MessageState `protogen:"open.v1"`
	Tenant *v1.TenantContext      `protobuf:"bytes,1,opt,name=tenant,proto3" json:"tenant,omitempty"`
	// Max items in one page. <= 0 or > 100 falls back to 20.
	Limit int32 `protobuf:"varint,2,opt,name=limit,proto3" json:"limit,omitempty"`
	// Opaque token from a previous response. Empty for the first page.
	Token string `protobuf:"bytes,3,opt,name=token,proto3" json:"token,omitempty"`
	// The client making the read. Unspecified is answered as the web storefront.
	Surface       v1.ClientSurface `protobuf:"varint,4,opt,name=surface,proto3,enum=publira.types.v1.ClientSurface" json:"surface,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ListPublishedGenresRequest) Reset() {
	*x = ListPublishedGenresRequest{}
	mi := &file_publira_v1_catalog_proto_msgTypes[21]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ListPublishedGenresRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ListPublishedGenresRequest) ProtoMessage() {}

func (x *ListPublishedGenresRequest) ProtoReflect() protoreflect.Message {
	mi := &file_publira_v1_catalog_proto_msgTypes[21]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ListPublishedGenresRequest.ProtoReflect.Descriptor instead.
func (*ListPublishedGenresRequest) Descriptor() ([]byte, []int) {
	return file_publira_v1_catalog_proto_rawDescGZIP(), []int{21}
}

func (x *ListPublishedGenresRequest) GetTenant() *v1.TenantContext {
	if x != nil {
		return x.Tenant
	}
	return nil
}

func (x *ListPublishedGenresRequest) GetLimit() int32 {
	if x != nil {
		return x.Limit
	}
	return 0
}

func (x *ListPublishedGenresRequest) GetToken() string {
	if x != nil {
		return x.Token
	}
	return ""
}

func (x *ListPublishedGenresRequest) GetSurface() v1.ClientSurface {
	if x != nil {
		return x.Surface
	}
	return v1.ClientSurface(0)
}

type ListPublishedGenresResponse struct {
	state  protoimpl.MessageState `protogen:"open.v1"`
	Genres []*PublishedGenre      `protobuf:"bytes,1,rep,name=genres,proto3" json:"genres,omitempty"`
	// Token for the previous page. Empty on the first page.
	PreviousToken string `protobuf:"bytes,2,opt,name=previous_token,json=previousToken,proto3" json:"previous_token,omitempty"`
	// Token for the next page. Empty on the last page.
	NextToken     string `protobuf:"bytes,3,opt,name=next_token,json=nextToken,proto3" json:"next_token,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ListPublishedGenresResponse) Reset() {
	*x = ListPublishedGenresResponse{}
	mi := &file_publira_v1_catalog_proto_msgTypes[22]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ListPublishedGenresResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ListPublishedGenresResponse) ProtoMessage() {}

func (x *ListPublishedGenresResponse) ProtoReflect() protoreflect.Message {
	mi := &file_publira_v1_catalog_proto_msgTypes[22]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ListPublishedGenresResponse.ProtoReflect.Descriptor instead.
func (*ListPublishedGenresResponse) Descriptor() ([]byte, []int) {
	return file_publira_v1_catalog_proto_rawDescGZIP(), []int{22}
}

func (x *ListPublishedGenresResponse) GetGenres() []*PublishedGenre {
	if x != nil {
		return x.Genres
	}
	return nil
}

func (x *ListPublishedGenresResponse) GetPreviousToken() string {
	if x != nil {
		return x.PreviousToken
	}
	return ""
}

func (x *ListPublishedGenresResponse) GetNextToken() string {
	if x != nil {
		return x.NextToken
	}
	return ""
}

// A tag at least one published series carries, next to how many carry it. A
// tag is addressed by slug, which is why it has no public ID of its own.
type PublishedTag struct {
	state                protoimpl.MessageState `protogen:"open.v1"`
	Name                 string                 `protobuf:"bytes,1,opt,name=name,proto3" json:"name,omitempty"`
	Slug                 string                 `protobuf:"bytes,2,opt,name=slug,proto3" json:"slug,omitempty"`
	PublishedSeriesCount int32                  `protobuf:"varint,3,opt,name=published_series_count,json=publishedSeriesCount,proto3" json:"published_series_count,omitempty"`
	unknownFields        protoimpl.UnknownFields
	sizeCache            protoimpl.SizeCache
}

func (x *PublishedTag) Reset() {
	*x = PublishedTag{}
	mi := &file_publira_v1_catalog_proto_msgTypes[23]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *PublishedTag) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*PublishedTag) ProtoMessage() {}

func (x *PublishedTag) ProtoReflect() protoreflect.Message {
	mi := &file_publira_v1_catalog_proto_msgTypes[23]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use PublishedTag.ProtoReflect.Descriptor instead.
func (*PublishedTag) Descriptor() ([]byte, []int) {
	return file_publira_v1_catalog_proto_rawDescGZIP(), []int{23}
}

func (x *PublishedTag) GetName() string {
	if x != nil {
		return x.Name
	}
	return ""
}

func (x *PublishedTag) GetSlug() string {
	if x != nil {
		return x.Slug
	}
	return ""
}

func (x *PublishedTag) GetPublishedSeriesCount() int32 {
	if x != nil {
		return x.PublishedSeriesCount
	}
	return 0
}

// Cursor pagination. Field shape and token rules: proto/README.md.
// Sorted by how many published series carry the tag, most first, then by slug,
// which is unique within the tenant and therefore breaks every tie. There is
// no order field: a tag cloud has one order, the one that puts the tags a
// storefront actually uses in front.
// Unlike a genre, a tag nothing published carries is not listed: a tag exists
// because a series carries it, so one no published series carries has no page
// to keep working.
type ListPublishedTagsRequest struct {
	state  protoimpl.MessageState `protogen:"open.v1"`
	Tenant *v1.TenantContext      `protobuf:"bytes,1,opt,name=tenant,proto3" json:"tenant,omitempty"`
	// Max items in one page. <= 0 or > 100 falls back to 20.
	Limit int32 `protobuf:"varint,2,opt,name=limit,proto3" json:"limit,omitempty"`
	// Opaque token from a previous response. Empty for the first page.
	Token string `protobuf:"bytes,3,opt,name=token,proto3" json:"token,omitempty"`
	// The client making the read. Unspecified is answered as the web storefront.
	Surface       v1.ClientSurface `protobuf:"varint,4,opt,name=surface,proto3,enum=publira.types.v1.ClientSurface" json:"surface,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ListPublishedTagsRequest) Reset() {
	*x = ListPublishedTagsRequest{}
	mi := &file_publira_v1_catalog_proto_msgTypes[24]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ListPublishedTagsRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ListPublishedTagsRequest) ProtoMessage() {}

func (x *ListPublishedTagsRequest) ProtoReflect() protoreflect.Message {
	mi := &file_publira_v1_catalog_proto_msgTypes[24]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ListPublishedTagsRequest.ProtoReflect.Descriptor instead.
func (*ListPublishedTagsRequest) Descriptor() ([]byte, []int) {
	return file_publira_v1_catalog_proto_rawDescGZIP(), []int{24}
}

func (x *ListPublishedTagsRequest) GetTenant() *v1.TenantContext {
	if x != nil {
		return x.Tenant
	}
	return nil
}

func (x *ListPublishedTagsRequest) GetLimit() int32 {
	if x != nil {
		return x.Limit
	}
	return 0
}

func (x *ListPublishedTagsRequest) GetToken() string {
	if x != nil {
		return x.Token
	}
	return ""
}

func (x *ListPublishedTagsRequest) GetSurface() v1.ClientSurface {
	if x != nil {
		return x.Surface
	}
	return v1.ClientSurface(0)
}

type ListPublishedTagsResponse struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	Tags  []*PublishedTag        `protobuf:"bytes,1,rep,name=tags,proto3" json:"tags,omitempty"`
	// Token for the previous page. Empty on the first page.
	PreviousToken string `protobuf:"bytes,2,opt,name=previous_token,json=previousToken,proto3" json:"previous_token,omitempty"`
	// Token for the next page. Empty on the last page.
	NextToken     string `protobuf:"bytes,3,opt,name=next_token,json=nextToken,proto3" json:"next_token,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ListPublishedTagsResponse) Reset() {
	*x = ListPublishedTagsResponse{}
	mi := &file_publira_v1_catalog_proto_msgTypes[25]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ListPublishedTagsResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ListPublishedTagsResponse) ProtoMessage() {}

func (x *ListPublishedTagsResponse) ProtoReflect() protoreflect.Message {
	mi := &file_publira_v1_catalog_proto_msgTypes[25]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ListPublishedTagsResponse.ProtoReflect.Descriptor instead.
func (*ListPublishedTagsResponse) Descriptor() ([]byte, []int) {
	return file_publira_v1_catalog_proto_rawDescGZIP(), []int{25}
}

func (x *ListPublishedTagsResponse) GetTags() []*PublishedTag {
	if x != nil {
		return x.Tags
	}
	return nil
}

func (x *ListPublishedTagsResponse) GetPreviousToken() string {
	if x != nil {
		return x.PreviousToken
	}
	return ""
}

func (x *ListPublishedTagsResponse) GetNextToken() string {
	if x != nil {
		return x.NextToken
	}
	return ""
}

// Cursor pagination. Field shape and token rules: proto/README.md.
// Matches published series whose title or synopsis contains `query`
// (case-insensitive substring). Sorted by title, then id. There is no order
// field: title order is the only search result list the storefront needs.
// The token carries the Unicode-lowercased query it was built for; that same
// string is the ILIKE pattern. Sending the token with a query that lowers to
// a different string is invalid_argument. ASCII case changes of the same
// query are the same search. Changing to a different query restarts at page 1.
type SearchPublishedSeriesRequest struct {
	state  protoimpl.MessageState `protogen:"open.v1"`
	Tenant *v1.TenantContext      `protobuf:"bytes,1,opt,name=tenant,proto3" json:"tenant,omitempty"`
	// Trimmed keyword. Empty after trim is invalid_argument. Max 100 runes.
	// Not restricted to ASCII; Japanese titles are the expected input.
	Query string `protobuf:"bytes,2,opt,name=query,proto3" json:"query,omitempty"`
	// Max items in one page. <= 0 or > 100 falls back to 20.
	Limit int32 `protobuf:"varint,3,opt,name=limit,proto3" json:"limit,omitempty"`
	// Opaque token from a previous response. Empty for the first page.
	Token string `protobuf:"bytes,4,opt,name=token,proto3" json:"token,omitempty"`
	// The client making the read. Unspecified is answered as the web storefront.
	Surface       v1.ClientSurface `protobuf:"varint,5,opt,name=surface,proto3,enum=publira.types.v1.ClientSurface" json:"surface,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *SearchPublishedSeriesRequest) Reset() {
	*x = SearchPublishedSeriesRequest{}
	mi := &file_publira_v1_catalog_proto_msgTypes[26]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *SearchPublishedSeriesRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*SearchPublishedSeriesRequest) ProtoMessage() {}

func (x *SearchPublishedSeriesRequest) ProtoReflect() protoreflect.Message {
	mi := &file_publira_v1_catalog_proto_msgTypes[26]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use SearchPublishedSeriesRequest.ProtoReflect.Descriptor instead.
func (*SearchPublishedSeriesRequest) Descriptor() ([]byte, []int) {
	return file_publira_v1_catalog_proto_rawDescGZIP(), []int{26}
}

func (x *SearchPublishedSeriesRequest) GetTenant() *v1.TenantContext {
	if x != nil {
		return x.Tenant
	}
	return nil
}

func (x *SearchPublishedSeriesRequest) GetQuery() string {
	if x != nil {
		return x.Query
	}
	return ""
}

func (x *SearchPublishedSeriesRequest) GetLimit() int32 {
	if x != nil {
		return x.Limit
	}
	return 0
}

func (x *SearchPublishedSeriesRequest) GetToken() string {
	if x != nil {
		return x.Token
	}
	return ""
}

func (x *SearchPublishedSeriesRequest) GetSurface() v1.ClientSurface {
	if x != nil {
		return x.Surface
	}
	return v1.ClientSurface(0)
}

type SearchPublishedSeriesResponse struct {
	state  protoimpl.MessageState `protogen:"open.v1"`
	Series []*v1.Series           `protobuf:"bytes,1,rep,name=series,proto3" json:"series,omitempty"`
	// Token for the previous page. Empty on the first page.
	PreviousToken string `protobuf:"bytes,2,opt,name=previous_token,json=previousToken,proto3" json:"previous_token,omitempty"`
	// Token for the next page. Empty on the last page.
	NextToken     string `protobuf:"bytes,3,opt,name=next_token,json=nextToken,proto3" json:"next_token,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *SearchPublishedSeriesResponse) Reset() {
	*x = SearchPublishedSeriesResponse{}
	mi := &file_publira_v1_catalog_proto_msgTypes[27]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *SearchPublishedSeriesResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*SearchPublishedSeriesResponse) ProtoMessage() {}

func (x *SearchPublishedSeriesResponse) ProtoReflect() protoreflect.Message {
	mi := &file_publira_v1_catalog_proto_msgTypes[27]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use SearchPublishedSeriesResponse.ProtoReflect.Descriptor instead.
func (*SearchPublishedSeriesResponse) Descriptor() ([]byte, []int) {
	return file_publira_v1_catalog_proto_rawDescGZIP(), []int{27}
}

func (x *SearchPublishedSeriesResponse) GetSeries() []*v1.Series {
	if x != nil {
		return x.Series
	}
	return nil
}

func (x *SearchPublishedSeriesResponse) GetPreviousToken() string {
	if x != nil {
		return x.PreviousToken
	}
	return ""
}

func (x *SearchPublishedSeriesResponse) GetNextToken() string {
	if x != nil {
		return x.NextToken
	}
	return ""
}

// Cursor pagination. Field shape and token rules: proto/README.md.
// Matches creators whose name contains `query` (case-insensitive substring)
// and who hold at least one currently published series, the same population
// ListPublishedCreators shows. Sorted by name, then id. There is no order
// field: name order is the only creator list the storefront needs.
// Only the name is matched. A biography mentioning another creator answers a
// name search with someone the reader did not ask for.
// The token carries the Unicode-lowercased query it was built for, exactly as
// SearchPublishedSeries does; sending it with a query that lowers to a
// different string is invalid_argument.
type SearchPublishedCreatorsRequest struct {
	state  protoimpl.MessageState `protogen:"open.v1"`
	Tenant *v1.TenantContext      `protobuf:"bytes,1,opt,name=tenant,proto3" json:"tenant,omitempty"`
	// Trimmed keyword. Empty after trim is invalid_argument. Max 100 runes.
	// Not restricted to ASCII; Japanese names are the expected input.
	Query string `protobuf:"bytes,2,opt,name=query,proto3" json:"query,omitempty"`
	// Max items in one page. <= 0 or > 100 falls back to 20.
	Limit int32 `protobuf:"varint,3,opt,name=limit,proto3" json:"limit,omitempty"`
	// Opaque token from a previous response. Empty for the first page.
	Token string `protobuf:"bytes,4,opt,name=token,proto3" json:"token,omitempty"`
	// The client making the read. Unspecified is answered as the web storefront.
	Surface       v1.ClientSurface `protobuf:"varint,5,opt,name=surface,proto3,enum=publira.types.v1.ClientSurface" json:"surface,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *SearchPublishedCreatorsRequest) Reset() {
	*x = SearchPublishedCreatorsRequest{}
	mi := &file_publira_v1_catalog_proto_msgTypes[28]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *SearchPublishedCreatorsRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*SearchPublishedCreatorsRequest) ProtoMessage() {}

func (x *SearchPublishedCreatorsRequest) ProtoReflect() protoreflect.Message {
	mi := &file_publira_v1_catalog_proto_msgTypes[28]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use SearchPublishedCreatorsRequest.ProtoReflect.Descriptor instead.
func (*SearchPublishedCreatorsRequest) Descriptor() ([]byte, []int) {
	return file_publira_v1_catalog_proto_rawDescGZIP(), []int{28}
}

func (x *SearchPublishedCreatorsRequest) GetTenant() *v1.TenantContext {
	if x != nil {
		return x.Tenant
	}
	return nil
}

func (x *SearchPublishedCreatorsRequest) GetQuery() string {
	if x != nil {
		return x.Query
	}
	return ""
}

func (x *SearchPublishedCreatorsRequest) GetLimit() int32 {
	if x != nil {
		return x.Limit
	}
	return 0
}

func (x *SearchPublishedCreatorsRequest) GetToken() string {
	if x != nil {
		return x.Token
	}
	return ""
}

func (x *SearchPublishedCreatorsRequest) GetSurface() v1.ClientSurface {
	if x != nil {
		return x.Surface
	}
	return v1.ClientSurface(0)
}

type SearchPublishedCreatorsResponse struct {
	state    protoimpl.MessageState `protogen:"open.v1"`
	Creators []*PublishedCreator    `protobuf:"bytes,1,rep,name=creators,proto3" json:"creators,omitempty"`
	// Token for the previous page. Empty on the first page.
	PreviousToken string `protobuf:"bytes,2,opt,name=previous_token,json=previousToken,proto3" json:"previous_token,omitempty"`
	// Token for the next page. Empty on the last page.
	NextToken     string `protobuf:"bytes,3,opt,name=next_token,json=nextToken,proto3" json:"next_token,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *SearchPublishedCreatorsResponse) Reset() {
	*x = SearchPublishedCreatorsResponse{}
	mi := &file_publira_v1_catalog_proto_msgTypes[29]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *SearchPublishedCreatorsResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*SearchPublishedCreatorsResponse) ProtoMessage() {}

func (x *SearchPublishedCreatorsResponse) ProtoReflect() protoreflect.Message {
	mi := &file_publira_v1_catalog_proto_msgTypes[29]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use SearchPublishedCreatorsResponse.ProtoReflect.Descriptor instead.
func (*SearchPublishedCreatorsResponse) Descriptor() ([]byte, []int) {
	return file_publira_v1_catalog_proto_rawDescGZIP(), []int{29}
}

func (x *SearchPublishedCreatorsResponse) GetCreators() []*PublishedCreator {
	if x != nil {
		return x.Creators
	}
	return nil
}

func (x *SearchPublishedCreatorsResponse) GetPreviousToken() string {
	if x != nil {
		return x.PreviousToken
	}
	return ""
}

func (x *SearchPublishedCreatorsResponse) GetNextToken() string {
	if x != nil {
		return x.NextToken
	}
	return ""
}

// Cursor pagination. Field shape and token rules: proto/README.md.
// Matches labels whose name contains `query` (case-insensitive substring) and
// that hold at least one currently published series. Sorted by name, then id.
// There is no order field: name order is the only label search the storefront
// needs.
// The published-series requirement is what separates this from
// GetPublishedLabelDetail, which keeps answering for a label whose last series
// was taken down so a shared URL stays valid. Nothing links to a search hit
// that has nothing behind it.
// The token carries the Unicode-lowercased query it was built for, exactly as
// SearchPublishedSeries does; sending it with a query that lowers to a
// different string is invalid_argument.
type SearchPublishedLabelsRequest struct {
	state  protoimpl.MessageState `protogen:"open.v1"`
	Tenant *v1.TenantContext      `protobuf:"bytes,1,opt,name=tenant,proto3" json:"tenant,omitempty"`
	// Trimmed keyword. Empty after trim is invalid_argument. Max 100 runes.
	// Not restricted to ASCII; Japanese names are the expected input.
	Query string `protobuf:"bytes,2,opt,name=query,proto3" json:"query,omitempty"`
	// Max items in one page. <= 0 or > 100 falls back to 20.
	Limit int32 `protobuf:"varint,3,opt,name=limit,proto3" json:"limit,omitempty"`
	// Opaque token from a previous response. Empty for the first page.
	Token string `protobuf:"bytes,4,opt,name=token,proto3" json:"token,omitempty"`
	// The client making the read. Unspecified is answered as the web storefront.
	Surface       v1.ClientSurface `protobuf:"varint,5,opt,name=surface,proto3,enum=publira.types.v1.ClientSurface" json:"surface,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *SearchPublishedLabelsRequest) Reset() {
	*x = SearchPublishedLabelsRequest{}
	mi := &file_publira_v1_catalog_proto_msgTypes[30]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *SearchPublishedLabelsRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*SearchPublishedLabelsRequest) ProtoMessage() {}

func (x *SearchPublishedLabelsRequest) ProtoReflect() protoreflect.Message {
	mi := &file_publira_v1_catalog_proto_msgTypes[30]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use SearchPublishedLabelsRequest.ProtoReflect.Descriptor instead.
func (*SearchPublishedLabelsRequest) Descriptor() ([]byte, []int) {
	return file_publira_v1_catalog_proto_rawDescGZIP(), []int{30}
}

func (x *SearchPublishedLabelsRequest) GetTenant() *v1.TenantContext {
	if x != nil {
		return x.Tenant
	}
	return nil
}

func (x *SearchPublishedLabelsRequest) GetQuery() string {
	if x != nil {
		return x.Query
	}
	return ""
}

func (x *SearchPublishedLabelsRequest) GetLimit() int32 {
	if x != nil {
		return x.Limit
	}
	return 0
}

func (x *SearchPublishedLabelsRequest) GetToken() string {
	if x != nil {
		return x.Token
	}
	return ""
}

func (x *SearchPublishedLabelsRequest) GetSurface() v1.ClientSurface {
	if x != nil {
		return x.Surface
	}
	return v1.ClientSurface(0)
}

type SearchPublishedLabelsResponse struct {
	state  protoimpl.MessageState `protogen:"open.v1"`
	Labels []*v1.Label            `protobuf:"bytes,1,rep,name=labels,proto3" json:"labels,omitempty"`
	// Token for the previous page. Empty on the first page.
	PreviousToken string `protobuf:"bytes,2,opt,name=previous_token,json=previousToken,proto3" json:"previous_token,omitempty"`
	// Token for the next page. Empty on the last page.
	NextToken     string `protobuf:"bytes,3,opt,name=next_token,json=nextToken,proto3" json:"next_token,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *SearchPublishedLabelsResponse) Reset() {
	*x = SearchPublishedLabelsResponse{}
	mi := &file_publira_v1_catalog_proto_msgTypes[31]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *SearchPublishedLabelsResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*SearchPublishedLabelsResponse) ProtoMessage() {}

func (x *SearchPublishedLabelsResponse) ProtoReflect() protoreflect.Message {
	mi := &file_publira_v1_catalog_proto_msgTypes[31]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use SearchPublishedLabelsResponse.ProtoReflect.Descriptor instead.
func (*SearchPublishedLabelsResponse) Descriptor() ([]byte, []int) {
	return file_publira_v1_catalog_proto_rawDescGZIP(), []int{31}
}

func (x *SearchPublishedLabelsResponse) GetLabels() []*v1.Label {
	if x != nil {
		return x.Labels
	}
	return nil
}

func (x *SearchPublishedLabelsResponse) GetPreviousToken() string {
	if x != nil {
		return x.PreviousToken
	}
	return ""
}

func (x *SearchPublishedLabelsResponse) GetNextToken() string {
	if x != nil {
		return x.NextToken
	}
	return ""
}

// Cursor pagination. Field shape and token rules: proto/README.md.
// There is no order field: the recommendation order is the one this RPC exists
// to produce.
type ListRecommendedSeriesRequest struct {
	state  protoimpl.MessageState `protogen:"open.v1"`
	Tenant *v1.TenantContext      `protobuf:"bytes,1,opt,name=tenant,proto3" json:"tenant,omitempty"`
	// Max items in one page. <= 0 or > 100 falls back to 20.
	Limit int32 `protobuf:"varint,2,opt,name=limit,proto3" json:"limit,omitempty"`
	// Opaque token from a previous response. Empty for the first page.
	Token string `protobuf:"bytes,3,opt,name=token,proto3" json:"token,omitempty"`
	// The client making the read. Unspecified is answered as the web storefront.
	Surface       v1.ClientSurface `protobuf:"varint,4,opt,name=surface,proto3,enum=publira.types.v1.ClientSurface" json:"surface,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ListRecommendedSeriesRequest) Reset() {
	*x = ListRecommendedSeriesRequest{}
	mi := &file_publira_v1_catalog_proto_msgTypes[32]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ListRecommendedSeriesRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ListRecommendedSeriesRequest) ProtoMessage() {}

func (x *ListRecommendedSeriesRequest) ProtoReflect() protoreflect.Message {
	mi := &file_publira_v1_catalog_proto_msgTypes[32]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ListRecommendedSeriesRequest.ProtoReflect.Descriptor instead.
func (*ListRecommendedSeriesRequest) Descriptor() ([]byte, []int) {
	return file_publira_v1_catalog_proto_rawDescGZIP(), []int{32}
}

func (x *ListRecommendedSeriesRequest) GetTenant() *v1.TenantContext {
	if x != nil {
		return x.Tenant
	}
	return nil
}

func (x *ListRecommendedSeriesRequest) GetLimit() int32 {
	if x != nil {
		return x.Limit
	}
	return 0
}

func (x *ListRecommendedSeriesRequest) GetToken() string {
	if x != nil {
		return x.Token
	}
	return ""
}

func (x *ListRecommendedSeriesRequest) GetSurface() v1.ClientSurface {
	if x != nil {
		return x.Surface
	}
	return v1.ClientSurface(0)
}

type ListRecommendedSeriesResponse struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Ranked series first, in ranking order, then every other published series
	// newest first.
	Series []*v1.Series `protobuf:"bytes,1,rep,name=series,proto3" json:"series,omitempty"`
	// Token for the previous page. Empty on the first page.
	PreviousToken string `protobuf:"bytes,2,opt,name=previous_token,json=previousToken,proto3" json:"previous_token,omitempty"`
	// Token for the next page. Empty on the last page.
	NextToken     string               `protobuf:"bytes,3,opt,name=next_token,json=nextToken,proto3" json:"next_token,omitempty"`
	Source        RecommendationSource `protobuf:"varint,4,opt,name=source,proto3,enum=publira.v1.RecommendationSource" json:"source,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ListRecommendedSeriesResponse) Reset() {
	*x = ListRecommendedSeriesResponse{}
	mi := &file_publira_v1_catalog_proto_msgTypes[33]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ListRecommendedSeriesResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ListRecommendedSeriesResponse) ProtoMessage() {}

func (x *ListRecommendedSeriesResponse) ProtoReflect() protoreflect.Message {
	mi := &file_publira_v1_catalog_proto_msgTypes[33]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ListRecommendedSeriesResponse.ProtoReflect.Descriptor instead.
func (*ListRecommendedSeriesResponse) Descriptor() ([]byte, []int) {
	return file_publira_v1_catalog_proto_rawDescGZIP(), []int{33}
}

func (x *ListRecommendedSeriesResponse) GetSeries() []*v1.Series {
	if x != nil {
		return x.Series
	}
	return nil
}

func (x *ListRecommendedSeriesResponse) GetPreviousToken() string {
	if x != nil {
		return x.PreviousToken
	}
	return ""
}

func (x *ListRecommendedSeriesResponse) GetNextToken() string {
	if x != nil {
		return x.NextToken
	}
	return ""
}

func (x *ListRecommendedSeriesResponse) GetSource() RecommendationSource {
	if x != nil {
		return x.Source
	}
	return RecommendationSource_RECOMMENDATION_SOURCE_UNSPECIFIED
}

// One series at the position a ranking snapshot gave it.
type RankedSeries struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// The position in the snapshot, counting from 1. Positions are the
	// snapshot's own, so they stay put when a series between them is
	// unpublished: a page can run 1, 2, 4.
	Rank   int32      `protobuf:"varint,1,opt,name=rank,proto3" json:"rank,omitempty"`
	Series *v1.Series `protobuf:"bytes,2,opt,name=series,proto3" json:"series,omitempty"`
	// The position this series held in the snapshot before the current one, for
	// an up / down marker. Absent when there is no earlier snapshot, or when
	// that snapshot did not rank this series — a new entry has no movement to
	// draw, which is not the same as having moved from position 0.
	PreviousRank  *int32 `protobuf:"varint,3,opt,name=previous_rank,json=previousRank,proto3,oneof" json:"previous_rank,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *RankedSeries) Reset() {
	*x = RankedSeries{}
	mi := &file_publira_v1_catalog_proto_msgTypes[34]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *RankedSeries) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*RankedSeries) ProtoMessage() {}

func (x *RankedSeries) ProtoReflect() protoreflect.Message {
	mi := &file_publira_v1_catalog_proto_msgTypes[34]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use RankedSeries.ProtoReflect.Descriptor instead.
func (*RankedSeries) Descriptor() ([]byte, []int) {
	return file_publira_v1_catalog_proto_rawDescGZIP(), []int{34}
}

func (x *RankedSeries) GetRank() int32 {
	if x != nil {
		return x.Rank
	}
	return 0
}

func (x *RankedSeries) GetSeries() *v1.Series {
	if x != nil {
		return x.Series
	}
	return nil
}

func (x *RankedSeries) GetPreviousRank() int32 {
	if x != nil && x.PreviousRank != nil {
		return *x.PreviousRank
	}
	return 0
}

// Cursor pagination. Field shape and token rules: proto/README.md.
// There is no order field: a ranking has one order, the one it was computed
// in. The token carries the period it was built for, because the same
// position names a different series in the other period; sending it with a
// different period is invalid_argument.
// It also pins the snapshot the first page came from, so the rest of a
// traversal keeps the positions of one ranking even when the batch writes a
// new one in between. A token whose snapshot has since been dropped by the
// retention purge is invalid_argument: the page it names no longer exists, and
// the client starts again at the first page of the current ranking.
type ListRankedSeriesRequest struct {
	state  protoimpl.MessageState `protogen:"open.v1"`
	Tenant *v1.TenantContext      `protobuf:"bytes,1,opt,name=tenant,proto3" json:"tenant,omitempty"`
	// Which period to rank. Unspecified is the daily ranking.
	Period RankingPeriod `protobuf:"varint,2,opt,name=period,proto3,enum=publira.v1.RankingPeriod" json:"period,omitempty"`
	// Max items in one page. <= 0 or > 100 falls back to 20.
	Limit int32 `protobuf:"varint,3,opt,name=limit,proto3" json:"limit,omitempty"`
	// Opaque token from a previous response. Empty for the first page.
	Token string `protobuf:"bytes,4,opt,name=token,proto3" json:"token,omitempty"`
	// The client making the read. Unspecified is answered as the web storefront.
	Surface       v1.ClientSurface `protobuf:"varint,5,opt,name=surface,proto3,enum=publira.types.v1.ClientSurface" json:"surface,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ListRankedSeriesRequest) Reset() {
	*x = ListRankedSeriesRequest{}
	mi := &file_publira_v1_catalog_proto_msgTypes[35]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ListRankedSeriesRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ListRankedSeriesRequest) ProtoMessage() {}

func (x *ListRankedSeriesRequest) ProtoReflect() protoreflect.Message {
	mi := &file_publira_v1_catalog_proto_msgTypes[35]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ListRankedSeriesRequest.ProtoReflect.Descriptor instead.
func (*ListRankedSeriesRequest) Descriptor() ([]byte, []int) {
	return file_publira_v1_catalog_proto_rawDescGZIP(), []int{35}
}

func (x *ListRankedSeriesRequest) GetTenant() *v1.TenantContext {
	if x != nil {
		return x.Tenant
	}
	return nil
}

func (x *ListRankedSeriesRequest) GetPeriod() RankingPeriod {
	if x != nil {
		return x.Period
	}
	return RankingPeriod_RANKING_PERIOD_UNSPECIFIED
}

func (x *ListRankedSeriesRequest) GetLimit() int32 {
	if x != nil {
		return x.Limit
	}
	return 0
}

func (x *ListRankedSeriesRequest) GetToken() string {
	if x != nil {
		return x.Token
	}
	return ""
}

func (x *ListRankedSeriesRequest) GetSurface() v1.ClientSurface {
	if x != nil {
		return x.Surface
	}
	return v1.ClientSurface(0)
}

type ListRankedSeriesResponse struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// One page of the snapshot, in ranking order.
	RankedSeries []*RankedSeries `protobuf:"bytes,1,rep,name=ranked_series,json=rankedSeries,proto3" json:"ranked_series,omitempty"`
	// Token for the previous page. Empty on the first page.
	PreviousToken string `protobuf:"bytes,2,opt,name=previous_token,json=previousToken,proto3" json:"previous_token,omitempty"`
	// Token for the next page. Empty on the last page.
	NextToken string `protobuf:"bytes,3,opt,name=next_token,json=nextToken,proto3" json:"next_token,omitempty"`
	// When the batch computed the snapshot this page comes from (RFC 3339).
	// Every page of one traversal reports the same instant, because they are all
	// pages of the same snapshot. Empty when the tenant has no snapshot yet.
	ComputedAt string `protobuf:"bytes,4,opt,name=computed_at,json=computedAt,proto3" json:"computed_at,omitempty"`
	// The ranked period, as inclusive calendar dates (YYYY-MM-DD) in the
	// tenant's time zone, which is the zone the daily stats behind the ranking
	// are counted in. Empty when the tenant has no snapshot yet.
	PeriodStart   string `protobuf:"bytes,5,opt,name=period_start,json=periodStart,proto3" json:"period_start,omitempty"`
	PeriodEnd     string `protobuf:"bytes,6,opt,name=period_end,json=periodEnd,proto3" json:"period_end,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ListRankedSeriesResponse) Reset() {
	*x = ListRankedSeriesResponse{}
	mi := &file_publira_v1_catalog_proto_msgTypes[36]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ListRankedSeriesResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ListRankedSeriesResponse) ProtoMessage() {}

func (x *ListRankedSeriesResponse) ProtoReflect() protoreflect.Message {
	mi := &file_publira_v1_catalog_proto_msgTypes[36]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ListRankedSeriesResponse.ProtoReflect.Descriptor instead.
func (*ListRankedSeriesResponse) Descriptor() ([]byte, []int) {
	return file_publira_v1_catalog_proto_rawDescGZIP(), []int{36}
}

func (x *ListRankedSeriesResponse) GetRankedSeries() []*RankedSeries {
	if x != nil {
		return x.RankedSeries
	}
	return nil
}

func (x *ListRankedSeriesResponse) GetPreviousToken() string {
	if x != nil {
		return x.PreviousToken
	}
	return ""
}

func (x *ListRankedSeriesResponse) GetNextToken() string {
	if x != nil {
		return x.NextToken
	}
	return ""
}

func (x *ListRankedSeriesResponse) GetComputedAt() string {
	if x != nil {
		return x.ComputedAt
	}
	return ""
}

func (x *ListRankedSeriesResponse) GetPeriodStart() string {
	if x != nil {
		return x.PeriodStart
	}
	return ""
}

func (x *ListRankedSeriesResponse) GetPeriodEnd() string {
	if x != nil {
		return x.PeriodEnd
	}
	return ""
}

// Cursor pagination. Field shape and token rules: proto/README.md.
// There is no order field: relatedness to one series is the order this RPC
// exists to produce. The token carries the series it was built for, because a
// score means nothing outside the list it was scored against; sending it with
// a different series is invalid_argument, and changing the series restarts at
// page 1.
type ListRelatedSeriesRequest struct {
	state  protoimpl.MessageState `protogen:"open.v1"`
	Tenant *v1.TenantContext      `protobuf:"bytes,1,opt,name=tenant,proto3" json:"tenant,omitempty"`
	// The series to find neighbours for. Unpublished, cross-tenant, and missing
	// series, and a series the calling surface may not show, are all surfaced as
	// not_found, the way every other public read of a series is.
	SeriesPublicId string `protobuf:"bytes,2,opt,name=series_public_id,json=seriesPublicId,proto3" json:"series_public_id,omitempty"`
	// Max items in one page. <= 0 or > 100 falls back to 12, which is what a
	// "more like this" strip on the series page and the end-of-episode panel
	// show.
	Limit int32 `protobuf:"varint,3,opt,name=limit,proto3" json:"limit,omitempty"`
	// Opaque token from a previous response. Empty for the first page.
	Token string `protobuf:"bytes,4,opt,name=token,proto3" json:"token,omitempty"`
	// The client making the read. Unspecified is answered as the web storefront.
	Surface       v1.ClientSurface `protobuf:"varint,5,opt,name=surface,proto3,enum=publira.types.v1.ClientSurface" json:"surface,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ListRelatedSeriesRequest) Reset() {
	*x = ListRelatedSeriesRequest{}
	mi := &file_publira_v1_catalog_proto_msgTypes[37]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ListRelatedSeriesRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ListRelatedSeriesRequest) ProtoMessage() {}

func (x *ListRelatedSeriesRequest) ProtoReflect() protoreflect.Message {
	mi := &file_publira_v1_catalog_proto_msgTypes[37]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ListRelatedSeriesRequest.ProtoReflect.Descriptor instead.
func (*ListRelatedSeriesRequest) Descriptor() ([]byte, []int) {
	return file_publira_v1_catalog_proto_rawDescGZIP(), []int{37}
}

func (x *ListRelatedSeriesRequest) GetTenant() *v1.TenantContext {
	if x != nil {
		return x.Tenant
	}
	return nil
}

func (x *ListRelatedSeriesRequest) GetSeriesPublicId() string {
	if x != nil {
		return x.SeriesPublicId
	}
	return ""
}

func (x *ListRelatedSeriesRequest) GetLimit() int32 {
	if x != nil {
		return x.Limit
	}
	return 0
}

func (x *ListRelatedSeriesRequest) GetToken() string {
	if x != nil {
		return x.Token
	}
	return ""
}

func (x *ListRelatedSeriesRequest) GetSurface() v1.ClientSurface {
	if x != nil {
		return x.Surface
	}
	return v1.ClientSurface(0)
}

type ListRelatedSeriesResponse struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// One page of the tenant's other published series, most related first.
	Series []*v1.Series `protobuf:"bytes,1,rep,name=series,proto3" json:"series,omitempty"`
	// Token for the previous page. Empty on the first page.
	PreviousToken string `protobuf:"bytes,2,opt,name=previous_token,json=previousToken,proto3" json:"previous_token,omitempty"`
	// Token for the next page. Empty on the last page.
	NextToken     string `protobuf:"bytes,3,opt,name=next_token,json=nextToken,proto3" json:"next_token,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ListRelatedSeriesResponse) Reset() {
	*x = ListRelatedSeriesResponse{}
	mi := &file_publira_v1_catalog_proto_msgTypes[38]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ListRelatedSeriesResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ListRelatedSeriesResponse) ProtoMessage() {}

func (x *ListRelatedSeriesResponse) ProtoReflect() protoreflect.Message {
	mi := &file_publira_v1_catalog_proto_msgTypes[38]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ListRelatedSeriesResponse.ProtoReflect.Descriptor instead.
func (*ListRelatedSeriesResponse) Descriptor() ([]byte, []int) {
	return file_publira_v1_catalog_proto_rawDescGZIP(), []int{38}
}

func (x *ListRelatedSeriesResponse) GetSeries() []*v1.Series {
	if x != nil {
		return x.Series
	}
	return nil
}

func (x *ListRelatedSeriesResponse) GetPreviousToken() string {
	if x != nil {
		return x.PreviousToken
	}
	return ""
}

func (x *ListRelatedSeriesResponse) GetNextToken() string {
	if x != nil {
		return x.NextToken
	}
	return ""
}

type MarkEpisodeAsReadRequest struct {
	state  protoimpl.MessageState `protogen:"open.v1"`
	Tenant *v1.TenantContext      `protobuf:"bytes,1,opt,name=tenant,proto3" json:"tenant,omitempty"`
	// The public ID of the episode the signed-in member finished reading.
	EpisodePublicId string `protobuf:"bytes,2,opt,name=episode_public_id,json=episodePublicId,proto3" json:"episode_public_id,omitempty"`
	// The client making the call. Unspecified is answered as the web storefront.
	Surface       v1.ClientSurface `protobuf:"varint,3,opt,name=surface,proto3,enum=publira.types.v1.ClientSurface" json:"surface,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *MarkEpisodeAsReadRequest) Reset() {
	*x = MarkEpisodeAsReadRequest{}
	mi := &file_publira_v1_catalog_proto_msgTypes[39]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *MarkEpisodeAsReadRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*MarkEpisodeAsReadRequest) ProtoMessage() {}

func (x *MarkEpisodeAsReadRequest) ProtoReflect() protoreflect.Message {
	mi := &file_publira_v1_catalog_proto_msgTypes[39]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use MarkEpisodeAsReadRequest.ProtoReflect.Descriptor instead.
func (*MarkEpisodeAsReadRequest) Descriptor() ([]byte, []int) {
	return file_publira_v1_catalog_proto_rawDescGZIP(), []int{39}
}

func (x *MarkEpisodeAsReadRequest) GetTenant() *v1.TenantContext {
	if x != nil {
		return x.Tenant
	}
	return nil
}

func (x *MarkEpisodeAsReadRequest) GetEpisodePublicId() string {
	if x != nil {
		return x.EpisodePublicId
	}
	return ""
}

func (x *MarkEpisodeAsReadRequest) GetSurface() v1.ClientSurface {
	if x != nil {
		return x.Surface
	}
	return v1.ClientSurface(0)
}

type MarkEpisodeAsReadResponse struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// The first time this member marked the episode as read. Repeated calls keep
	// this value unchanged.
	ReadAt        string `protobuf:"bytes,1,opt,name=read_at,json=readAt,proto3" json:"read_at,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *MarkEpisodeAsReadResponse) Reset() {
	*x = MarkEpisodeAsReadResponse{}
	mi := &file_publira_v1_catalog_proto_msgTypes[40]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *MarkEpisodeAsReadResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*MarkEpisodeAsReadResponse) ProtoMessage() {}

func (x *MarkEpisodeAsReadResponse) ProtoReflect() protoreflect.Message {
	mi := &file_publira_v1_catalog_proto_msgTypes[40]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use MarkEpisodeAsReadResponse.ProtoReflect.Descriptor instead.
func (*MarkEpisodeAsReadResponse) Descriptor() ([]byte, []int) {
	return file_publira_v1_catalog_proto_rawDescGZIP(), []int{40}
}

func (x *MarkEpisodeAsReadResponse) GetReadAt() string {
	if x != nil {
		return x.ReadAt
	}
	return ""
}

// Where a reader stopped inside one episode. page_index is zero-based, so the
// last page is page_count - 1; page_count is what the episode held when the
// position was saved, which is what lets a client show progress without
// reading the episode itself.
type ReadingPosition struct {
	state           protoimpl.MessageState `protogen:"open.v1"`
	EpisodePublicId string                 `protobuf:"bytes,1,opt,name=episode_public_id,json=episodePublicId,proto3" json:"episode_public_id,omitempty"`
	PageIndex       int32                  `protobuf:"varint,2,opt,name=page_index,json=pageIndex,proto3" json:"page_index,omitempty"`
	PageCount       int32                  `protobuf:"varint,3,opt,name=page_count,json=pageCount,proto3" json:"page_count,omitempty"`
	UpdatedAt       string                 `protobuf:"bytes,4,opt,name=updated_at,json=updatedAt,proto3" json:"updated_at,omitempty"`
	unknownFields   protoimpl.UnknownFields
	sizeCache       protoimpl.SizeCache
}

func (x *ReadingPosition) Reset() {
	*x = ReadingPosition{}
	mi := &file_publira_v1_catalog_proto_msgTypes[41]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ReadingPosition) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ReadingPosition) ProtoMessage() {}

func (x *ReadingPosition) ProtoReflect() protoreflect.Message {
	mi := &file_publira_v1_catalog_proto_msgTypes[41]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ReadingPosition.ProtoReflect.Descriptor instead.
func (*ReadingPosition) Descriptor() ([]byte, []int) {
	return file_publira_v1_catalog_proto_rawDescGZIP(), []int{41}
}

func (x *ReadingPosition) GetEpisodePublicId() string {
	if x != nil {
		return x.EpisodePublicId
	}
	return ""
}

func (x *ReadingPosition) GetPageIndex() int32 {
	if x != nil {
		return x.PageIndex
	}
	return 0
}

func (x *ReadingPosition) GetPageCount() int32 {
	if x != nil {
		return x.PageCount
	}
	return 0
}

func (x *ReadingPosition) GetUpdatedAt() string {
	if x != nil {
		return x.UpdatedAt
	}
	return ""
}

type SaveReadingPositionRequest struct {
	state  protoimpl.MessageState `protogen:"open.v1"`
	Tenant *v1.TenantContext      `protobuf:"bytes,1,opt,name=tenant,proto3" json:"tenant,omitempty"`
	// The public ID of the episode the signed-in member is reading.
	EpisodePublicId string `protobuf:"bytes,2,opt,name=episode_public_id,json=episodePublicId,proto3" json:"episode_public_id,omitempty"`
	// The zero-based page the member stopped on. A page outside the episode is
	// invalid_argument.
	PageIndex int32 `protobuf:"varint,3,opt,name=page_index,json=pageIndex,proto3" json:"page_index,omitempty"`
	// The client making the call. Unspecified is answered as the web storefront.
	Surface       v1.ClientSurface `protobuf:"varint,4,opt,name=surface,proto3,enum=publira.types.v1.ClientSurface" json:"surface,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *SaveReadingPositionRequest) Reset() {
	*x = SaveReadingPositionRequest{}
	mi := &file_publira_v1_catalog_proto_msgTypes[42]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *SaveReadingPositionRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*SaveReadingPositionRequest) ProtoMessage() {}

func (x *SaveReadingPositionRequest) ProtoReflect() protoreflect.Message {
	mi := &file_publira_v1_catalog_proto_msgTypes[42]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use SaveReadingPositionRequest.ProtoReflect.Descriptor instead.
func (*SaveReadingPositionRequest) Descriptor() ([]byte, []int) {
	return file_publira_v1_catalog_proto_rawDescGZIP(), []int{42}
}

func (x *SaveReadingPositionRequest) GetTenant() *v1.TenantContext {
	if x != nil {
		return x.Tenant
	}
	return nil
}

func (x *SaveReadingPositionRequest) GetEpisodePublicId() string {
	if x != nil {
		return x.EpisodePublicId
	}
	return ""
}

func (x *SaveReadingPositionRequest) GetPageIndex() int32 {
	if x != nil {
		return x.PageIndex
	}
	return 0
}

func (x *SaveReadingPositionRequest) GetSurface() v1.ClientSurface {
	if x != nil {
		return x.Surface
	}
	return v1.ClientSurface(0)
}

type SaveReadingPositionResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Position      *ReadingPosition       `protobuf:"bytes,1,opt,name=position,proto3" json:"position,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *SaveReadingPositionResponse) Reset() {
	*x = SaveReadingPositionResponse{}
	mi := &file_publira_v1_catalog_proto_msgTypes[43]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *SaveReadingPositionResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*SaveReadingPositionResponse) ProtoMessage() {}

func (x *SaveReadingPositionResponse) ProtoReflect() protoreflect.Message {
	mi := &file_publira_v1_catalog_proto_msgTypes[43]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use SaveReadingPositionResponse.ProtoReflect.Descriptor instead.
func (*SaveReadingPositionResponse) Descriptor() ([]byte, []int) {
	return file_publira_v1_catalog_proto_rawDescGZIP(), []int{43}
}

func (x *SaveReadingPositionResponse) GetPosition() *ReadingPosition {
	if x != nil {
		return x.Position
	}
	return nil
}

type GetMyReadingPositionRequest struct {
	state           protoimpl.MessageState `protogen:"open.v1"`
	Tenant          *v1.TenantContext      `protobuf:"bytes,1,opt,name=tenant,proto3" json:"tenant,omitempty"`
	EpisodePublicId string                 `protobuf:"bytes,2,opt,name=episode_public_id,json=episodePublicId,proto3" json:"episode_public_id,omitempty"`
	// The client making the read. Unspecified is answered as the web storefront.
	Surface       v1.ClientSurface `protobuf:"varint,3,opt,name=surface,proto3,enum=publira.types.v1.ClientSurface" json:"surface,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetMyReadingPositionRequest) Reset() {
	*x = GetMyReadingPositionRequest{}
	mi := &file_publira_v1_catalog_proto_msgTypes[44]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetMyReadingPositionRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetMyReadingPositionRequest) ProtoMessage() {}

func (x *GetMyReadingPositionRequest) ProtoReflect() protoreflect.Message {
	mi := &file_publira_v1_catalog_proto_msgTypes[44]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetMyReadingPositionRequest.ProtoReflect.Descriptor instead.
func (*GetMyReadingPositionRequest) Descriptor() ([]byte, []int) {
	return file_publira_v1_catalog_proto_rawDescGZIP(), []int{44}
}

func (x *GetMyReadingPositionRequest) GetTenant() *v1.TenantContext {
	if x != nil {
		return x.Tenant
	}
	return nil
}

func (x *GetMyReadingPositionRequest) GetEpisodePublicId() string {
	if x != nil {
		return x.EpisodePublicId
	}
	return ""
}

func (x *GetMyReadingPositionRequest) GetSurface() v1.ClientSurface {
	if x != nil {
		return x.Surface
	}
	return v1.ClientSurface(0)
}

type GetMyReadingPositionResponse struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Unset when this member has no saved position in the episode, which is what
	// a viewer opening it for the first time gets.
	Position      *ReadingPosition `protobuf:"bytes,1,opt,name=position,proto3" json:"position,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetMyReadingPositionResponse) Reset() {
	*x = GetMyReadingPositionResponse{}
	mi := &file_publira_v1_catalog_proto_msgTypes[45]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetMyReadingPositionResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetMyReadingPositionResponse) ProtoMessage() {}

func (x *GetMyReadingPositionResponse) ProtoReflect() protoreflect.Message {
	mi := &file_publira_v1_catalog_proto_msgTypes[45]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetMyReadingPositionResponse.ProtoReflect.Descriptor instead.
func (*GetMyReadingPositionResponse) Descriptor() ([]byte, []int) {
	return file_publira_v1_catalog_proto_rawDescGZIP(), []int{45}
}

func (x *GetMyReadingPositionResponse) GetPosition() *ReadingPosition {
	if x != nil {
		return x.Position
	}
	return nil
}

type GetMySeriesProgressRequest struct {
	state          protoimpl.MessageState `protogen:"open.v1"`
	Tenant         *v1.TenantContext      `protobuf:"bytes,1,opt,name=tenant,proto3" json:"tenant,omitempty"`
	SeriesPublicId string                 `protobuf:"bytes,2,opt,name=series_public_id,json=seriesPublicId,proto3" json:"series_public_id,omitempty"`
	// The client making the read. Unspecified is answered as the web storefront.
	Surface       v1.ClientSurface `protobuf:"varint,3,opt,name=surface,proto3,enum=publira.types.v1.ClientSurface" json:"surface,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetMySeriesProgressRequest) Reset() {
	*x = GetMySeriesProgressRequest{}
	mi := &file_publira_v1_catalog_proto_msgTypes[46]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetMySeriesProgressRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetMySeriesProgressRequest) ProtoMessage() {}

func (x *GetMySeriesProgressRequest) ProtoReflect() protoreflect.Message {
	mi := &file_publira_v1_catalog_proto_msgTypes[46]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetMySeriesProgressRequest.ProtoReflect.Descriptor instead.
func (*GetMySeriesProgressRequest) Descriptor() ([]byte, []int) {
	return file_publira_v1_catalog_proto_rawDescGZIP(), []int{46}
}

func (x *GetMySeriesProgressRequest) GetTenant() *v1.TenantContext {
	if x != nil {
		return x.Tenant
	}
	return nil
}

func (x *GetMySeriesProgressRequest) GetSeriesPublicId() string {
	if x != nil {
		return x.SeriesPublicId
	}
	return ""
}

func (x *GetMySeriesProgressRequest) GetSurface() v1.ClientSurface {
	if x != nil {
		return x.Surface
	}
	return v1.ClientSurface(0)
}

// The member's standing in one series: the episode they last moved in, where
// they stopped in it, and whether they already finished it. A series page
// turns these three into "continue from here" or "start the next episode".
type SeriesProgress struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Episode       *v1.Episode            `protobuf:"bytes,1,opt,name=episode,proto3" json:"episode,omitempty"`
	Position      *ReadingPosition       `protobuf:"bytes,2,opt,name=position,proto3" json:"position,omitempty"`
	IsFinished    bool                   `protobuf:"varint,3,opt,name=is_finished,json=isFinished,proto3" json:"is_finished,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *SeriesProgress) Reset() {
	*x = SeriesProgress{}
	mi := &file_publira_v1_catalog_proto_msgTypes[47]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *SeriesProgress) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*SeriesProgress) ProtoMessage() {}

func (x *SeriesProgress) ProtoReflect() protoreflect.Message {
	mi := &file_publira_v1_catalog_proto_msgTypes[47]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use SeriesProgress.ProtoReflect.Descriptor instead.
func (*SeriesProgress) Descriptor() ([]byte, []int) {
	return file_publira_v1_catalog_proto_rawDescGZIP(), []int{47}
}

func (x *SeriesProgress) GetEpisode() *v1.Episode {
	if x != nil {
		return x.Episode
	}
	return nil
}

func (x *SeriesProgress) GetPosition() *ReadingPosition {
	if x != nil {
		return x.Position
	}
	return nil
}

func (x *SeriesProgress) GetIsFinished() bool {
	if x != nil {
		return x.IsFinished
	}
	return false
}

type GetMySeriesProgressResponse struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Unset when the member has opened no episode of the series they can still
	// read.
	Progress *SeriesProgress `protobuf:"bytes,1,opt,name=progress,proto3" json:"progress,omitempty"`
	// The public IDs of this series' episodes the member has already finished,
	// in the order the series detail lists them. It rides on this response
	// rather than on GetSeriesDetail because it is the member's own state, and
	// rather than on an RPC of its own because the series page already makes
	// this one private read; the episode list marks its rows from it.
	//
	// It is not derived from `progress`: a member is marked as having finished
	// an episode by MarkEpisodeAsRead, which is a different write from the
	// position SaveReadingPosition stores, so a reader can have finished
	// episodes while `progress` is unset.
	FinishedEpisodePublicIds []string `protobuf:"bytes,2,rep,name=finished_episode_public_ids,json=finishedEpisodePublicIds,proto3" json:"finished_episode_public_ids,omitempty"`
	unknownFields            protoimpl.UnknownFields
	sizeCache                protoimpl.SizeCache
}

func (x *GetMySeriesProgressResponse) Reset() {
	*x = GetMySeriesProgressResponse{}
	mi := &file_publira_v1_catalog_proto_msgTypes[48]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetMySeriesProgressResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetMySeriesProgressResponse) ProtoMessage() {}

func (x *GetMySeriesProgressResponse) ProtoReflect() protoreflect.Message {
	mi := &file_publira_v1_catalog_proto_msgTypes[48]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetMySeriesProgressResponse.ProtoReflect.Descriptor instead.
func (*GetMySeriesProgressResponse) Descriptor() ([]byte, []int) {
	return file_publira_v1_catalog_proto_rawDescGZIP(), []int{48}
}

func (x *GetMySeriesProgressResponse) GetProgress() *SeriesProgress {
	if x != nil {
		return x.Progress
	}
	return nil
}

func (x *GetMySeriesProgressResponse) GetFinishedEpisodePublicIds() []string {
	if x != nil {
		return x.FinishedEpisodePublicIds
	}
	return nil
}

// One series in the reader's "continue reading" row: the series itself, the
// episode the offer points at, and where they stopped in that episode.
type RecentSeries struct {
	state  protoimpl.MessageState `protogen:"open.v1"`
	Series *v1.Series             `protobuf:"bytes,1,opt,name=series,proto3" json:"series,omitempty"`
	// The episode to open: the one the reader last moved in while it is still
	// unfinished, and otherwise the first published episode after it they have
	// not finished. It is offered whatever the reader may do with its body, so a
	// paid episode they have not bought is the episode they are sent to buy.
	Episode *v1.Episode `protobuf:"bytes,2,opt,name=episode,proto3" json:"episode,omitempty"`
	// Where the reader stopped in that episode. Unset when they have never
	// opened it, and also when they can no longer open its body: a page they
	// cannot reach is not a place to resume, which is the answer
	// GetMyReadingPosition gives for the same episode.
	Position *ReadingPosition `protobuf:"bytes,3,opt,name=position,proto3" json:"position,omitempty"`
	// The last time the reader moved in this series, over every published
	// episode of it they saved a position in or finished. It is the sort key of
	// the list.
	LastActivityAt string `protobuf:"bytes,4,opt,name=last_activity_at,json=lastActivityAt,proto3" json:"last_activity_at,omitempty"`
	unknownFields  protoimpl.UnknownFields
	sizeCache      protoimpl.SizeCache
}

func (x *RecentSeries) Reset() {
	*x = RecentSeries{}
	mi := &file_publira_v1_catalog_proto_msgTypes[49]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *RecentSeries) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*RecentSeries) ProtoMessage() {}

func (x *RecentSeries) ProtoReflect() protoreflect.Message {
	mi := &file_publira_v1_catalog_proto_msgTypes[49]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use RecentSeries.ProtoReflect.Descriptor instead.
func (*RecentSeries) Descriptor() ([]byte, []int) {
	return file_publira_v1_catalog_proto_rawDescGZIP(), []int{49}
}

func (x *RecentSeries) GetSeries() *v1.Series {
	if x != nil {
		return x.Series
	}
	return nil
}

func (x *RecentSeries) GetEpisode() *v1.Episode {
	if x != nil {
		return x.Episode
	}
	return nil
}

func (x *RecentSeries) GetPosition() *ReadingPosition {
	if x != nil {
		return x.Position
	}
	return nil
}

func (x *RecentSeries) GetLastActivityAt() string {
	if x != nil {
		return x.LastActivityAt
	}
	return ""
}

// Cursor pagination. Field shape and token rules: proto/README.md.
// There is no order field: the reader's own most recent activity is the one
// order a "continue reading" row has.
type ListMyRecentSeriesRequest struct {
	state  protoimpl.MessageState `protogen:"open.v1"`
	Tenant *v1.TenantContext      `protobuf:"bytes,1,opt,name=tenant,proto3" json:"tenant,omitempty"`
	// Max items in one page. <= 0 or > 100 falls back to 20.
	Limit int32 `protobuf:"varint,2,opt,name=limit,proto3" json:"limit,omitempty"`
	// Opaque token from a previous response. Empty for the first page.
	Token string `protobuf:"bytes,3,opt,name=token,proto3" json:"token,omitempty"`
	// The client making the read. Unspecified is answered as the web storefront.
	Surface       v1.ClientSurface `protobuf:"varint,4,opt,name=surface,proto3,enum=publira.types.v1.ClientSurface" json:"surface,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ListMyRecentSeriesRequest) Reset() {
	*x = ListMyRecentSeriesRequest{}
	mi := &file_publira_v1_catalog_proto_msgTypes[50]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ListMyRecentSeriesRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ListMyRecentSeriesRequest) ProtoMessage() {}

func (x *ListMyRecentSeriesRequest) ProtoReflect() protoreflect.Message {
	mi := &file_publira_v1_catalog_proto_msgTypes[50]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ListMyRecentSeriesRequest.ProtoReflect.Descriptor instead.
func (*ListMyRecentSeriesRequest) Descriptor() ([]byte, []int) {
	return file_publira_v1_catalog_proto_rawDescGZIP(), []int{50}
}

func (x *ListMyRecentSeriesRequest) GetTenant() *v1.TenantContext {
	if x != nil {
		return x.Tenant
	}
	return nil
}

func (x *ListMyRecentSeriesRequest) GetLimit() int32 {
	if x != nil {
		return x.Limit
	}
	return 0
}

func (x *ListMyRecentSeriesRequest) GetToken() string {
	if x != nil {
		return x.Token
	}
	return ""
}

func (x *ListMyRecentSeriesRequest) GetSurface() v1.ClientSurface {
	if x != nil {
		return x.Surface
	}
	return v1.ClientSurface(0)
}

type ListMyRecentSeriesResponse struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Newest activity first.
	Series []*RecentSeries `protobuf:"bytes,1,rep,name=series,proto3" json:"series,omitempty"`
	// Token for the previous page. Empty on the first page.
	PreviousToken string `protobuf:"bytes,2,opt,name=previous_token,json=previousToken,proto3" json:"previous_token,omitempty"`
	// Token for the next page. Empty on the last page.
	NextToken     string `protobuf:"bytes,3,opt,name=next_token,json=nextToken,proto3" json:"next_token,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ListMyRecentSeriesResponse) Reset() {
	*x = ListMyRecentSeriesResponse{}
	mi := &file_publira_v1_catalog_proto_msgTypes[51]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ListMyRecentSeriesResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ListMyRecentSeriesResponse) ProtoMessage() {}

func (x *ListMyRecentSeriesResponse) ProtoReflect() protoreflect.Message {
	mi := &file_publira_v1_catalog_proto_msgTypes[51]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ListMyRecentSeriesResponse.ProtoReflect.Descriptor instead.
func (*ListMyRecentSeriesResponse) Descriptor() ([]byte, []int) {
	return file_publira_v1_catalog_proto_rawDescGZIP(), []int{51}
}

func (x *ListMyRecentSeriesResponse) GetSeries() []*RecentSeries {
	if x != nil {
		return x.Series
	}
	return nil
}

func (x *ListMyRecentSeriesResponse) GetPreviousToken() string {
	if x != nil {
		return x.PreviousToken
	}
	return ""
}

func (x *ListMyRecentSeriesResponse) GetNextToken() string {
	if x != nil {
		return x.NextToken
	}
	return ""
}

// One episode the reader has finished, with the series it belongs to, which is
// what a reading history entry links to and names.
type MyEpisodeRead struct {
	state   protoimpl.MessageState `protogen:"open.v1"`
	Series  *v1.Series             `protobuf:"bytes,1,opt,name=series,proto3" json:"series,omitempty"`
	Episode *v1.Episode            `protobuf:"bytes,2,opt,name=episode,proto3" json:"episode,omitempty"`
	// When the reader first finished it. A re-read keeps this instant, so the
	// history is ordered by when each episode was finished rather than by when
	// it was last opened.
	ReadAt        string `protobuf:"bytes,3,opt,name=read_at,json=readAt,proto3" json:"read_at,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *MyEpisodeRead) Reset() {
	*x = MyEpisodeRead{}
	mi := &file_publira_v1_catalog_proto_msgTypes[52]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *MyEpisodeRead) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*MyEpisodeRead) ProtoMessage() {}

func (x *MyEpisodeRead) ProtoReflect() protoreflect.Message {
	mi := &file_publira_v1_catalog_proto_msgTypes[52]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use MyEpisodeRead.ProtoReflect.Descriptor instead.
func (*MyEpisodeRead) Descriptor() ([]byte, []int) {
	return file_publira_v1_catalog_proto_rawDescGZIP(), []int{52}
}

func (x *MyEpisodeRead) GetSeries() *v1.Series {
	if x != nil {
		return x.Series
	}
	return nil
}

func (x *MyEpisodeRead) GetEpisode() *v1.Episode {
	if x != nil {
		return x.Episode
	}
	return nil
}

func (x *MyEpisodeRead) GetReadAt() string {
	if x != nil {
		return x.ReadAt
	}
	return ""
}

// Cursor pagination. Field shape and token rules: proto/README.md.
// There is no order field: a reading history is read newest first, and the
// reader has no second order to ask for.
type ListMyEpisodeReadsRequest struct {
	state  protoimpl.MessageState `protogen:"open.v1"`
	Tenant *v1.TenantContext      `protobuf:"bytes,1,opt,name=tenant,proto3" json:"tenant,omitempty"`
	// Max items in one page. <= 0 or > 100 falls back to 20.
	Limit int32 `protobuf:"varint,2,opt,name=limit,proto3" json:"limit,omitempty"`
	// Opaque token from a previous response. Empty for the first page.
	Token string `protobuf:"bytes,3,opt,name=token,proto3" json:"token,omitempty"`
	// The client making the read. Unspecified is answered as the web storefront.
	Surface       v1.ClientSurface `protobuf:"varint,4,opt,name=surface,proto3,enum=publira.types.v1.ClientSurface" json:"surface,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ListMyEpisodeReadsRequest) Reset() {
	*x = ListMyEpisodeReadsRequest{}
	mi := &file_publira_v1_catalog_proto_msgTypes[53]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ListMyEpisodeReadsRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ListMyEpisodeReadsRequest) ProtoMessage() {}

func (x *ListMyEpisodeReadsRequest) ProtoReflect() protoreflect.Message {
	mi := &file_publira_v1_catalog_proto_msgTypes[53]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ListMyEpisodeReadsRequest.ProtoReflect.Descriptor instead.
func (*ListMyEpisodeReadsRequest) Descriptor() ([]byte, []int) {
	return file_publira_v1_catalog_proto_rawDescGZIP(), []int{53}
}

func (x *ListMyEpisodeReadsRequest) GetTenant() *v1.TenantContext {
	if x != nil {
		return x.Tenant
	}
	return nil
}

func (x *ListMyEpisodeReadsRequest) GetLimit() int32 {
	if x != nil {
		return x.Limit
	}
	return 0
}

func (x *ListMyEpisodeReadsRequest) GetToken() string {
	if x != nil {
		return x.Token
	}
	return ""
}

func (x *ListMyEpisodeReadsRequest) GetSurface() v1.ClientSurface {
	if x != nil {
		return x.Surface
	}
	return v1.ClientSurface(0)
}

type ListMyEpisodeReadsResponse struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Most recently finished first.
	Reads []*MyEpisodeRead `protobuf:"bytes,1,rep,name=reads,proto3" json:"reads,omitempty"`
	// Token for the previous page. Empty on the first page.
	PreviousToken string `protobuf:"bytes,2,opt,name=previous_token,json=previousToken,proto3" json:"previous_token,omitempty"`
	// Token for the next page. Empty on the last page.
	NextToken     string `protobuf:"bytes,3,opt,name=next_token,json=nextToken,proto3" json:"next_token,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ListMyEpisodeReadsResponse) Reset() {
	*x = ListMyEpisodeReadsResponse{}
	mi := &file_publira_v1_catalog_proto_msgTypes[54]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ListMyEpisodeReadsResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ListMyEpisodeReadsResponse) ProtoMessage() {}

func (x *ListMyEpisodeReadsResponse) ProtoReflect() protoreflect.Message {
	mi := &file_publira_v1_catalog_proto_msgTypes[54]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ListMyEpisodeReadsResponse.ProtoReflect.Descriptor instead.
func (*ListMyEpisodeReadsResponse) Descriptor() ([]byte, []int) {
	return file_publira_v1_catalog_proto_rawDescGZIP(), []int{54}
}

func (x *ListMyEpisodeReadsResponse) GetReads() []*MyEpisodeRead {
	if x != nil {
		return x.Reads
	}
	return nil
}

func (x *ListMyEpisodeReadsResponse) GetPreviousToken() string {
	if x != nil {
		return x.PreviousToken
	}
	return ""
}

func (x *ListMyEpisodeReadsResponse) GetNextToken() string {
	if x != nil {
		return x.NextToken
	}
	return ""
}

// A public catalog target. The public ID is interpreted according to type.
type FollowTarget struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Type          FollowTargetType       `protobuf:"varint,1,opt,name=type,proto3,enum=publira.v1.FollowTargetType" json:"type,omitempty"`
	PublicId      string                 `protobuf:"bytes,2,opt,name=public_id,json=publicId,proto3" json:"public_id,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *FollowTarget) Reset() {
	*x = FollowTarget{}
	mi := &file_publira_v1_catalog_proto_msgTypes[55]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *FollowTarget) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*FollowTarget) ProtoMessage() {}

func (x *FollowTarget) ProtoReflect() protoreflect.Message {
	mi := &file_publira_v1_catalog_proto_msgTypes[55]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use FollowTarget.ProtoReflect.Descriptor instead.
func (*FollowTarget) Descriptor() ([]byte, []int) {
	return file_publira_v1_catalog_proto_rawDescGZIP(), []int{55}
}

func (x *FollowTarget) GetType() FollowTargetType {
	if x != nil {
		return x.Type
	}
	return FollowTargetType_FOLLOW_TARGET_TYPE_UNSPECIFIED
}

func (x *FollowTarget) GetPublicId() string {
	if x != nil {
		return x.PublicId
	}
	return ""
}

// The current member's state for one public target. This is deliberately kept
// separate from CatalogService so catalog reads remain shared-cacheable.
type GetMyFollowStatusRequest struct {
	state  protoimpl.MessageState `protogen:"open.v1"`
	Tenant *v1.TenantContext      `protobuf:"bytes,1,opt,name=tenant,proto3" json:"tenant,omitempty"`
	Target *FollowTarget          `protobuf:"bytes,2,opt,name=target,proto3" json:"target,omitempty"`
	// The client making the read. Unspecified is answered as the web storefront.
	Surface       v1.ClientSurface `protobuf:"varint,3,opt,name=surface,proto3,enum=publira.types.v1.ClientSurface" json:"surface,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetMyFollowStatusRequest) Reset() {
	*x = GetMyFollowStatusRequest{}
	mi := &file_publira_v1_catalog_proto_msgTypes[56]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetMyFollowStatusRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetMyFollowStatusRequest) ProtoMessage() {}

func (x *GetMyFollowStatusRequest) ProtoReflect() protoreflect.Message {
	mi := &file_publira_v1_catalog_proto_msgTypes[56]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetMyFollowStatusRequest.ProtoReflect.Descriptor instead.
func (*GetMyFollowStatusRequest) Descriptor() ([]byte, []int) {
	return file_publira_v1_catalog_proto_rawDescGZIP(), []int{56}
}

func (x *GetMyFollowStatusRequest) GetTenant() *v1.TenantContext {
	if x != nil {
		return x.Tenant
	}
	return nil
}

func (x *GetMyFollowStatusRequest) GetTarget() *FollowTarget {
	if x != nil {
		return x.Target
	}
	return nil
}

func (x *GetMyFollowStatusRequest) GetSurface() v1.ClientSurface {
	if x != nil {
		return x.Surface
	}
	return v1.ClientSurface(0)
}

type GetMyFollowStatusResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	IsFollowing   bool                   `protobuf:"varint,1,opt,name=is_following,json=isFollowing,proto3" json:"is_following,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetMyFollowStatusResponse) Reset() {
	*x = GetMyFollowStatusResponse{}
	mi := &file_publira_v1_catalog_proto_msgTypes[57]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetMyFollowStatusResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetMyFollowStatusResponse) ProtoMessage() {}

func (x *GetMyFollowStatusResponse) ProtoReflect() protoreflect.Message {
	mi := &file_publira_v1_catalog_proto_msgTypes[57]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetMyFollowStatusResponse.ProtoReflect.Descriptor instead.
func (*GetMyFollowStatusResponse) Descriptor() ([]byte, []int) {
	return file_publira_v1_catalog_proto_rawDescGZIP(), []int{57}
}

func (x *GetMyFollowStatusResponse) GetIsFollowing() bool {
	if x != nil {
		return x.IsFollowing
	}
	return false
}

type FollowRequest struct {
	state  protoimpl.MessageState `protogen:"open.v1"`
	Tenant *v1.TenantContext      `protobuf:"bytes,1,opt,name=tenant,proto3" json:"tenant,omitempty"`
	Target *FollowTarget          `protobuf:"bytes,2,opt,name=target,proto3" json:"target,omitempty"`
	// The client making the call. Unspecified is answered as the web storefront.
	Surface       v1.ClientSurface `protobuf:"varint,3,opt,name=surface,proto3,enum=publira.types.v1.ClientSurface" json:"surface,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *FollowRequest) Reset() {
	*x = FollowRequest{}
	mi := &file_publira_v1_catalog_proto_msgTypes[58]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *FollowRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*FollowRequest) ProtoMessage() {}

func (x *FollowRequest) ProtoReflect() protoreflect.Message {
	mi := &file_publira_v1_catalog_proto_msgTypes[58]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use FollowRequest.ProtoReflect.Descriptor instead.
func (*FollowRequest) Descriptor() ([]byte, []int) {
	return file_publira_v1_catalog_proto_rawDescGZIP(), []int{58}
}

func (x *FollowRequest) GetTenant() *v1.TenantContext {
	if x != nil {
		return x.Tenant
	}
	return nil
}

func (x *FollowRequest) GetTarget() *FollowTarget {
	if x != nil {
		return x.Target
	}
	return nil
}

func (x *FollowRequest) GetSurface() v1.ClientSurface {
	if x != nil {
		return x.Surface
	}
	return v1.ClientSurface(0)
}

type FollowResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	IsFollowing   bool                   `protobuf:"varint,1,opt,name=is_following,json=isFollowing,proto3" json:"is_following,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *FollowResponse) Reset() {
	*x = FollowResponse{}
	mi := &file_publira_v1_catalog_proto_msgTypes[59]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *FollowResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*FollowResponse) ProtoMessage() {}

func (x *FollowResponse) ProtoReflect() protoreflect.Message {
	mi := &file_publira_v1_catalog_proto_msgTypes[59]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use FollowResponse.ProtoReflect.Descriptor instead.
func (*FollowResponse) Descriptor() ([]byte, []int) {
	return file_publira_v1_catalog_proto_rawDescGZIP(), []int{59}
}

func (x *FollowResponse) GetIsFollowing() bool {
	if x != nil {
		return x.IsFollowing
	}
	return false
}

type UnfollowRequest struct {
	state  protoimpl.MessageState `protogen:"open.v1"`
	Tenant *v1.TenantContext      `protobuf:"bytes,1,opt,name=tenant,proto3" json:"tenant,omitempty"`
	Target *FollowTarget          `protobuf:"bytes,2,opt,name=target,proto3" json:"target,omitempty"`
	// The client making the call. Unspecified is answered as the web storefront.
	Surface       v1.ClientSurface `protobuf:"varint,3,opt,name=surface,proto3,enum=publira.types.v1.ClientSurface" json:"surface,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *UnfollowRequest) Reset() {
	*x = UnfollowRequest{}
	mi := &file_publira_v1_catalog_proto_msgTypes[60]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *UnfollowRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*UnfollowRequest) ProtoMessage() {}

func (x *UnfollowRequest) ProtoReflect() protoreflect.Message {
	mi := &file_publira_v1_catalog_proto_msgTypes[60]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use UnfollowRequest.ProtoReflect.Descriptor instead.
func (*UnfollowRequest) Descriptor() ([]byte, []int) {
	return file_publira_v1_catalog_proto_rawDescGZIP(), []int{60}
}

func (x *UnfollowRequest) GetTenant() *v1.TenantContext {
	if x != nil {
		return x.Tenant
	}
	return nil
}

func (x *UnfollowRequest) GetTarget() *FollowTarget {
	if x != nil {
		return x.Target
	}
	return nil
}

func (x *UnfollowRequest) GetSurface() v1.ClientSurface {
	if x != nil {
		return x.Surface
	}
	return v1.ClientSurface(0)
}

type UnfollowResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	IsFollowing   bool                   `protobuf:"varint,1,opt,name=is_following,json=isFollowing,proto3" json:"is_following,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *UnfollowResponse) Reset() {
	*x = UnfollowResponse{}
	mi := &file_publira_v1_catalog_proto_msgTypes[61]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *UnfollowResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*UnfollowResponse) ProtoMessage() {}

func (x *UnfollowResponse) ProtoReflect() protoreflect.Message {
	mi := &file_publira_v1_catalog_proto_msgTypes[61]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use UnfollowResponse.ProtoReflect.Descriptor instead.
func (*UnfollowResponse) Descriptor() ([]byte, []int) {
	return file_publira_v1_catalog_proto_rawDescGZIP(), []int{61}
}

func (x *UnfollowResponse) GetIsFollowing() bool {
	if x != nil {
		return x.IsFollowing
	}
	return false
}

// One currently public target followed by the authenticated member.
type MyFollow struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Explicitly distinguishes episode, creator, and series follows in a mixed timeline.
	TargetType     FollowTargetType `protobuf:"varint,1,opt,name=target_type,json=targetType,proto3,enum=publira.v1.FollowTargetType" json:"target_type,omitempty"`
	TargetPublicId string           `protobuf:"bytes,2,opt,name=target_public_id,json=targetPublicId,proto3" json:"target_public_id,omitempty"`
	FollowedAt     string           `protobuf:"bytes,3,opt,name=followed_at,json=followedAt,proto3" json:"followed_at,omitempty"`
	unknownFields  protoimpl.UnknownFields
	sizeCache      protoimpl.SizeCache
}

func (x *MyFollow) Reset() {
	*x = MyFollow{}
	mi := &file_publira_v1_catalog_proto_msgTypes[62]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *MyFollow) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*MyFollow) ProtoMessage() {}

func (x *MyFollow) ProtoReflect() protoreflect.Message {
	mi := &file_publira_v1_catalog_proto_msgTypes[62]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use MyFollow.ProtoReflect.Descriptor instead.
func (*MyFollow) Descriptor() ([]byte, []int) {
	return file_publira_v1_catalog_proto_rawDescGZIP(), []int{62}
}

func (x *MyFollow) GetTargetType() FollowTargetType {
	if x != nil {
		return x.TargetType
	}
	return FollowTargetType_FOLLOW_TARGET_TYPE_UNSPECIFIED
}

func (x *MyFollow) GetTargetPublicId() string {
	if x != nil {
		return x.TargetPublicId
	}
	return ""
}

func (x *MyFollow) GetFollowedAt() string {
	if x != nil {
		return x.FollowedAt
	}
	return ""
}

// Cursor pagination. Field shape and token rules: proto/README.md.
type ListMyFollowsRequest struct {
	state  protoimpl.MessageState `protogen:"open.v1"`
	Tenant *v1.TenantContext      `protobuf:"bytes,1,opt,name=tenant,proto3" json:"tenant,omitempty"`
	// Max items in one page. <= 0 or > 100 falls back to 20.
	Limit int32 `protobuf:"varint,2,opt,name=limit,proto3" json:"limit,omitempty"`
	// Opaque token from a previous response. Empty for the first page.
	Token string `protobuf:"bytes,3,opt,name=token,proto3" json:"token,omitempty"`
	// The client making the read. Unspecified is answered as the web storefront.
	Surface       v1.ClientSurface `protobuf:"varint,4,opt,name=surface,proto3,enum=publira.types.v1.ClientSurface" json:"surface,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ListMyFollowsRequest) Reset() {
	*x = ListMyFollowsRequest{}
	mi := &file_publira_v1_catalog_proto_msgTypes[63]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ListMyFollowsRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ListMyFollowsRequest) ProtoMessage() {}

func (x *ListMyFollowsRequest) ProtoReflect() protoreflect.Message {
	mi := &file_publira_v1_catalog_proto_msgTypes[63]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ListMyFollowsRequest.ProtoReflect.Descriptor instead.
func (*ListMyFollowsRequest) Descriptor() ([]byte, []int) {
	return file_publira_v1_catalog_proto_rawDescGZIP(), []int{63}
}

func (x *ListMyFollowsRequest) GetTenant() *v1.TenantContext {
	if x != nil {
		return x.Tenant
	}
	return nil
}

func (x *ListMyFollowsRequest) GetLimit() int32 {
	if x != nil {
		return x.Limit
	}
	return 0
}

func (x *ListMyFollowsRequest) GetToken() string {
	if x != nil {
		return x.Token
	}
	return ""
}

func (x *ListMyFollowsRequest) GetSurface() v1.ClientSurface {
	if x != nil {
		return x.Surface
	}
	return v1.ClientSurface(0)
}

type ListMyFollowsResponse struct {
	state   protoimpl.MessageState `protogen:"open.v1"`
	Follows []*MyFollow            `protobuf:"bytes,1,rep,name=follows,proto3" json:"follows,omitempty"`
	// Token for the previous page. Empty on the first page.
	PreviousToken string `protobuf:"bytes,2,opt,name=previous_token,json=previousToken,proto3" json:"previous_token,omitempty"`
	// Token for the next page. Empty on the last page.
	NextToken     string `protobuf:"bytes,3,opt,name=next_token,json=nextToken,proto3" json:"next_token,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ListMyFollowsResponse) Reset() {
	*x = ListMyFollowsResponse{}
	mi := &file_publira_v1_catalog_proto_msgTypes[64]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ListMyFollowsResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ListMyFollowsResponse) ProtoMessage() {}

func (x *ListMyFollowsResponse) ProtoReflect() protoreflect.Message {
	mi := &file_publira_v1_catalog_proto_msgTypes[64]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ListMyFollowsResponse.ProtoReflect.Descriptor instead.
func (*ListMyFollowsResponse) Descriptor() ([]byte, []int) {
	return file_publira_v1_catalog_proto_rawDescGZIP(), []int{64}
}

func (x *ListMyFollowsResponse) GetFollows() []*MyFollow {
	if x != nil {
		return x.Follows
	}
	return nil
}

func (x *ListMyFollowsResponse) GetPreviousToken() string {
	if x != nil {
		return x.PreviousToken
	}
	return ""
}

func (x *ListMyFollowsResponse) GetNextToken() string {
	if x != nil {
		return x.NextToken
	}
	return ""
}

// One episode that has arrived in what a member follows: the episode, and the
// series it belongs to, which is what the row links to and names.
type FollowUpdate struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Series        *v1.Series             `protobuf:"bytes,1,opt,name=series,proto3" json:"series,omitempty"`
	Episode       *v1.Episode            `protobuf:"bytes,2,opt,name=episode,proto3" json:"episode,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *FollowUpdate) Reset() {
	*x = FollowUpdate{}
	mi := &file_publira_v1_catalog_proto_msgTypes[65]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *FollowUpdate) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*FollowUpdate) ProtoMessage() {}

func (x *FollowUpdate) ProtoReflect() protoreflect.Message {
	mi := &file_publira_v1_catalog_proto_msgTypes[65]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use FollowUpdate.ProtoReflect.Descriptor instead.
func (*FollowUpdate) Descriptor() ([]byte, []int) {
	return file_publira_v1_catalog_proto_rawDescGZIP(), []int{65}
}

func (x *FollowUpdate) GetSeries() *v1.Series {
	if x != nil {
		return x.Series
	}
	return nil
}

func (x *FollowUpdate) GetEpisode() *v1.Episode {
	if x != nil {
		return x.Episode
	}
	return nil
}

// Cursor pagination. Field shape and token rules: proto/README.md.
// There is no order field: what is new is read newest first, and a member has
// no second order to ask for.
type ListMyFollowUpdatesRequest struct {
	state  protoimpl.MessageState `protogen:"open.v1"`
	Tenant *v1.TenantContext      `protobuf:"bytes,1,opt,name=tenant,proto3" json:"tenant,omitempty"`
	// Max items in one page. <= 0 or > 100 falls back to 20.
	Limit int32 `protobuf:"varint,2,opt,name=limit,proto3" json:"limit,omitempty"`
	// Opaque token from a previous response. Empty for the first page.
	Token string `protobuf:"bytes,3,opt,name=token,proto3" json:"token,omitempty"`
	// The client making the read. Unspecified is answered as the web storefront.
	Surface       v1.ClientSurface `protobuf:"varint,4,opt,name=surface,proto3,enum=publira.types.v1.ClientSurface" json:"surface,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ListMyFollowUpdatesRequest) Reset() {
	*x = ListMyFollowUpdatesRequest{}
	mi := &file_publira_v1_catalog_proto_msgTypes[66]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ListMyFollowUpdatesRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ListMyFollowUpdatesRequest) ProtoMessage() {}

func (x *ListMyFollowUpdatesRequest) ProtoReflect() protoreflect.Message {
	mi := &file_publira_v1_catalog_proto_msgTypes[66]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ListMyFollowUpdatesRequest.ProtoReflect.Descriptor instead.
func (*ListMyFollowUpdatesRequest) Descriptor() ([]byte, []int) {
	return file_publira_v1_catalog_proto_rawDescGZIP(), []int{66}
}

func (x *ListMyFollowUpdatesRequest) GetTenant() *v1.TenantContext {
	if x != nil {
		return x.Tenant
	}
	return nil
}

func (x *ListMyFollowUpdatesRequest) GetLimit() int32 {
	if x != nil {
		return x.Limit
	}
	return 0
}

func (x *ListMyFollowUpdatesRequest) GetToken() string {
	if x != nil {
		return x.Token
	}
	return ""
}

func (x *ListMyFollowUpdatesRequest) GetSurface() v1.ClientSurface {
	if x != nil {
		return x.Surface
	}
	return v1.ClientSurface(0)
}

type ListMyFollowUpdatesResponse struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Most recently published first.
	Updates []*FollowUpdate `protobuf:"bytes,1,rep,name=updates,proto3" json:"updates,omitempty"`
	// Token for the previous page. Empty on the first page.
	PreviousToken string `protobuf:"bytes,2,opt,name=previous_token,json=previousToken,proto3" json:"previous_token,omitempty"`
	// Token for the next page. Empty on the last page.
	NextToken     string `protobuf:"bytes,3,opt,name=next_token,json=nextToken,proto3" json:"next_token,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ListMyFollowUpdatesResponse) Reset() {
	*x = ListMyFollowUpdatesResponse{}
	mi := &file_publira_v1_catalog_proto_msgTypes[67]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ListMyFollowUpdatesResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ListMyFollowUpdatesResponse) ProtoMessage() {}

func (x *ListMyFollowUpdatesResponse) ProtoReflect() protoreflect.Message {
	mi := &file_publira_v1_catalog_proto_msgTypes[67]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ListMyFollowUpdatesResponse.ProtoReflect.Descriptor instead.
func (*ListMyFollowUpdatesResponse) Descriptor() ([]byte, []int) {
	return file_publira_v1_catalog_proto_rawDescGZIP(), []int{67}
}

func (x *ListMyFollowUpdatesResponse) GetUpdates() []*FollowUpdate {
	if x != nil {
		return x.Updates
	}
	return nil
}

func (x *ListMyFollowUpdatesResponse) GetPreviousToken() string {
	if x != nil {
		return x.PreviousToken
	}
	return ""
}

func (x *ListMyFollowUpdatesResponse) GetNextToken() string {
	if x != nil {
		return x.NextToken
	}
	return ""
}

// Records the authenticated reader's reaction to one episode, on the 1-5 scale
// with no neutral point: more is better, and there is no way to call an episode
// bad. A rating cannot be taken back or lowered, so there is no RPC for either,
// and pressing again only ever raises the score towards its ceiling.
type RateEpisodeRequest struct {
	state           protoimpl.MessageState `protogen:"open.v1"`
	Tenant          *v1.TenantContext      `protobuf:"bytes,1,opt,name=tenant,proto3" json:"tenant,omitempty"`
	EpisodePublicId string                 `protobuf:"bytes,2,opt,name=episode_public_id,json=episodePublicId,proto3" json:"episode_public_id,omitempty"`
	// How many presses this call reports, so a client that batched a burst of
	// them makes one request. <= 0 counts as one, and anything above 5 is capped.
	// Ignored in EPISODE_RATING_MODE_SINGLE, where one press is the whole rating.
	Presses int32 `protobuf:"varint,3,opt,name=presses,proto3" json:"presses,omitempty"`
	// The client making the call. Unspecified is answered as the web storefront.
	Surface       v1.ClientSurface `protobuf:"varint,4,opt,name=surface,proto3,enum=publira.types.v1.ClientSurface" json:"surface,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *RateEpisodeRequest) Reset() {
	*x = RateEpisodeRequest{}
	mi := &file_publira_v1_catalog_proto_msgTypes[68]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *RateEpisodeRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*RateEpisodeRequest) ProtoMessage() {}

func (x *RateEpisodeRequest) ProtoReflect() protoreflect.Message {
	mi := &file_publira_v1_catalog_proto_msgTypes[68]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use RateEpisodeRequest.ProtoReflect.Descriptor instead.
func (*RateEpisodeRequest) Descriptor() ([]byte, []int) {
	return file_publira_v1_catalog_proto_rawDescGZIP(), []int{68}
}

func (x *RateEpisodeRequest) GetTenant() *v1.TenantContext {
	if x != nil {
		return x.Tenant
	}
	return nil
}

func (x *RateEpisodeRequest) GetEpisodePublicId() string {
	if x != nil {
		return x.EpisodePublicId
	}
	return ""
}

func (x *RateEpisodeRequest) GetPresses() int32 {
	if x != nil {
		return x.Presses
	}
	return 0
}

func (x *RateEpisodeRequest) GetSurface() v1.ClientSurface {
	if x != nil {
		return x.Surface
	}
	return v1.ClientSurface(0)
}

type RateEpisodeResponse struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// The reader's score after this call, 1 to 5.
	Score int32 `protobuf:"varint,1,opt,name=score,proto3" json:"score,omitempty"`
	// How many readers have rated this episode, the same figure
	// `publira.types.v1.Episode.rating_count` carries. It counts readers rather
	// than presses, so it means the same thing whichever mode the tenant chose.
	RatingCount   int64             `protobuf:"varint,2,opt,name=rating_count,json=ratingCount,proto3" json:"rating_count,omitempty"`
	Mode          EpisodeRatingMode `protobuf:"varint,3,opt,name=mode,proto3,enum=publira.v1.EpisodeRatingMode" json:"mode,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *RateEpisodeResponse) Reset() {
	*x = RateEpisodeResponse{}
	mi := &file_publira_v1_catalog_proto_msgTypes[69]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *RateEpisodeResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*RateEpisodeResponse) ProtoMessage() {}

func (x *RateEpisodeResponse) ProtoReflect() protoreflect.Message {
	mi := &file_publira_v1_catalog_proto_msgTypes[69]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use RateEpisodeResponse.ProtoReflect.Descriptor instead.
func (*RateEpisodeResponse) Descriptor() ([]byte, []int) {
	return file_publira_v1_catalog_proto_rawDescGZIP(), []int{69}
}

func (x *RateEpisodeResponse) GetScore() int32 {
	if x != nil {
		return x.Score
	}
	return 0
}

func (x *RateEpisodeResponse) GetRatingCount() int64 {
	if x != nil {
		return x.RatingCount
	}
	return 0
}

func (x *RateEpisodeResponse) GetMode() EpisodeRatingMode {
	if x != nil {
		return x.Mode
	}
	return EpisodeRatingMode_EPISODE_RATING_MODE_UNSPECIFIED
}

type GetMyEpisodeRatingRequest struct {
	state           protoimpl.MessageState `protogen:"open.v1"`
	Tenant          *v1.TenantContext      `protobuf:"bytes,1,opt,name=tenant,proto3" json:"tenant,omitempty"`
	EpisodePublicId string                 `protobuf:"bytes,2,opt,name=episode_public_id,json=episodePublicId,proto3" json:"episode_public_id,omitempty"`
	// The client making the read. Unspecified is answered as the web storefront.
	Surface       v1.ClientSurface `protobuf:"varint,3,opt,name=surface,proto3,enum=publira.types.v1.ClientSurface" json:"surface,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetMyEpisodeRatingRequest) Reset() {
	*x = GetMyEpisodeRatingRequest{}
	mi := &file_publira_v1_catalog_proto_msgTypes[70]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetMyEpisodeRatingRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetMyEpisodeRatingRequest) ProtoMessage() {}

func (x *GetMyEpisodeRatingRequest) ProtoReflect() protoreflect.Message {
	mi := &file_publira_v1_catalog_proto_msgTypes[70]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetMyEpisodeRatingRequest.ProtoReflect.Descriptor instead.
func (*GetMyEpisodeRatingRequest) Descriptor() ([]byte, []int) {
	return file_publira_v1_catalog_proto_rawDescGZIP(), []int{70}
}

func (x *GetMyEpisodeRatingRequest) GetTenant() *v1.TenantContext {
	if x != nil {
		return x.Tenant
	}
	return nil
}

func (x *GetMyEpisodeRatingRequest) GetEpisodePublicId() string {
	if x != nil {
		return x.EpisodePublicId
	}
	return ""
}

func (x *GetMyEpisodeRatingRequest) GetSurface() v1.ClientSurface {
	if x != nil {
		return x.Surface
	}
	return v1.ClientSurface(0)
}

type GetMyEpisodeRatingResponse struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// 0 when this reader has not rated the episode, 1 to 5 once they have.
	Score         int32             `protobuf:"varint,1,opt,name=score,proto3" json:"score,omitempty"`
	RatingCount   int64             `protobuf:"varint,2,opt,name=rating_count,json=ratingCount,proto3" json:"rating_count,omitempty"`
	Mode          EpisodeRatingMode `protobuf:"varint,3,opt,name=mode,proto3,enum=publira.v1.EpisodeRatingMode" json:"mode,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetMyEpisodeRatingResponse) Reset() {
	*x = GetMyEpisodeRatingResponse{}
	mi := &file_publira_v1_catalog_proto_msgTypes[71]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetMyEpisodeRatingResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetMyEpisodeRatingResponse) ProtoMessage() {}

func (x *GetMyEpisodeRatingResponse) ProtoReflect() protoreflect.Message {
	mi := &file_publira_v1_catalog_proto_msgTypes[71]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetMyEpisodeRatingResponse.ProtoReflect.Descriptor instead.
func (*GetMyEpisodeRatingResponse) Descriptor() ([]byte, []int) {
	return file_publira_v1_catalog_proto_rawDescGZIP(), []int{71}
}

func (x *GetMyEpisodeRatingResponse) GetScore() int32 {
	if x != nil {
		return x.Score
	}
	return 0
}

func (x *GetMyEpisodeRatingResponse) GetRatingCount() int64 {
	if x != nil {
		return x.RatingCount
	}
	return 0
}

func (x *GetMyEpisodeRatingResponse) GetMode() EpisodeRatingMode {
	if x != nil {
		return x.Mode
	}
	return EpisodeRatingMode_EPISODE_RATING_MODE_UNSPECIFIED
}

type GetMySeriesRatingRequest struct {
	state          protoimpl.MessageState `protogen:"open.v1"`
	Tenant         *v1.TenantContext      `protobuf:"bytes,1,opt,name=tenant,proto3" json:"tenant,omitempty"`
	SeriesPublicId string                 `protobuf:"bytes,2,opt,name=series_public_id,json=seriesPublicId,proto3" json:"series_public_id,omitempty"`
	// The client making the read. Unspecified is answered as the web storefront.
	Surface       v1.ClientSurface `protobuf:"varint,3,opt,name=surface,proto3,enum=publira.types.v1.ClientSurface" json:"surface,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetMySeriesRatingRequest) Reset() {
	*x = GetMySeriesRatingRequest{}
	mi := &file_publira_v1_catalog_proto_msgTypes[72]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetMySeriesRatingRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetMySeriesRatingRequest) ProtoMessage() {}

func (x *GetMySeriesRatingRequest) ProtoReflect() protoreflect.Message {
	mi := &file_publira_v1_catalog_proto_msgTypes[72]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetMySeriesRatingRequest.ProtoReflect.Descriptor instead.
func (*GetMySeriesRatingRequest) Descriptor() ([]byte, []int) {
	return file_publira_v1_catalog_proto_rawDescGZIP(), []int{72}
}

func (x *GetMySeriesRatingRequest) GetTenant() *v1.TenantContext {
	if x != nil {
		return x.Tenant
	}
	return nil
}

func (x *GetMySeriesRatingRequest) GetSeriesPublicId() string {
	if x != nil {
		return x.SeriesPublicId
	}
	return ""
}

func (x *GetMySeriesRatingRequest) GetSurface() v1.ClientSurface {
	if x != nil {
		return x.Surface
	}
	return v1.ClientSurface(0)
}

type GetMySeriesRatingResponse struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// The mean of the scores this reader gave the series' episodes, to one
	// decimal, and 0 when they have reacted to none of them. It is the reader's
	// own statement rather than a rate over a cohort, so it is neither pulled
	// towards a mean nor held to the bottom of the scale the way
	// `publira.types.v1.Series.rating_average` is.
	RatingAverage float64 `protobuf:"fixed64,1,opt,name=rating_average,json=ratingAverage,proto3" json:"rating_average,omitempty"`
	// How many of the series' episodes this reader has reacted to, which is what
	// the figure above is the mean of.
	RatedEpisodeCount int32 `protobuf:"varint,2,opt,name=rated_episode_count,json=ratedEpisodeCount,proto3" json:"rated_episode_count,omitempty"`
	unknownFields     protoimpl.UnknownFields
	sizeCache         protoimpl.SizeCache
}

func (x *GetMySeriesRatingResponse) Reset() {
	*x = GetMySeriesRatingResponse{}
	mi := &file_publira_v1_catalog_proto_msgTypes[73]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetMySeriesRatingResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetMySeriesRatingResponse) ProtoMessage() {}

func (x *GetMySeriesRatingResponse) ProtoReflect() protoreflect.Message {
	mi := &file_publira_v1_catalog_proto_msgTypes[73]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetMySeriesRatingResponse.ProtoReflect.Descriptor instead.
func (*GetMySeriesRatingResponse) Descriptor() ([]byte, []int) {
	return file_publira_v1_catalog_proto_rawDescGZIP(), []int{73}
}

func (x *GetMySeriesRatingResponse) GetRatingAverage() float64 {
	if x != nil {
		return x.RatingAverage
	}
	return 0
}

func (x *GetMySeriesRatingResponse) GetRatedEpisodeCount() int32 {
	if x != nil {
		return x.RatedEpisodeCount
	}
	return 0
}

// A public catalog target. The public ID is interpreted according to type.
type ContentViewTarget struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Type          ContentViewTargetType  `protobuf:"varint,1,opt,name=type,proto3,enum=publira.v1.ContentViewTargetType" json:"type,omitempty"`
	PublicId      string                 `protobuf:"bytes,2,opt,name=public_id,json=publicId,proto3" json:"public_id,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ContentViewTarget) Reset() {
	*x = ContentViewTarget{}
	mi := &file_publira_v1_catalog_proto_msgTypes[74]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ContentViewTarget) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ContentViewTarget) ProtoMessage() {}

func (x *ContentViewTarget) ProtoReflect() protoreflect.Message {
	mi := &file_publira_v1_catalog_proto_msgTypes[74]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ContentViewTarget.ProtoReflect.Descriptor instead.
func (*ContentViewTarget) Descriptor() ([]byte, []int) {
	return file_publira_v1_catalog_proto_rawDescGZIP(), []int{74}
}

func (x *ContentViewTarget) GetType() ContentViewTargetType {
	if x != nil {
		return x.Type
	}
	return ContentViewTargetType_CONTENT_VIEW_TARGET_TYPE_UNSPECIFIED
}

func (x *ContentViewTarget) GetPublicId() string {
	if x != nil {
		return x.PublicId
	}
	return ""
}

// Records that a reader opened the target's detail page.
type RecordContentViewRequest struct {
	state  protoimpl.MessageState `protogen:"open.v1"`
	Tenant *v1.TenantContext      `protobuf:"bytes,1,opt,name=tenant,proto3" json:"tenant,omitempty"`
	Target *ContentViewTarget     `protobuf:"bytes,2,opt,name=target,proto3" json:"target,omitempty"`
	// The client making the call. Unspecified is answered as the web storefront.
	Surface       v1.ClientSurface `protobuf:"varint,3,opt,name=surface,proto3,enum=publira.types.v1.ClientSurface" json:"surface,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *RecordContentViewRequest) Reset() {
	*x = RecordContentViewRequest{}
	mi := &file_publira_v1_catalog_proto_msgTypes[75]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *RecordContentViewRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*RecordContentViewRequest) ProtoMessage() {}

func (x *RecordContentViewRequest) ProtoReflect() protoreflect.Message {
	mi := &file_publira_v1_catalog_proto_msgTypes[75]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use RecordContentViewRequest.ProtoReflect.Descriptor instead.
func (*RecordContentViewRequest) Descriptor() ([]byte, []int) {
	return file_publira_v1_catalog_proto_rawDescGZIP(), []int{75}
}

func (x *RecordContentViewRequest) GetTenant() *v1.TenantContext {
	if x != nil {
		return x.Tenant
	}
	return nil
}

func (x *RecordContentViewRequest) GetTarget() *ContentViewTarget {
	if x != nil {
		return x.Target
	}
	return nil
}

func (x *RecordContentViewRequest) GetSurface() v1.ClientSurface {
	if x != nil {
		return x.Surface
	}
	return v1.ClientSurface(0)
}

// Deliberately empty. The caller reports a view it already rendered, so there
// is nothing for it to learn back; whether the view was recorded, deduplicated,
// or dropped is not the reader's concern.
type RecordContentViewResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *RecordContentViewResponse) Reset() {
	*x = RecordContentViewResponse{}
	mi := &file_publira_v1_catalog_proto_msgTypes[76]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *RecordContentViewResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*RecordContentViewResponse) ProtoMessage() {}

func (x *RecordContentViewResponse) ProtoReflect() protoreflect.Message {
	mi := &file_publira_v1_catalog_proto_msgTypes[76]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use RecordContentViewResponse.ProtoReflect.Descriptor instead.
func (*RecordContentViewResponse) Descriptor() ([]byte, []int) {
	return file_publira_v1_catalog_proto_rawDescGZIP(), []int{76}
}

// A one-time paid-episode checkout. Purchases are created only by the Stripe
// webhook after a successful payment, never by the browser's return URL.
//
// `client` also names the surface the checkout is started from: CLIENT_MOBILE
// is the app, and every other value the storefront. An episode that surface
// may not show is not_found, as it is in the catalog, and one it shows but may
// not sell (Episode.purchase_availability) is failed_precondition.
type StartEpisodeCheckoutRequest struct {
	state           protoimpl.MessageState `protogen:"open.v1"`
	Tenant          *v1.TenantContext      `protobuf:"bytes,1,opt,name=tenant,proto3" json:"tenant,omitempty"`
	EpisodePublicId string                 `protobuf:"bytes,2,opt,name=episode_public_id,json=episodePublicId,proto3" json:"episode_public_id,omitempty"`
	// Where Stripe should return the reader after checkout, and the surface the
	// episode has to be sold on. Unspecified is the storefront.
	Client        StartEpisodeCheckoutRequest_Client `protobuf:"varint,3,opt,name=client,proto3,enum=publira.v1.StartEpisodeCheckoutRequest_Client" json:"client,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *StartEpisodeCheckoutRequest) Reset() {
	*x = StartEpisodeCheckoutRequest{}
	mi := &file_publira_v1_catalog_proto_msgTypes[77]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *StartEpisodeCheckoutRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*StartEpisodeCheckoutRequest) ProtoMessage() {}

func (x *StartEpisodeCheckoutRequest) ProtoReflect() protoreflect.Message {
	mi := &file_publira_v1_catalog_proto_msgTypes[77]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use StartEpisodeCheckoutRequest.ProtoReflect.Descriptor instead.
func (*StartEpisodeCheckoutRequest) Descriptor() ([]byte, []int) {
	return file_publira_v1_catalog_proto_rawDescGZIP(), []int{77}
}

func (x *StartEpisodeCheckoutRequest) GetTenant() *v1.TenantContext {
	if x != nil {
		return x.Tenant
	}
	return nil
}

func (x *StartEpisodeCheckoutRequest) GetEpisodePublicId() string {
	if x != nil {
		return x.EpisodePublicId
	}
	return ""
}

func (x *StartEpisodeCheckoutRequest) GetClient() StartEpisodeCheckoutRequest_Client {
	if x != nil {
		return x.Client
	}
	return StartEpisodeCheckoutRequest_CLIENT_UNSPECIFIED
}

type StartEpisodeCheckoutResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	CheckoutUrl   string                 `protobuf:"bytes,1,opt,name=checkout_url,json=checkoutUrl,proto3" json:"checkout_url,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *StartEpisodeCheckoutResponse) Reset() {
	*x = StartEpisodeCheckoutResponse{}
	mi := &file_publira_v1_catalog_proto_msgTypes[78]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *StartEpisodeCheckoutResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*StartEpisodeCheckoutResponse) ProtoMessage() {}

func (x *StartEpisodeCheckoutResponse) ProtoReflect() protoreflect.Message {
	mi := &file_publira_v1_catalog_proto_msgTypes[78]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use StartEpisodeCheckoutResponse.ProtoReflect.Descriptor instead.
func (*StartEpisodeCheckoutResponse) Descriptor() ([]byte, []int) {
	return file_publira_v1_catalog_proto_rawDescGZIP(), []int{78}
}

func (x *StartEpisodeCheckoutResponse) GetCheckoutUrl() string {
	if x != nil {
		return x.CheckoutUrl
	}
	return ""
}

// One purchase belonging to the signed-in reader. `is_active` is evaluated by
// the API at read time: a purchase without an expiry is always active.
type MyPurchase struct {
	state           protoimpl.MessageState `protogen:"open.v1"`
	Id              string                 `protobuf:"bytes,1,opt,name=id,proto3" json:"id,omitempty"`
	Episode         *v1.Episode            `protobuf:"bytes,2,opt,name=episode,proto3" json:"episode,omitempty"`
	Series          *v1.Series             `protobuf:"bytes,3,opt,name=series,proto3" json:"series,omitempty"`
	PriceAtPurchase int32                  `protobuf:"varint,4,opt,name=price_at_purchase,json=priceAtPurchase,proto3" json:"price_at_purchase,omitempty"`
	PurchasedAt     string                 `protobuf:"bytes,5,opt,name=purchased_at,json=purchasedAt,proto3" json:"purchased_at,omitempty"`
	// Empty when this purchase does not expire.
	ExpiresAt     string `protobuf:"bytes,6,opt,name=expires_at,json=expiresAt,proto3" json:"expires_at,omitempty"`
	IsActive      bool   `protobuf:"varint,7,opt,name=is_active,json=isActive,proto3" json:"is_active,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *MyPurchase) Reset() {
	*x = MyPurchase{}
	mi := &file_publira_v1_catalog_proto_msgTypes[79]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *MyPurchase) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*MyPurchase) ProtoMessage() {}

func (x *MyPurchase) ProtoReflect() protoreflect.Message {
	mi := &file_publira_v1_catalog_proto_msgTypes[79]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use MyPurchase.ProtoReflect.Descriptor instead.
func (*MyPurchase) Descriptor() ([]byte, []int) {
	return file_publira_v1_catalog_proto_rawDescGZIP(), []int{79}
}

func (x *MyPurchase) GetId() string {
	if x != nil {
		return x.Id
	}
	return ""
}

func (x *MyPurchase) GetEpisode() *v1.Episode {
	if x != nil {
		return x.Episode
	}
	return nil
}

func (x *MyPurchase) GetSeries() *v1.Series {
	if x != nil {
		return x.Series
	}
	return nil
}

func (x *MyPurchase) GetPriceAtPurchase() int32 {
	if x != nil {
		return x.PriceAtPurchase
	}
	return 0
}

func (x *MyPurchase) GetPurchasedAt() string {
	if x != nil {
		return x.PurchasedAt
	}
	return ""
}

func (x *MyPurchase) GetExpiresAt() string {
	if x != nil {
		return x.ExpiresAt
	}
	return ""
}

func (x *MyPurchase) GetIsActive() bool {
	if x != nil {
		return x.IsActive
	}
	return false
}

// Cursor pagination. Field shape and token rules: proto/README.md.
type ListMyPurchasesRequest struct {
	state  protoimpl.MessageState `protogen:"open.v1"`
	Tenant *v1.TenantContext      `protobuf:"bytes,1,opt,name=tenant,proto3" json:"tenant,omitempty"`
	// Max items in one page. <= 0 or > 100 falls back to 20.
	Limit int32 `protobuf:"varint,2,opt,name=limit,proto3" json:"limit,omitempty"`
	// Opaque token from a previous response. Empty for the first page.
	Token string `protobuf:"bytes,3,opt,name=token,proto3" json:"token,omitempty"`
	// The client making the read. Unspecified is answered as the web storefront.
	Surface       v1.ClientSurface `protobuf:"varint,4,opt,name=surface,proto3,enum=publira.types.v1.ClientSurface" json:"surface,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ListMyPurchasesRequest) Reset() {
	*x = ListMyPurchasesRequest{}
	mi := &file_publira_v1_catalog_proto_msgTypes[80]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ListMyPurchasesRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ListMyPurchasesRequest) ProtoMessage() {}

func (x *ListMyPurchasesRequest) ProtoReflect() protoreflect.Message {
	mi := &file_publira_v1_catalog_proto_msgTypes[80]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ListMyPurchasesRequest.ProtoReflect.Descriptor instead.
func (*ListMyPurchasesRequest) Descriptor() ([]byte, []int) {
	return file_publira_v1_catalog_proto_rawDescGZIP(), []int{80}
}

func (x *ListMyPurchasesRequest) GetTenant() *v1.TenantContext {
	if x != nil {
		return x.Tenant
	}
	return nil
}

func (x *ListMyPurchasesRequest) GetLimit() int32 {
	if x != nil {
		return x.Limit
	}
	return 0
}

func (x *ListMyPurchasesRequest) GetToken() string {
	if x != nil {
		return x.Token
	}
	return ""
}

func (x *ListMyPurchasesRequest) GetSurface() v1.ClientSurface {
	if x != nil {
		return x.Surface
	}
	return v1.ClientSurface(0)
}

type ListMyPurchasesResponse struct {
	state     protoimpl.MessageState `protogen:"open.v1"`
	Purchases []*MyPurchase          `protobuf:"bytes,1,rep,name=purchases,proto3" json:"purchases,omitempty"`
	// Token for the previous page. Empty on the first page.
	PreviousToken string `protobuf:"bytes,2,opt,name=previous_token,json=previousToken,proto3" json:"previous_token,omitempty"`
	// Token for the next page. Empty on the last page.
	NextToken     string `protobuf:"bytes,3,opt,name=next_token,json=nextToken,proto3" json:"next_token,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ListMyPurchasesResponse) Reset() {
	*x = ListMyPurchasesResponse{}
	mi := &file_publira_v1_catalog_proto_msgTypes[81]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ListMyPurchasesResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ListMyPurchasesResponse) ProtoMessage() {}

func (x *ListMyPurchasesResponse) ProtoReflect() protoreflect.Message {
	mi := &file_publira_v1_catalog_proto_msgTypes[81]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ListMyPurchasesResponse.ProtoReflect.Descriptor instead.
func (*ListMyPurchasesResponse) Descriptor() ([]byte, []int) {
	return file_publira_v1_catalog_proto_rawDescGZIP(), []int{81}
}

func (x *ListMyPurchasesResponse) GetPurchases() []*MyPurchase {
	if x != nil {
		return x.Purchases
	}
	return nil
}

func (x *ListMyPurchasesResponse) GetPreviousToken() string {
	if x != nil {
		return x.PreviousToken
	}
	return ""
}

func (x *ListMyPurchasesResponse) GetNextToken() string {
	if x != nil {
		return x.NextToken
	}
	return ""
}

// `web-host` forwards a payment provider's raw request body and headers to this
// RPC. Signature verification and all purchase validation stay on the API
// server, which reads the signature from the header the provider declares.
type ProcessPaymentWebhookRequest struct {
	state  protoimpl.MessageState `protogen:"open.v1"`
	Tenant *v1.TenantContext      `protobuf:"bytes,1,opt,name=tenant,proto3" json:"tenant,omitempty"`
	// The id of the provider the notification came from, such as `stripe`.
	Provider string `protobuf:"bytes,2,opt,name=provider,proto3" json:"provider,omitempty"`
	Payload  []byte `protobuf:"bytes,3,opt,name=payload,proto3" json:"payload,omitempty"`
	// The request headers, keyed by name. Names are matched case-insensitively.
	Headers       map[string]string `protobuf:"bytes,4,rep,name=headers,proto3" json:"headers,omitempty" protobuf_key:"bytes,1,opt,name=key" protobuf_val:"bytes,2,opt,name=value"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ProcessPaymentWebhookRequest) Reset() {
	*x = ProcessPaymentWebhookRequest{}
	mi := &file_publira_v1_catalog_proto_msgTypes[82]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ProcessPaymentWebhookRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ProcessPaymentWebhookRequest) ProtoMessage() {}

func (x *ProcessPaymentWebhookRequest) ProtoReflect() protoreflect.Message {
	mi := &file_publira_v1_catalog_proto_msgTypes[82]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ProcessPaymentWebhookRequest.ProtoReflect.Descriptor instead.
func (*ProcessPaymentWebhookRequest) Descriptor() ([]byte, []int) {
	return file_publira_v1_catalog_proto_rawDescGZIP(), []int{82}
}

func (x *ProcessPaymentWebhookRequest) GetTenant() *v1.TenantContext {
	if x != nil {
		return x.Tenant
	}
	return nil
}

func (x *ProcessPaymentWebhookRequest) GetProvider() string {
	if x != nil {
		return x.Provider
	}
	return ""
}

func (x *ProcessPaymentWebhookRequest) GetPayload() []byte {
	if x != nil {
		return x.Payload
	}
	return nil
}

func (x *ProcessPaymentWebhookRequest) GetHeaders() map[string]string {
	if x != nil {
		return x.Headers
	}
	return nil
}

type ProcessPaymentWebhookResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ProcessPaymentWebhookResponse) Reset() {
	*x = ProcessPaymentWebhookResponse{}
	mi := &file_publira_v1_catalog_proto_msgTypes[83]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ProcessPaymentWebhookResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ProcessPaymentWebhookResponse) ProtoMessage() {}

func (x *ProcessPaymentWebhookResponse) ProtoReflect() protoreflect.Message {
	mi := &file_publira_v1_catalog_proto_msgTypes[83]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ProcessPaymentWebhookResponse.ProtoReflect.Descriptor instead.
func (*ProcessPaymentWebhookResponse) Descriptor() ([]byte, []int) {
	return file_publira_v1_catalog_proto_rawDescGZIP(), []int{83}
}

// Opens a purchase through the App Store's or Google Play's in-app purchase,
// for a tenant whose app sells through the store. The app buys `product_id`
// with `intent_id` as the transaction's account token (`appAccountToken` on
// iOS, `obfuscatedAccountId` on Android), which is how ConfirmStorePurchase
// learns which episode the transaction pays for. The intent keeps the price and
// the reading period the episode is sold with at this moment, and the purchase
// is recorded on those terms.
//
// Signed-in readers only. An episode the app may not show is not_found; one it
// may not sell, a free one, a tenant whose app does not sell through the
// store, and a `store` that is not ready are failed_precondition, and an
// episode the reader already holds is already_exists. Asking again for the
// same episode at the same price answers the same intent.
type StartStorePurchaseRequest struct {
	state           protoimpl.MessageState `protogen:"open.v1"`
	Tenant          *v1.TenantContext      `protobuf:"bytes,1,opt,name=tenant,proto3" json:"tenant,omitempty"`
	EpisodePublicId string                 `protobuf:"bytes,2,opt,name=episode_public_id,json=episodePublicId,proto3" json:"episode_public_id,omitempty"`
	// The store the app is about to charge through. Required.
	Store         InAppPurchaseStore `protobuf:"varint,3,opt,name=store,proto3,enum=publira.v1.InAppPurchaseStore" json:"store,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *StartStorePurchaseRequest) Reset() {
	*x = StartStorePurchaseRequest{}
	mi := &file_publira_v1_catalog_proto_msgTypes[84]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *StartStorePurchaseRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*StartStorePurchaseRequest) ProtoMessage() {}

func (x *StartStorePurchaseRequest) ProtoReflect() protoreflect.Message {
	mi := &file_publira_v1_catalog_proto_msgTypes[84]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use StartStorePurchaseRequest.ProtoReflect.Descriptor instead.
func (*StartStorePurchaseRequest) Descriptor() ([]byte, []int) {
	return file_publira_v1_catalog_proto_rawDescGZIP(), []int{84}
}

func (x *StartStorePurchaseRequest) GetTenant() *v1.TenantContext {
	if x != nil {
		return x.Tenant
	}
	return nil
}

func (x *StartStorePurchaseRequest) GetEpisodePublicId() string {
	if x != nil {
		return x.EpisodePublicId
	}
	return ""
}

func (x *StartStorePurchaseRequest) GetStore() InAppPurchaseStore {
	if x != nil {
		return x.Store
	}
	return InAppPurchaseStore_IN_APP_PURCHASE_STORE_UNSPECIFIED
}

type StartStorePurchaseResponse struct {
	state    protoimpl.MessageState `protogen:"open.v1"`
	IntentId string                 `protobuf:"bytes,1,opt,name=intent_id,json=intentId,proto3" json:"intent_id,omitempty"`
	// The store product the episode's price is sold as, such as `episode_300`.
	ProductId     string `protobuf:"bytes,2,opt,name=product_id,json=productId,proto3" json:"product_id,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *StartStorePurchaseResponse) Reset() {
	*x = StartStorePurchaseResponse{}
	mi := &file_publira_v1_catalog_proto_msgTypes[85]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *StartStorePurchaseResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*StartStorePurchaseResponse) ProtoMessage() {}

func (x *StartStorePurchaseResponse) ProtoReflect() protoreflect.Message {
	mi := &file_publira_v1_catalog_proto_msgTypes[85]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use StartStorePurchaseResponse.ProtoReflect.Descriptor instead.
func (*StartStorePurchaseResponse) Descriptor() ([]byte, []int) {
	return file_publira_v1_catalog_proto_rawDescGZIP(), []int{85}
}

func (x *StartStorePurchaseResponse) GetIntentId() string {
	if x != nil {
		return x.IntentId
	}
	return ""
}

func (x *StartStorePurchaseResponse) GetProductId() string {
	if x != nil {
		return x.ProductId
	}
	return ""
}

// Hands a transaction the store charged to the server, which verifies it with
// the store and records it as a purchase.
//
// The transaction's account token has to name an open intent the caller
// opened, for the product the transaction bought, in the tenant's own app:
// anything else is invalid_argument, or permission_denied for another reader's
// intent, and creates nothing. A transaction the store has not settled (Ask to
// Buy, a Play pending purchase) is failed_precondition, and a store the server
// cannot reach is unavailable; the app confirms again later either way.
// Confirming a recorded transaction again answers the same purchase. Each
// call spends the reader's allowance for store confirmations, and a reader past
// it is resource_exhausted before the store is asked.
type ConfirmStorePurchaseRequest struct {
	state  protoimpl.MessageState `protogen:"open.v1"`
	Tenant *v1.TenantContext      `protobuf:"bytes,1,opt,name=tenant,proto3" json:"tenant,omitempty"`
	Store  InAppPurchaseStore     `protobuf:"varint,2,opt,name=store,proto3,enum=publira.v1.InAppPurchaseStore" json:"store,omitempty"`
	// IN_APP_PURCHASE_STORE_APP_STORE: the JWS signed transaction StoreKit 2
	// reports. IN_APP_PURCHASE_STORE_GOOGLE_PLAY: the purchase token.
	Transaction string `protobuf:"bytes,3,opt,name=transaction,proto3" json:"transaction,omitempty"`
	// IN_APP_PURCHASE_STORE_GOOGLE_PLAY: the product the token was bought as,
	// which the Google Play Developer API looks a purchase up by. Unused for the
	// App Store, whose signed transaction names its product.
	ProductId     string `protobuf:"bytes,4,opt,name=product_id,json=productId,proto3" json:"product_id,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ConfirmStorePurchaseRequest) Reset() {
	*x = ConfirmStorePurchaseRequest{}
	mi := &file_publira_v1_catalog_proto_msgTypes[86]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ConfirmStorePurchaseRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ConfirmStorePurchaseRequest) ProtoMessage() {}

func (x *ConfirmStorePurchaseRequest) ProtoReflect() protoreflect.Message {
	mi := &file_publira_v1_catalog_proto_msgTypes[86]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ConfirmStorePurchaseRequest.ProtoReflect.Descriptor instead.
func (*ConfirmStorePurchaseRequest) Descriptor() ([]byte, []int) {
	return file_publira_v1_catalog_proto_rawDescGZIP(), []int{86}
}

func (x *ConfirmStorePurchaseRequest) GetTenant() *v1.TenantContext {
	if x != nil {
		return x.Tenant
	}
	return nil
}

func (x *ConfirmStorePurchaseRequest) GetStore() InAppPurchaseStore {
	if x != nil {
		return x.Store
	}
	return InAppPurchaseStore_IN_APP_PURCHASE_STORE_UNSPECIFIED
}

func (x *ConfirmStorePurchaseRequest) GetTransaction() string {
	if x != nil {
		return x.Transaction
	}
	return ""
}

func (x *ConfirmStorePurchaseRequest) GetProductId() string {
	if x != nil {
		return x.ProductId
	}
	return ""
}

type ConfirmStorePurchaseResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Purchase      *MyPurchase            `protobuf:"bytes,1,opt,name=purchase,proto3" json:"purchase,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ConfirmStorePurchaseResponse) Reset() {
	*x = ConfirmStorePurchaseResponse{}
	mi := &file_publira_v1_catalog_proto_msgTypes[87]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ConfirmStorePurchaseResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ConfirmStorePurchaseResponse) ProtoMessage() {}

func (x *ConfirmStorePurchaseResponse) ProtoReflect() protoreflect.Message {
	mi := &file_publira_v1_catalog_proto_msgTypes[87]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ConfirmStorePurchaseResponse.ProtoReflect.Descriptor instead.
func (*ConfirmStorePurchaseResponse) Descriptor() ([]byte, []int) {
	return file_publira_v1_catalog_proto_rawDescGZIP(), []int{87}
}

func (x *ConfirmStorePurchaseResponse) GetPurchase() *MyPurchase {
	if x != nil {
		return x.Purchase
	}
	return nil
}

// `web-host` forwards the raw body of an App Store Server Notifications V2
// request to this RPC. The signed payload is verified against Apple's root
// certificate here, and it has to name the tenant's own iOS app.
//
// REFUND and REVOKE take back the purchase of the transaction they carry,
// holding the refund until the purchase is recorded when the app has not
// confirmed it yet; every other type is acknowledged and changes nothing. A
// payload that does not verify, or names another app, is invalid_argument, and
// a tenant with no iOS app is failed_precondition.
type ProcessAppStoreNotificationRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Tenant        *v1.TenantContext      `protobuf:"bytes,1,opt,name=tenant,proto3" json:"tenant,omitempty"`
	Payload       []byte                 `protobuf:"bytes,2,opt,name=payload,proto3" json:"payload,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ProcessAppStoreNotificationRequest) Reset() {
	*x = ProcessAppStoreNotificationRequest{}
	mi := &file_publira_v1_catalog_proto_msgTypes[88]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ProcessAppStoreNotificationRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ProcessAppStoreNotificationRequest) ProtoMessage() {}

func (x *ProcessAppStoreNotificationRequest) ProtoReflect() protoreflect.Message {
	mi := &file_publira_v1_catalog_proto_msgTypes[88]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ProcessAppStoreNotificationRequest.ProtoReflect.Descriptor instead.
func (*ProcessAppStoreNotificationRequest) Descriptor() ([]byte, []int) {
	return file_publira_v1_catalog_proto_rawDescGZIP(), []int{88}
}

func (x *ProcessAppStoreNotificationRequest) GetTenant() *v1.TenantContext {
	if x != nil {
		return x.Tenant
	}
	return nil
}

func (x *ProcessAppStoreNotificationRequest) GetPayload() []byte {
	if x != nil {
		return x.Payload
	}
	return nil
}

type ProcessAppStoreNotificationResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ProcessAppStoreNotificationResponse) Reset() {
	*x = ProcessAppStoreNotificationResponse{}
	mi := &file_publira_v1_catalog_proto_msgTypes[89]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ProcessAppStoreNotificationResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ProcessAppStoreNotificationResponse) ProtoMessage() {}

func (x *ProcessAppStoreNotificationResponse) ProtoReflect() protoreflect.Message {
	mi := &file_publira_v1_catalog_proto_msgTypes[89]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ProcessAppStoreNotificationResponse.ProtoReflect.Descriptor instead.
func (*ProcessAppStoreNotificationResponse) Descriptor() ([]byte, []int) {
	return file_publira_v1_catalog_proto_rawDescGZIP(), []int{89}
}

var File_publira_v1_catalog_proto protoreflect.FileDescriptor

const file_publira_v1_catalog_proto_rawDesc = "" +
	"\n" +
	"\x18publira/v1/catalog.proto\x12\n" +
	"publira.v1\x1a\x1cpublira/types/v1/types.proto\"\xca\x01\n" +
	"\x1aListPublishedLabelsRequest\x127\n" +
	"\x06tenant\x18\x01 \x01(\v2\x1f.publira.types.v1.TenantContextR\x06tenant\x12\x14\n" +
	"\x05limit\x18\x02 \x01(\x05R\x05limit\x12\x14\n" +
	"\x05token\x18\x04 \x01(\tR\x05token\x129\n" +
	"\asurface\x18\x05 \x01(\x0e2\x1f.publira.types.v1.ClientSurfaceR\asurfaceJ\x04\b\x03\x10\x04R\x06offset\"\x94\x01\n" +
	"\x1bListPublishedLabelsResponse\x12/\n" +
	"\x06labels\x18\x01 \x03(\v2\x17.publira.types.v1.LabelR\x06labels\x12%\n" +
	"\x0eprevious_token\x18\x02 \x01(\tR\rpreviousToken\x12\x1d\n" +
	"\n" +
	"next_token\x18\x03 \x01(\tR\tnextToken\"\xcb\x03\n" +
	"\x1aListPublishedSeriesRequest\x127\n" +
	"\x06tenant\x18\x01 \x01(\v2\x1f.publira.types.v1.TenantContextR\x06tenant\x12\x14\n" +
	"\x05limit\x18\x02 \x01(\x05R\x05limit\x12\x14\n" +
	"\x05token\x18\x04 \x01(\tR\x05token\x12-\n" +
	"\x05order\x18\x05 \x01(\x0e2\x17.publira.v1.SeriesOrderR\x05order\x12*\n" +
	"\x11has_free_episodes\x18\x06 \x01(\bR\x0fhasFreeEpisodes\x12&\n" +
	"\x0fgenre_public_id\x18\a \x01(\tR\rgenrePublicId\x12\x19\n" +
	"\btag_slug\x18\b \x01(\tR\atagSlug\x126\n" +
	"\x06status\x18\t \x01(\x0e2\x1e.publira.types.v1.SeriesStatusR\x06status\x12\x1d\n" +
	"\aweekday\x18\n" +
	" \x01(\x05H\x00R\aweekday\x88\x01\x01\x129\n" +
	"\asurface\x18\v \x01(\x0e2\x1f.publira.types.v1.ClientSurfaceR\asurfaceB\n" +
	"\n" +
	"\b_weekdayJ\x04\b\x03\x10\x04R\x06offset\"\x95\x01\n" +
	"\x1bListPublishedSeriesResponse\x120\n" +
	"\x06series\x18\x01 \x03(\v2\x18.publira.types.v1.SeriesR\x06series\x12%\n" +
	"\x0eprevious_token\x18\x02 \x01(\tR\rpreviousToken\x12\x1d\n" +
	"\n" +
	"next_token\x18\x03 \x01(\tR\tnextToken\"\xa9\x01\n" +
	"\x16GetSeriesDetailRequest\x127\n" +
	"\x06tenant\x18\x01 \x01(\v2\x1f.publira.types.v1.TenantContextR\x06tenant\x12\x1b\n" +
	"\tpublic_id\x18\x02 \x01(\tR\bpublicId\x129\n" +
	"\asurface\x18\x03 \x01(\x0e2\x1f.publira.types.v1.ClientSurfaceR\asurface\"\xf6\x01\n" +
	"\x17GetSeriesDetailResponse\x120\n" +
	"\x06series\x18\x01 \x01(\v2\x18.publira.types.v1.SeriesR\x06series\x125\n" +
	"\bepisodes\x18\x02 \x03(\v2\x19.publira.types.v1.EpisodeR\bepisodes\x120\n" +
	"\x14required_minimum_age\x18\x03 \x01(\x05R\x12requiredMinimumAge\x12@\n" +
	"\fcomment_mode\x18\x04 \x01(\x0e2\x1d.publira.types.v1.CommentModeR\vcommentMode\"\xbd\x01\n" +
	"\x1dGetSeriesEpisodeAccessRequest\x127\n" +
	"\x06tenant\x18\x01 \x01(\v2\x1f.publira.types.v1.TenantContextR\x06tenant\x12(\n" +
	"\x10series_public_id\x18\x02 \x01(\tR\x0eseriesPublicId\x129\n" +
	"\asurface\x18\x03 \x01(\x0e2\x1f.publira.types.v1.ClientSurfaceR\asurface\"t\n" +
	"\x13SeriesEpisodeAccess\x12*\n" +
	"\x11episode_public_id\x18\x01 \x01(\tR\x0fepisodePublicId\x121\n" +
	"\x06access\x18\x02 \x01(\x0e2\x19.publira.v1.EpisodeAccessR\x06access\"]\n" +
	"\x1eGetSeriesEpisodeAccessResponse\x12;\n" +
	"\bepisodes\x18\x01 \x03(\v2\x1f.publira.v1.SeriesEpisodeAccessR\bepisodes\"\xaa\x01\n" +
	"\x17GetEpisodeDetailRequest\x127\n" +
	"\x06tenant\x18\x01 \x01(\v2\x1f.publira.types.v1.TenantContextR\x06tenant\x12\x1b\n" +
	"\tpublic_id\x18\x02 \x01(\tR\bpublicId\x129\n" +
	"\asurface\x18\x03 \x01(\x0e2\x1f.publira.types.v1.ClientSurfaceR\asurface\"\xa7\x02\n" +
	"\x0fEpisodeNeighbor\x12\x1b\n" +
	"\tpublic_id\x18\x01 \x01(\tR\bpublicId\x12\x14\n" +
	"\x05title\x18\x02 \x01(\tR\x05title\x12\x1f\n" +
	"\vorder_index\x18\x03 \x01(\x05R\n" +
	"orderIndex\x12\x14\n" +
	"\x05price\x18\x04 \x01(\x05R\x05price\x12\x17\n" +
	"\ais_free\x18\x05 \x01(\bR\x06isFree\x125\n" +
	"\bcreators\x18\x06 \x03(\v2\x19.publira.types.v1.CreatorR\bcreators\x12Z\n" +
	"\x15purchase_availability\x18\a \x01(\x0e2%.publira.types.v1.SurfaceAvailabilityR\x14purchaseAvailability\"\x93\x03\n" +
	"\x18GetEpisodeDetailResponse\x123\n" +
	"\aepisode\x18\x01 \x01(\v2\x19.publira.types.v1.EpisodeR\aepisode\x120\n" +
	"\x06series\x18\x02 \x01(\v2\x18.publira.types.v1.SeriesR\x06series\x126\n" +
	"\x06images\x18\x03 \x03(\v2\x1e.publira.types.v1.EpisodeImageR\x06images\x121\n" +
	"\x06access\x18\x04 \x01(\x0e2\x19.publira.v1.EpisodeAccessR\x06access\x12\x1d\n" +
	"\n" +
	"free_until\x18\x05 \x01(\tR\tfreeUntil\x12F\n" +
	"\x10previous_episode\x18\x06 \x01(\v2\x1b.publira.v1.EpisodeNeighborR\x0fpreviousEpisode\x12>\n" +
	"\fnext_episode\x18\a \x01(\v2\x1b.publira.v1.EpisodeNeighborR\vnextEpisode\"\xb1\x02\n" +
	"\x10PublishedCreator\x12\x1b\n" +
	"\tpublic_id\x18\x01 \x01(\tR\bpublicId\x12\x12\n" +
	"\x04name\x18\x02 \x01(\tR\x04name\x12!\n" +
	"\fprofile_text\x18\x03 \x01(\tR\vprofileText\x12$\n" +
	"\x0eicon_image_url\x18\x04 \x01(\tR\ficonImageUrl\x12:\n" +
	"\x1aicon_image_file_size_bytes\x18\x05 \x01(\x03R\x16iconImageFileSizeBytes\x121\n" +
	"\x15icon_image_updated_at\x18\x06 \x01(\tR\x12iconImageUpdatedAt\x124\n" +
	"\x16published_series_count\x18\a \x01(\x05R\x14publishedSeriesCount\"\xbe\x01\n" +
	"\x1cListPublishedCreatorsRequest\x127\n" +
	"\x06tenant\x18\x01 \x01(\v2\x1f.publira.types.v1.TenantContextR\x06tenant\x12\x14\n" +
	"\x05limit\x18\x02 \x01(\x05R\x05limit\x12\x14\n" +
	"\x05token\x18\x03 \x01(\tR\x05token\x129\n" +
	"\asurface\x18\x04 \x01(\x0e2\x1f.publira.types.v1.ClientSurfaceR\asurface\"\x9f\x01\n" +
	"\x1dListPublishedCreatorsResponse\x128\n" +
	"\bcreators\x18\x01 \x03(\v2\x1c.publira.v1.PublishedCreatorR\bcreators\x12%\n" +
	"\x0eprevious_token\x18\x02 \x01(\tR\rpreviousToken\x12\x1d\n" +
	"\n" +
	"next_token\x18\x03 \x01(\tR\tnextToken\"\xdf\x01\n" +
	" GetPublishedCreatorDetailRequest\x127\n" +
	"\x06tenant\x18\x01 \x01(\v2\x1f.publira.types.v1.TenantContextR\x06tenant\x12\x1b\n" +
	"\tpublic_id\x18\x02 \x01(\tR\bpublicId\x12\x14\n" +
	"\x05limit\x18\x03 \x01(\x05R\x05limit\x12\x14\n" +
	"\x05token\x18\x04 \x01(\tR\x05token\x129\n" +
	"\asurface\x18\x05 \x01(\x0e2\x1f.publira.types.v1.ClientSurfaceR\asurface\"\xd3\x01\n" +
	"!GetPublishedCreatorDetailResponse\x126\n" +
	"\acreator\x18\x01 \x01(\v2\x1c.publira.v1.PublishedCreatorR\acreator\x120\n" +
	"\x06series\x18\x02 \x03(\v2\x18.publira.types.v1.SeriesR\x06series\x12%\n" +
	"\x0eprevious_token\x18\x03 \x01(\tR\rpreviousToken\x12\x1d\n" +
	"\n" +
	"next_token\x18\x04 \x01(\tR\tnextToken\"\x95\x02\n" +
	"\x0ePublishedLabel\x12\x1b\n" +
	"\tpublic_id\x18\x01 \x01(\tR\bpublicId\x12\x12\n" +
	"\x04name\x18\x02 \x01(\tR\x04name\x12:\n" +
	"\x1aeye_catch_image_updated_at\x18\x03 \x01(\tR\x16eyeCatchImageUpdatedAt\x12`\n" +
	"\x18eye_catch_image_variants\x18\x04 \x03(\v2'.publira.types.v1.SeriesEyeCatchVariantR\x15eyeCatchImageVariants\x124\n" +
	"\x16published_series_count\x18\x05 \x01(\x05R\x14publishedSeriesCount\"\xdd\x01\n" +
	"\x1eGetPublishedLabelDetailRequest\x127\n" +
	"\x06tenant\x18\x01 \x01(\v2\x1f.publira.types.v1.TenantContextR\x06tenant\x12\x1b\n" +
	"\tpublic_id\x18\x02 \x01(\tR\bpublicId\x12\x14\n" +
	"\x05limit\x18\x03 \x01(\x05R\x05limit\x12\x14\n" +
	"\x05token\x18\x04 \x01(\tR\x05token\x129\n" +
	"\asurface\x18\x05 \x01(\x0e2\x1f.publira.types.v1.ClientSurfaceR\asurface\"\xcb\x01\n" +
	"\x1fGetPublishedLabelDetailResponse\x120\n" +
	"\x05label\x18\x01 \x01(\v2\x1a.publira.v1.PublishedLabelR\x05label\x120\n" +
	"\x06series\x18\x02 \x03(\v2\x18.publira.types.v1.SeriesR\x06series\x12%\n" +
	"\x0eprevious_token\x18\x03 \x01(\tR\rpreviousToken\x12\x1d\n" +
	"\n" +
	"next_token\x18\x04 \x01(\tR\tnextToken\"\x8b\x01\n" +
	"\x0ePublishedGenre\x12\x1b\n" +
	"\tpublic_id\x18\x01 \x01(\tR\bpublicId\x12\x12\n" +
	"\x04name\x18\x02 \x01(\tR\x04name\x12\x12\n" +
	"\x04slug\x18\x03 \x01(\tR\x04slug\x124\n" +
	"\x16published_series_count\x18\x04 \x01(\x05R\x14publishedSeriesCount\"\xbc\x01\n" +
	"\x1aListPublishedGenresRequest\x127\n" +
	"\x06tenant\x18\x01 \x01(\v2\x1f.publira.types.v1.TenantContextR\x06tenant\x12\x14\n" +
	"\x05limit\x18\x02 \x01(\x05R\x05limit\x12\x14\n" +
	"\x05token\x18\x03 \x01(\tR\x05token\x129\n" +
	"\asurface\x18\x04 \x01(\x0e2\x1f.publira.types.v1.ClientSurfaceR\asurface\"\x97\x01\n" +
	"\x1bListPublishedGenresResponse\x122\n" +
	"\x06genres\x18\x01 \x03(\v2\x1a.publira.v1.PublishedGenreR\x06genres\x12%\n" +
	"\x0eprevious_token\x18\x02 \x01(\tR\rpreviousToken\x12\x1d\n" +
	"\n" +
	"next_token\x18\x03 \x01(\tR\tnextToken\"l\n" +
	"\fPublishedTag\x12\x12\n" +
	"\x04name\x18\x01 \x01(\tR\x04name\x12\x12\n" +
	"\x04slug\x18\x02 \x01(\tR\x04slug\x124\n" +
	"\x16published_series_count\x18\x03 \x01(\x05R\x14publishedSeriesCount\"\xba\x01\n" +
	"\x18ListPublishedTagsRequest\x127\n" +
	"\x06tenant\x18\x01 \x01(\v2\x1f.publira.types.v1.TenantContextR\x06tenant\x12\x14\n" +
	"\x05limit\x18\x02 \x01(\x05R\x05limit\x12\x14\n" +
	"\x05token\x18\x03 \x01(\tR\x05token\x129\n" +
	"\asurface\x18\x04 \x01(\x0e2\x1f.publira.types.v1.ClientSurfaceR\asurface\"\x8f\x01\n" +
	"\x19ListPublishedTagsResponse\x12,\n" +
	"\x04tags\x18\x01 \x03(\v2\x18.publira.v1.PublishedTagR\x04tags\x12%\n" +
	"\x0eprevious_token\x18\x02 \x01(\tR\rpreviousToken\x12\x1d\n" +
	"\n" +
	"next_token\x18\x03 \x01(\tR\tnextToken\"\xd4\x01\n" +
	"\x1cSearchPublishedSeriesRequest\x127\n" +
	"\x06tenant\x18\x01 \x01(\v2\x1f.publira.types.v1.TenantContextR\x06tenant\x12\x14\n" +
	"\x05query\x18\x02 \x01(\tR\x05query\x12\x14\n" +
	"\x05limit\x18\x03 \x01(\x05R\x05limit\x12\x14\n" +
	"\x05token\x18\x04 \x01(\tR\x05token\x129\n" +
	"\asurface\x18\x05 \x01(\x0e2\x1f.publira.types.v1.ClientSurfaceR\asurface\"\x97\x01\n" +
	"\x1dSearchPublishedSeriesResponse\x120\n" +
	"\x06series\x18\x01 \x03(\v2\x18.publira.types.v1.SeriesR\x06series\x12%\n" +
	"\x0eprevious_token\x18\x02 \x01(\tR\rpreviousToken\x12\x1d\n" +
	"\n" +
	"next_token\x18\x03 \x01(\tR\tnextToken\"\xd6\x01\n" +
	"\x1eSearchPublishedCreatorsRequest\x127\n" +
	"\x06tenant\x18\x01 \x01(\v2\x1f.publira.types.v1.TenantContextR\x06tenant\x12\x14\n" +
	"\x05query\x18\x02 \x01(\tR\x05query\x12\x14\n" +
	"\x05limit\x18\x03 \x01(\x05R\x05limit\x12\x14\n" +
	"\x05token\x18\x04 \x01(\tR\x05token\x129\n" +
	"\asurface\x18\x05 \x01(\x0e2\x1f.publira.types.v1.ClientSurfaceR\asurface\"\xa1\x01\n" +
	"\x1fSearchPublishedCreatorsResponse\x128\n" +
	"\bcreators\x18\x01 \x03(\v2\x1c.publira.v1.PublishedCreatorR\bcreators\x12%\n" +
	"\x0eprevious_token\x18\x02 \x01(\tR\rpreviousToken\x12\x1d\n" +
	"\n" +
	"next_token\x18\x03 \x01(\tR\tnextToken\"\xd4\x01\n" +
	"\x1cSearchPublishedLabelsRequest\x127\n" +
	"\x06tenant\x18\x01 \x01(\v2\x1f.publira.types.v1.TenantContextR\x06tenant\x12\x14\n" +
	"\x05query\x18\x02 \x01(\tR\x05query\x12\x14\n" +
	"\x05limit\x18\x03 \x01(\x05R\x05limit\x12\x14\n" +
	"\x05token\x18\x04 \x01(\tR\x05token\x129\n" +
	"\asurface\x18\x05 \x01(\x0e2\x1f.publira.types.v1.ClientSurfaceR\asurface\"\x96\x01\n" +
	"\x1dSearchPublishedLabelsResponse\x12/\n" +
	"\x06labels\x18\x01 \x03(\v2\x17.publira.types.v1.LabelR\x06labels\x12%\n" +
	"\x0eprevious_token\x18\x02 \x01(\tR\rpreviousToken\x12\x1d\n" +
	"\n" +
	"next_token\x18\x03 \x01(\tR\tnextToken\"\xbe\x01\n" +
	"\x1cListRecommendedSeriesRequest\x127\n" +
	"\x06tenant\x18\x01 \x01(\v2\x1f.publira.types.v1.TenantContextR\x06tenant\x12\x14\n" +
	"\x05limit\x18\x02 \x01(\x05R\x05limit\x12\x14\n" +
	"\x05token\x18\x03 \x01(\tR\x05token\x129\n" +
	"\asurface\x18\x04 \x01(\x0e2\x1f.publira.types.v1.ClientSurfaceR\asurface\"\xd1\x01\n" +
	"\x1dListRecommendedSeriesResponse\x120\n" +
	"\x06series\x18\x01 \x03(\v2\x18.publira.types.v1.SeriesR\x06series\x12%\n" +
	"\x0eprevious_token\x18\x02 \x01(\tR\rpreviousToken\x12\x1d\n" +
	"\n" +
	"next_token\x18\x03 \x01(\tR\tnextToken\x128\n" +
	"\x06source\x18\x04 \x01(\x0e2 .publira.v1.RecommendationSourceR\x06source\"\x90\x01\n" +
	"\fRankedSeries\x12\x12\n" +
	"\x04rank\x18\x01 \x01(\x05R\x04rank\x120\n" +
	"\x06series\x18\x02 \x01(\v2\x18.publira.types.v1.SeriesR\x06series\x12(\n" +
	"\rprevious_rank\x18\x03 \x01(\x05H\x00R\fpreviousRank\x88\x01\x01B\x10\n" +
	"\x0e_previous_rank\"\xec\x01\n" +
	"\x17ListRankedSeriesRequest\x127\n" +
	"\x06tenant\x18\x01 \x01(\v2\x1f.publira.types.v1.TenantContextR\x06tenant\x121\n" +
	"\x06period\x18\x02 \x01(\x0e2\x19.publira.v1.RankingPeriodR\x06period\x12\x14\n" +
	"\x05limit\x18\x03 \x01(\x05R\x05limit\x12\x14\n" +
	"\x05token\x18\x04 \x01(\tR\x05token\x129\n" +
	"\asurface\x18\x05 \x01(\x0e2\x1f.publira.types.v1.ClientSurfaceR\asurface\"\x82\x02\n" +
	"\x18ListRankedSeriesResponse\x12=\n" +
	"\rranked_series\x18\x01 \x03(\v2\x18.publira.v1.RankedSeriesR\frankedSeries\x12%\n" +
	"\x0eprevious_token\x18\x02 \x01(\tR\rpreviousToken\x12\x1d\n" +
	"\n" +
	"next_token\x18\x03 \x01(\tR\tnextToken\x12\x1f\n" +
	"\vcomputed_at\x18\x04 \x01(\tR\n" +
	"computedAt\x12!\n" +
	"\fperiod_start\x18\x05 \x01(\tR\vperiodStart\x12\x1d\n" +
	"\n" +
	"period_end\x18\x06 \x01(\tR\tperiodEnd\"\xe4\x01\n" +
	"\x18ListRelatedSeriesRequest\x127\n" +
	"\x06tenant\x18\x01 \x01(\v2\x1f.publira.types.v1.TenantContextR\x06tenant\x12(\n" +
	"\x10series_public_id\x18\x02 \x01(\tR\x0eseriesPublicId\x12\x14\n" +
	"\x05limit\x18\x03 \x01(\x05R\x05limit\x12\x14\n" +
	"\x05token\x18\x04 \x01(\tR\x05token\x129\n" +
	"\asurface\x18\x05 \x01(\x0e2\x1f.publira.types.v1.ClientSurfaceR\asurface\"\x93\x01\n" +
	"\x19ListRelatedSeriesResponse\x120\n" +
	"\x06series\x18\x01 \x03(\v2\x18.publira.types.v1.SeriesR\x06series\x12%\n" +
	"\x0eprevious_token\x18\x02 \x01(\tR\rpreviousToken\x12\x1d\n" +
	"\n" +
	"next_token\x18\x03 \x01(\tR\tnextToken\"\xba\x01\n" +
	"\x18MarkEpisodeAsReadRequest\x127\n" +
	"\x06tenant\x18\x01 \x01(\v2\x1f.publira.types.v1.TenantContextR\x06tenant\x12*\n" +
	"\x11episode_public_id\x18\x02 \x01(\tR\x0fepisodePublicId\x129\n" +
	"\asurface\x18\x03 \x01(\x0e2\x1f.publira.types.v1.ClientSurfaceR\asurface\"4\n" +
	"\x19MarkEpisodeAsReadResponse\x12\x17\n" +
	"\aread_at\x18\x01 \x01(\tR\x06readAt\"\x9a\x01\n" +
	"\x0fReadingPosition\x12*\n" +
	"\x11episode_public_id\x18\x01 \x01(\tR\x0fepisodePublicId\x12\x1d\n" +
	"\n" +
	"page_index\x18\x02 \x01(\x05R\tpageIndex\x12\x1d\n" +
	"\n" +
	"page_count\x18\x03 \x01(\x05R\tpageCount\x12\x1d\n" +
	"\n" +
	"updated_at\x18\x04 \x01(\tR\tupdatedAt\"\xdb\x01\n" +
	"\x1aSaveReadingPositionRequest\x127\n" +
	"\x06tenant\x18\x01 \x01(\v2\x1f.publira.types.v1.TenantContextR\x06tenant\x12*\n" +
	"\x11episode_public_id\x18\x02 \x01(\tR\x0fepisodePublicId\x12\x1d\n" +
	"\n" +
	"page_index\x18\x03 \x01(\x05R\tpageIndex\x129\n" +
	"\asurface\x18\x04 \x01(\x0e2\x1f.publira.types.v1.ClientSurfaceR\asurface\"V\n" +
	"\x1bSaveReadingPositionResponse\x127\n" +
	"\bposition\x18\x01 \x01(\v2\x1b.publira.v1.ReadingPositionR\bposition\"\xbd\x01\n" +
	"\x1bGetMyReadingPositionRequest\x127\n" +
	"\x06tenant\x18\x01 \x01(\v2\x1f.publira.types.v1.TenantContextR\x06tenant\x12*\n" +
	"\x11episode_public_id\x18\x02 \x01(\tR\x0fepisodePublicId\x129\n" +
	"\asurface\x18\x03 \x01(\x0e2\x1f.publira.types.v1.ClientSurfaceR\asurface\"W\n" +
	"\x1cGetMyReadingPositionResponse\x127\n" +
	"\bposition\x18\x01 \x01(\v2\x1b.publira.v1.ReadingPositionR\bposition\"\xba\x01\n" +
	"\x1aGetMySeriesProgressRequest\x127\n" +
	"\x06tenant\x18\x01 \x01(\v2\x1f.publira.types.v1.TenantContextR\x06tenant\x12(\n" +
	"\x10series_public_id\x18\x02 \x01(\tR\x0eseriesPublicId\x129\n" +
	"\asurface\x18\x03 \x01(\x0e2\x1f.publira.types.v1.ClientSurfaceR\asurface\"\x9f\x01\n" +
	"\x0eSeriesProgress\x123\n" +
	"\aepisode\x18\x01 \x01(\v2\x19.publira.types.v1.EpisodeR\aepisode\x127\n" +
	"\bposition\x18\x02 \x01(\v2\x1b.publira.v1.ReadingPositionR\bposition\x12\x1f\n" +
	"\vis_finished\x18\x03 \x01(\bR\n" +
	"isFinished\"\x94\x01\n" +
	"\x1bGetMySeriesProgressResponse\x126\n" +
	"\bprogress\x18\x01 \x01(\v2\x1a.publira.v1.SeriesProgressR\bprogress\x12=\n" +
	"\x1bfinished_episode_public_ids\x18\x02 \x03(\tR\x18finishedEpisodePublicIds\"\xd8\x01\n" +
	"\fRecentSeries\x120\n" +
	"\x06series\x18\x01 \x01(\v2\x18.publira.types.v1.SeriesR\x06series\x123\n" +
	"\aepisode\x18\x02 \x01(\v2\x19.publira.types.v1.EpisodeR\aepisode\x127\n" +
	"\bposition\x18\x03 \x01(\v2\x1b.publira.v1.ReadingPositionR\bposition\x12(\n" +
	"\x10last_activity_at\x18\x04 \x01(\tR\x0elastActivityAt\"\xbb\x01\n" +
	"\x19ListMyRecentSeriesRequest\x127\n" +
	"\x06tenant\x18\x01 \x01(\v2\x1f.publira.types.v1.TenantContextR\x06tenant\x12\x14\n" +
	"\x05limit\x18\x02 \x01(\x05R\x05limit\x12\x14\n" +
	"\x05token\x18\x03 \x01(\tR\x05token\x129\n" +
	"\asurface\x18\x04 \x01(\x0e2\x1f.publira.types.v1.ClientSurfaceR\asurface\"\x94\x01\n" +
	"\x1aListMyRecentSeriesResponse\x120\n" +
	"\x06series\x18\x01 \x03(\v2\x18.publira.v1.RecentSeriesR\x06series\x12%\n" +
	"\x0eprevious_token\x18\x02 \x01(\tR\rpreviousToken\x12\x1d\n" +
	"\n" +
	"next_token\x18\x03 \x01(\tR\tnextToken\"\x8f\x01\n" +
	"\rMyEpisodeRead\x120\n" +
	"\x06series\x18\x01 \x01(\v2\x18.publira.types.v1.SeriesR\x06series\x123\n" +
	"\aepisode\x18\x02 \x01(\v2\x19.publira.types.v1.EpisodeR\aepisode\x12\x17\n" +
	"\aread_at\x18\x03 \x01(\tR\x06readAt\"\xbb\x01\n" +
	"\x19ListMyEpisodeReadsRequest\x127\n" +
	"\x06tenant\x18\x01 \x01(\v2\x1f.publira.types.v1.TenantContextR\x06tenant\x12\x14\n" +
	"\x05limit\x18\x02 \x01(\x05R\x05limit\x12\x14\n" +
	"\x05token\x18\x03 \x01(\tR\x05token\x129\n" +
	"\asurface\x18\x04 \x01(\x0e2\x1f.publira.types.v1.ClientSurfaceR\asurface\"\x93\x01\n" +
	"\x1aListMyEpisodeReadsResponse\x12/\n" +
	"\x05reads\x18\x01 \x03(\v2\x19.publira.v1.MyEpisodeReadR\x05reads\x12%\n" +
	"\x0eprevious_token\x18\x02 \x01(\tR\rpreviousToken\x12\x1d\n" +
	"\n" +
	"next_token\x18\x03 \x01(\tR\tnextToken\"]\n" +
	"\fFollowTarget\x120\n" +
	"\x04type\x18\x01 \x01(\x0e2\x1c.publira.v1.FollowTargetTypeR\x04type\x12\x1b\n" +
	"\tpublic_id\x18\x02 \x01(\tR\bpublicId\"\xc0\x01\n" +
	"\x18GetMyFollowStatusRequest\x127\n" +
	"\x06tenant\x18\x01 \x01(\v2\x1f.publira.types.v1.TenantContextR\x06tenant\x120\n" +
	"\x06target\x18\x02 \x01(\v2\x18.publira.v1.FollowTargetR\x06target\x129\n" +
	"\asurface\x18\x03 \x01(\x0e2\x1f.publira.types.v1.ClientSurfaceR\asurface\">\n" +
	"\x19GetMyFollowStatusResponse\x12!\n" +
	"\fis_following\x18\x01 \x01(\bR\visFollowing\"\xb5\x01\n" +
	"\rFollowRequest\x127\n" +
	"\x06tenant\x18\x01 \x01(\v2\x1f.publira.types.v1.TenantContextR\x06tenant\x120\n" +
	"\x06target\x18\x02 \x01(\v2\x18.publira.v1.FollowTargetR\x06target\x129\n" +
	"\asurface\x18\x03 \x01(\x0e2\x1f.publira.types.v1.ClientSurfaceR\asurface\"3\n" +
	"\x0eFollowResponse\x12!\n" +
	"\fis_following\x18\x01 \x01(\bR\visFollowing\"\xb7\x01\n" +
	"\x0fUnfollowRequest\x127\n" +
	"\x06tenant\x18\x01 \x01(\v2\x1f.publira.types.v1.TenantContextR\x06tenant\x120\n" +
	"\x06target\x18\x02 \x01(\v2\x18.publira.v1.FollowTargetR\x06target\x129\n" +
	"\asurface\x18\x03 \x01(\x0e2\x1f.publira.types.v1.ClientSurfaceR\asurface\"5\n" +
	"\x10UnfollowResponse\x12!\n" +
	"\fis_following\x18\x01 \x01(\bR\visFollowing\"\x94\x01\n" +
	"\bMyFollow\x12=\n" +
	"\vtarget_type\x18\x01 \x01(\x0e2\x1c.publira.v1.FollowTargetTypeR\n" +
	"targetType\x12(\n" +
	"\x10target_public_id\x18\x02 \x01(\tR\x0etargetPublicId\x12\x1f\n" +
	"\vfollowed_at\x18\x03 \x01(\tR\n" +
	"followedAt\"\xb6\x01\n" +
	"\x14ListMyFollowsRequest\x127\n" +
	"\x06tenant\x18\x01 \x01(\v2\x1f.publira.types.v1.TenantContextR\x06tenant\x12\x14\n" +
	"\x05limit\x18\x02 \x01(\x05R\x05limit\x12\x14\n" +
	"\x05token\x18\x03 \x01(\tR\x05token\x129\n" +
	"\asurface\x18\x04 \x01(\x0e2\x1f.publira.types.v1.ClientSurfaceR\asurface\"\x8d\x01\n" +
	"\x15ListMyFollowsResponse\x12.\n" +
	"\afollows\x18\x01 \x03(\v2\x14.publira.v1.MyFollowR\afollows\x12%\n" +
	"\x0eprevious_token\x18\x02 \x01(\tR\rpreviousToken\x12\x1d\n" +
	"\n" +
	"next_token\x18\x03 \x01(\tR\tnextToken\"u\n" +
	"\fFollowUpdate\x120\n" +
	"\x06series\x18\x01 \x01(\v2\x18.publira.types.v1.SeriesR\x06series\x123\n" +
	"\aepisode\x18\x02 \x01(\v2\x19.publira.types.v1.EpisodeR\aepisode\"\xbc\x01\n" +
	"\x1aListMyFollowUpdatesRequest\x127\n" +
	"\x06tenant\x18\x01 \x01(\v2\x1f.publira.types.v1.TenantContextR\x06tenant\x12\x14\n" +
	"\x05limit\x18\x02 \x01(\x05R\x05limit\x12\x14\n" +
	"\x05token\x18\x03 \x01(\tR\x05token\x129\n" +
	"\asurface\x18\x04 \x01(\x0e2\x1f.publira.types.v1.ClientSurfaceR\asurface\"\x97\x01\n" +
	"\x1bListMyFollowUpdatesResponse\x122\n" +
	"\aupdates\x18\x01 \x03(\v2\x18.publira.v1.FollowUpdateR\aupdates\x12%\n" +
	"\x0eprevious_token\x18\x02 \x01(\tR\rpreviousToken\x12\x1d\n" +
	"\n" +
	"next_token\x18\x03 \x01(\tR\tnextToken\"\xce\x01\n" +
	"\x12RateEpisodeRequest\x127\n" +
	"\x06tenant\x18\x01 \x01(\v2\x1f.publira.types.v1.TenantContextR\x06tenant\x12*\n" +
	"\x11episode_public_id\x18\x02 \x01(\tR\x0fepisodePublicId\x12\x18\n" +
	"\apresses\x18\x03 \x01(\x05R\apresses\x129\n" +
	"\asurface\x18\x04 \x01(\x0e2\x1f.publira.types.v1.ClientSurfaceR\asurface\"\x81\x01\n" +
	"\x13RateEpisodeResponse\x12\x14\n" +
	"\x05score\x18\x01 \x01(\x05R\x05score\x12!\n" +
	"\frating_count\x18\x02 \x01(\x03R\vratingCount\x121\n" +
	"\x04mode\x18\x03 \x01(\x0e2\x1d.publira.v1.EpisodeRatingModeR\x04mode\"\xbb\x01\n" +
	"\x19GetMyEpisodeRatingRequest\x127\n" +
	"\x06tenant\x18\x01 \x01(\v2\x1f.publira.types.v1.TenantContextR\x06tenant\x12*\n" +
	"\x11episode_public_id\x18\x02 \x01(\tR\x0fepisodePublicId\x129\n" +
	"\asurface\x18\x03 \x01(\x0e2\x1f.publira.types.v1.ClientSurfaceR\asurface\"\x88\x01\n" +
	"\x1aGetMyEpisodeRatingResponse\x12\x14\n" +
	"\x05score\x18\x01 \x01(\x05R\x05score\x12!\n" +
	"\frating_count\x18\x02 \x01(\x03R\vratingCount\x121\n" +
	"\x04mode\x18\x03 \x01(\x0e2\x1d.publira.v1.EpisodeRatingModeR\x04mode\"\xb8\x01\n" +
	"\x18GetMySeriesRatingRequest\x127\n" +
	"\x06tenant\x18\x01 \x01(\v2\x1f.publira.types.v1.TenantContextR\x06tenant\x12(\n" +
	"\x10series_public_id\x18\x02 \x01(\tR\x0eseriesPublicId\x129\n" +
	"\asurface\x18\x03 \x01(\x0e2\x1f.publira.types.v1.ClientSurfaceR\asurface\"r\n" +
	"\x19GetMySeriesRatingResponse\x12%\n" +
	"\x0erating_average\x18\x01 \x01(\x01R\rratingAverage\x12.\n" +
	"\x13rated_episode_count\x18\x02 \x01(\x05R\x11ratedEpisodeCount\"g\n" +
	"\x11ContentViewTarget\x125\n" +
	"\x04type\x18\x01 \x01(\x0e2!.publira.v1.ContentViewTargetTypeR\x04type\x12\x1b\n" +
	"\tpublic_id\x18\x02 \x01(\tR\bpublicId\"\xc5\x01\n" +
	"\x18RecordContentViewRequest\x127\n" +
	"\x06tenant\x18\x01 \x01(\v2\x1f.publira.types.v1.TenantContextR\x06tenant\x125\n" +
	"\x06target\x18\x02 \x01(\v2\x1d.publira.v1.ContentViewTargetR\x06target\x129\n" +
	"\asurface\x18\x03 \x01(\x0e2\x1f.publira.types.v1.ClientSurfaceR\asurface\"\x1b\n" +
	"\x19RecordContentViewResponse\"\x8f\x02\n" +
	"\x1bStartEpisodeCheckoutRequest\x127\n" +
	"\x06tenant\x18\x01 \x01(\v2\x1f.publira.types.v1.TenantContextR\x06tenant\x12*\n" +
	"\x11episode_public_id\x18\x02 \x01(\tR\x0fepisodePublicId\x12F\n" +
	"\x06client\x18\x03 \x01(\x0e2..publira.v1.StartEpisodeCheckoutRequest.ClientR\x06client\"C\n" +
	"\x06Client\x12\x16\n" +
	"\x12CLIENT_UNSPECIFIED\x10\x00\x12\x0e\n" +
	"\n" +
	"CLIENT_WEB\x10\x01\x12\x11\n" +
	"\rCLIENT_MOBILE\x10\x02\"A\n" +
	"\x1cStartEpisodeCheckoutResponse\x12!\n" +
	"\fcheckout_url\x18\x01 \x01(\tR\vcheckoutUrl\"\x8e\x02\n" +
	"\n" +
	"MyPurchase\x12\x0e\n" +
	"\x02id\x18\x01 \x01(\tR\x02id\x123\n" +
	"\aepisode\x18\x02 \x01(\v2\x19.publira.types.v1.EpisodeR\aepisode\x120\n" +
	"\x06series\x18\x03 \x01(\v2\x18.publira.types.v1.SeriesR\x06series\x12*\n" +
	"\x11price_at_purchase\x18\x04 \x01(\x05R\x0fpriceAtPurchase\x12!\n" +
	"\fpurchased_at\x18\x05 \x01(\tR\vpurchasedAt\x12\x1d\n" +
	"\n" +
	"expires_at\x18\x06 \x01(\tR\texpiresAt\x12\x1b\n" +
	"\tis_active\x18\a \x01(\bR\bisActive\"\xb8\x01\n" +
	"\x16ListMyPurchasesRequest\x127\n" +
	"\x06tenant\x18\x01 \x01(\v2\x1f.publira.types.v1.TenantContextR\x06tenant\x12\x14\n" +
	"\x05limit\x18\x02 \x01(\x05R\x05limit\x12\x14\n" +
	"\x05token\x18\x03 \x01(\tR\x05token\x129\n" +
	"\asurface\x18\x04 \x01(\x0e2\x1f.publira.types.v1.ClientSurfaceR\asurface\"\x95\x01\n" +
	"\x17ListMyPurchasesResponse\x124\n" +
	"\tpurchases\x18\x01 \x03(\v2\x16.publira.v1.MyPurchaseR\tpurchases\x12%\n" +
	"\x0eprevious_token\x18\x02 \x01(\tR\rpreviousToken\x12\x1d\n" +
	"\n" +
	"next_token\x18\x03 \x01(\tR\tnextToken\"\x9a\x02\n" +
	"\x1cProcessPaymentWebhookRequest\x127\n" +
	"\x06tenant\x18\x01 \x01(\v2\x1f.publira.types.v1.TenantContextR\x06tenant\x12\x1a\n" +
	"\bprovider\x18\x02 \x01(\tR\bprovider\x12\x18\n" +
	"\apayload\x18\x03 \x01(\fR\apayload\x12O\n" +
	"\aheaders\x18\x04 \x03(\v25.publira.v1.ProcessPaymentWebhookRequest.HeadersEntryR\aheaders\x1a:\n" +
	"\fHeadersEntry\x12\x10\n" +
	"\x03key\x18\x01 \x01(\tR\x03key\x12\x14\n" +
	"\x05value\x18\x02 \x01(\tR\x05value:\x028\x01\"\x1f\n" +
	"\x1dProcessPaymentWebhookResponse\"\xb6\x01\n" +
	"\x19StartStorePurchaseRequest\x127\n" +
	"\x06tenant\x18\x01 \x01(\v2\x1f.publira.types.v1.TenantContextR\x06tenant\x12*\n" +
	"\x11episode_public_id\x18\x02 \x01(\tR\x0fepisodePublicId\x124\n" +
	"\x05store\x18\x03 \x01(\x0e2\x1e.publira.v1.InAppPurchaseStoreR\x05store\"X\n" +
	"\x1aStartStorePurchaseResponse\x12\x1b\n" +
	"\tintent_id\x18\x01 \x01(\tR\bintentId\x12\x1d\n" +
	"\n" +
	"product_id\x18\x02 \x01(\tR\tproductId\"\xcd\x01\n" +
	"\x1bConfirmStorePurchaseRequest\x127\n" +
	"\x06tenant\x18\x01 \x01(\v2\x1f.publira.types.v1.TenantContextR\x06tenant\x124\n" +
	"\x05store\x18\x02 \x01(\x0e2\x1e.publira.v1.InAppPurchaseStoreR\x05store\x12 \n" +
	"\vtransaction\x18\x03 \x01(\tR\vtransaction\x12\x1d\n" +
	"\n" +
	"product_id\x18\x04 \x01(\tR\tproductId\"R\n" +
	"\x1cConfirmStorePurchaseResponse\x122\n" +
	"\bpurchase\x18\x01 \x01(\v2\x16.publira.v1.MyPurchaseR\bpurchase\"w\n" +
	"\"ProcessAppStoreNotificationRequest\x127\n" +
	"\x06tenant\x18\x01 \x01(\v2\x1f.publira.types.v1.TenantContextR\x06tenant\x12\x18\n" +
	"\apayload\x18\x02 \x01(\fR\apayload\"%\n" +
	"#ProcessAppStoreNotificationResponse*\xd4\x01\n" +
	"\vSeriesOrder\x12\x1c\n" +
	"\x18SERIES_ORDER_UNSPECIFIED\x10\x00\x12\"\n" +
	"\x1eSERIES_ORDER_PUBLISHED_AT_DESC\x10\x01\x12!\n" +
	"\x1dSERIES_ORDER_PUBLISHED_AT_ASC\x10\x02\x12\x1a\n" +
	"\x16SERIES_ORDER_TITLE_ASC\x10\x03\x12\x1b\n" +
	"\x17SERIES_ORDER_TITLE_DESC\x10\x04\x12'\n" +
	"#SERIES_ORDER_LATEST_EPISODE_AT_DESC\x10\x05*\xa3\x01\n" +
	"\rEpisodeAccess\x12\x1e\n" +
	"\x1aEPISODE_ACCESS_UNSPECIFIED\x10\x00\x12\x17\n" +
	"\x13EPISODE_ACCESS_FREE\x10\x01\x12\x19\n" +
	"\x15EPISODE_ACCESS_LOCKED\x10\x02\x12\x1b\n" +
	"\x17EPISODE_ACCESS_ENTITLED\x10\x03\x12!\n" +
	"\x1dEPISODE_ACCESS_AGE_RESTRICTED\x10\x04*\x88\x01\n" +
	"\x14RecommendationSource\x12%\n" +
	"!RECOMMENDATION_SOURCE_UNSPECIFIED\x10\x00\x12!\n" +
	"\x1dRECOMMENDATION_SOURCE_RANKING\x10\x01\x12&\n" +
	"\"RECOMMENDATION_SOURCE_NEW_ARRIVALS\x10\x02*d\n" +
	"\rRankingPeriod\x12\x1e\n" +
	"\x1aRANKING_PERIOD_UNSPECIFIED\x10\x00\x12\x18\n" +
	"\x14RANKING_PERIOD_DAILY\x10\x01\x12\x19\n" +
	"\x15RANKING_PERIOD_WEEKLY\x10\x02*\x95\x01\n" +
	"\x10FollowTargetType\x12\"\n" +
	"\x1eFOLLOW_TARGET_TYPE_UNSPECIFIED\x10\x00\x12\x1e\n" +
	"\x1aFOLLOW_TARGET_TYPE_EPISODE\x10\x01\x12\x1e\n" +
	"\x1aFOLLOW_TARGET_TYPE_CREATOR\x10\x02\x12\x1d\n" +
	"\x19FOLLOW_TARGET_TYPE_SERIES\x10\x03*z\n" +
	"\x11EpisodeRatingMode\x12#\n" +
	"\x1fEPISODE_RATING_MODE_UNSPECIFIED\x10\x00\x12\x1e\n" +
	"\x1aEPISODE_RATING_MODE_SINGLE\x10\x01\x12 \n" +
	"\x1cEPISODE_RATING_MODE_MULTIPLE\x10\x02*\x8c\x01\n" +
	"\x15ContentViewTargetType\x12(\n" +
	"$CONTENT_VIEW_TARGET_TYPE_UNSPECIFIED\x10\x00\x12#\n" +
	"\x1fCONTENT_VIEW_TARGET_TYPE_SERIES\x10\x01\x12$\n" +
	" CONTENT_VIEW_TARGET_TYPE_EPISODE\x10\x02*\x87\x01\n" +
	"\x12InAppPurchaseStore\x12%\n" +
	"!IN_APP_PURCHASE_STORE_UNSPECIFIED\x10\x00\x12#\n" +
	"\x1fIN_APP_PURCHASE_STORE_APP_STORE\x10\x01\x12%\n" +
	"!IN_APP_PURCHASE_STORE_GOOGLE_PLAY\x10\x022\xd1\r\n" +
	"\x0eCatalogService\x12h\n" +
	"\x13ListPublishedLabels\x12&.publira.v1.ListPublishedLabelsRequest\x1a'.publira.v1.ListPublishedLabelsResponse\"\x00\x12h\n" +
	"\x13ListPublishedSeries\x12&.publira.v1.ListPublishedSeriesRequest\x1a'.publira.v1.ListPublishedSeriesResponse\"\x00\x12\\\n" +
	"\x0fGetSeriesDetail\x12\".publira.v1.GetSeriesDetailRequest\x1a#.publira.v1.GetSeriesDetailResponse\"\x00\x12q\n" +
	"\x16GetSeriesEpisodeAccess\x12).publira.v1.GetSeriesEpisodeAccessRequest\x1a*.publira.v1.GetSeriesEpisodeAccessResponse\"\x00\x12_\n" +
	"\x10GetEpisodeDetail\x12#.publira.v1.GetEpisodeDetailRequest\x1a$.publira.v1.GetEpisodeDetailResponse\"\x00\x12n\n" +
	"\x15ListPublishedCreators\x12(.publira.v1.ListPublishedCreatorsRequest\x1a).publira.v1.ListPublishedCreatorsResponse\"\x00\x12z\n" +
	"\x19GetPublishedCreatorDetail\x12,.publira.v1.GetPublishedCreatorDetailRequest\x1a-.publira.v1.GetPublishedCreatorDetailResponse\"\x00\x12t\n" +
	"\x17GetPublishedLabelDetail\x12*.publira.v1.GetPublishedLabelDetailRequest\x1a+.publira.v1.GetPublishedLabelDetailResponse\"\x00\x12h\n" +
	"\x13ListPublishedGenres\x12&.publira.v1.ListPublishedGenresRequest\x1a'.publira.v1.ListPublishedGenresResponse\"\x00\x12b\n" +
	"\x11ListPublishedTags\x12$.publira.v1.ListPublishedTagsRequest\x1a%.publira.v1.ListPublishedTagsResponse\"\x00\x12n\n" +
	"\x15SearchPublishedSeries\x12(.publira.v1.SearchPublishedSeriesRequest\x1a).publira.v1.SearchPublishedSeriesResponse\"\x00\x12t\n" +
	"\x17SearchPublishedCreators\x12*.publira.v1.SearchPublishedCreatorsRequest\x1a+.publira.v1.SearchPublishedCreatorsResponse\"\x00\x12n\n" +
	"\x15SearchPublishedLabels\x12(.publira.v1.SearchPublishedLabelsRequest\x1a).publira.v1.SearchPublishedLabelsResponse\"\x00\x12n\n" +
	"\x15ListRecommendedSeries\x12(.publira.v1.ListRecommendedSeriesRequest\x1a).publira.v1.ListRecommendedSeriesResponse\"\x00\x12_\n" +
	"\x10ListRankedSeries\x12#.publira.v1.ListRankedSeriesRequest\x1a$.publira.v1.ListRankedSeriesResponse\"\x00\x12b\n" +
	"\x11ListRelatedSeries\x12$.publira.v1.ListRelatedSeriesRequest\x1a%.publira.v1.ListRelatedSeriesResponse\"\x002\x87\x05\n" +
	"\x12EpisodeReadService\x12b\n" +
	"\x11MarkEpisodeAsRead\x12$.publira.v1.MarkEpisodeAsReadRequest\x1a%.publira.v1.MarkEpisodeAsReadResponse\"\x00\x12h\n" +
	"\x13SaveReadingPosition\x12&.publira.v1.SaveReadingPositionRequest\x1a'.publira.v1.SaveReadingPositionResponse\"\x00\x12k\n" +
	"\x14GetMyReadingPosition\x12'.publira.v1.GetMyReadingPositionRequest\x1a(.publira.v1.GetMyReadingPositionResponse\"\x00\x12h\n" +
	"\x13GetMySeriesProgress\x12&.publira.v1.GetMySeriesProgressRequest\x1a'.publira.v1.GetMySeriesProgressResponse\"\x00\x12e\n" +
	"\x12ListMyRecentSeries\x12%.publira.v1.ListMyRecentSeriesRequest\x1a&.publira.v1.ListMyRecentSeriesResponse\"\x00\x12e\n" +
	"\x12ListMyEpisodeReads\x12%.publira.v1.ListMyEpisodeReadsRequest\x1a&.publira.v1.ListMyEpisodeReadsResponse\"\x002\xc1\x03\n" +
	"\rFollowService\x12b\n" +
	"\x11GetMyFollowStatus\x12$.publira.v1.GetMyFollowStatusRequest\x1a%.publira.v1.GetMyFollowStatusResponse\"\x00\x12A\n" +
	"\x06Follow\x12\x19.publira.v1.FollowRequest\x1a\x1a.publira.v1.FollowResponse\"\x00\x12G\n" +
	"\bUnfollow\x12\x1b.publira.v1.UnfollowRequest\x1a\x1c.publira.v1.UnfollowResponse\"\x00\x12V\n" +
	"\rListMyFollows\x12 .publira.v1.ListMyFollowsRequest\x1a!.publira.v1.ListMyFollowsResponse\"\x00\x12h\n" +
	"\x13ListMyFollowUpdates\x12&.publira.v1.ListMyFollowUpdatesRequest\x1a'.publira.v1.ListMyFollowUpdatesResponse\"\x002\xac\x02\n" +
	"\rRatingService\x12P\n" +
	"\vRateEpisode\x12\x1e.publira.v1.RateEpisodeRequest\x1a\x1f.publira.v1.RateEpisodeResponse\"\x00\x12e\n" +
	"\x12GetMyEpisodeRating\x12%.publira.v1.GetMyEpisodeRatingRequest\x1a&.publira.v1.GetMyEpisodeRatingResponse\"\x00\x12b\n" +
	"\x11GetMySeriesRating\x12$.publira.v1.GetMySeriesRatingRequest\x1a%.publira.v1.GetMySeriesRatingResponse\"\x002x\n" +
	"\x12ContentViewService\x12b\n" +
	"\x11RecordContentView\x12$.publira.v1.RecordContentViewRequest\x1a%.publira.v1.RecordContentViewResponse\"\x002\xa3\x05\n" +
	"\x0fPurchaseService\x12k\n" +
	"\x14StartEpisodeCheckout\x12'.publira.v1.StartEpisodeCheckoutRequest\x1a(.publira.v1.StartEpisodeCheckoutResponse\"\x00\x12\\\n" +
	"\x0fListMyPurchases\x12\".publira.v1.ListMyPurchasesRequest\x1a#.publira.v1.ListMyPurchasesResponse\"\x00\x12n\n" +
	"\x15ProcessPaymentWebhook\x12(.publira.v1.ProcessPaymentWebhookRequest\x1a).publira.v1.ProcessPaymentWebhookResponse\"\x00\x12e\n" +
	"\x12StartStorePurchase\x12%.publira.v1.StartStorePurchaseRequest\x1a&.publira.v1.StartStorePurchaseResponse\"\x00\x12k\n" +
	"\x14ConfirmStorePurchase\x12'.publira.v1.ConfirmStorePurchaseRequest\x1a(.publira.v1.ConfirmStorePurchaseResponse\"\x00\x12\x80\x01\n" +
	"\x1bProcessAppStoreNotification\x12..publira.v1.ProcessAppStoreNotificationRequest\x1a/.publira.v1.ProcessAppStoreNotificationResponse\"\x00BKZIgithub.com/publira/publira/server/internal/proto/gen/publira/v1;publirav1b\x06proto3"

var (
	file_publira_v1_catalog_proto_rawDescOnce sync.Once
	file_publira_v1_catalog_proto_rawDescData []byte
)

func file_publira_v1_catalog_proto_rawDescGZIP() []byte {
	file_publira_v1_catalog_proto_rawDescOnce.Do(func() {
		file_publira_v1_catalog_proto_rawDescData = protoimpl.X.CompressGZIP(unsafe.Slice(unsafe.StringData(file_publira_v1_catalog_proto_rawDesc), len(file_publira_v1_catalog_proto_rawDesc)))
	})
	return file_publira_v1_catalog_proto_rawDescData
}

var file_publira_v1_catalog_proto_enumTypes = make([]protoimpl.EnumInfo, 9)
var file_publira_v1_catalog_proto_msgTypes = make([]protoimpl.MessageInfo, 91)
var file_publira_v1_catalog_proto_goTypes = []any{
	(SeriesOrder)(0),                            // 0: publira.v1.SeriesOrder
	(EpisodeAccess)(0),                          // 1: publira.v1.EpisodeAccess
	(RecommendationSource)(0),                   // 2: publira.v1.RecommendationSource
	(RankingPeriod)(0),                          // 3: publira.v1.RankingPeriod
	(FollowTargetType)(0),                       // 4: publira.v1.FollowTargetType
	(EpisodeRatingMode)(0),                      // 5: publira.v1.EpisodeRatingMode
	(ContentViewTargetType)(0),                  // 6: publira.v1.ContentViewTargetType
	(InAppPurchaseStore)(0),                     // 7: publira.v1.InAppPurchaseStore
	(StartEpisodeCheckoutRequest_Client)(0),     // 8: publira.v1.StartEpisodeCheckoutRequest.Client
	(*ListPublishedLabelsRequest)(nil),          // 9: publira.v1.ListPublishedLabelsRequest
	(*ListPublishedLabelsResponse)(nil),         // 10: publira.v1.ListPublishedLabelsResponse
	(*ListPublishedSeriesRequest)(nil),          // 11: publira.v1.ListPublishedSeriesRequest
	(*ListPublishedSeriesResponse)(nil),         // 12: publira.v1.ListPublishedSeriesResponse
	(*GetSeriesDetailRequest)(nil),              // 13: publira.v1.GetSeriesDetailRequest
	(*GetSeriesDetailResponse)(nil),             // 14: publira.v1.GetSeriesDetailResponse
	(*GetSeriesEpisodeAccessRequest)(nil),       // 15: publira.v1.GetSeriesEpisodeAccessRequest
	(*SeriesEpisodeAccess)(nil),                 // 16: publira.v1.SeriesEpisodeAccess
	(*GetSeriesEpisodeAccessResponse)(nil),      // 17: publira.v1.GetSeriesEpisodeAccessResponse
	(*GetEpisodeDetailRequest)(nil),             // 18: publira.v1.GetEpisodeDetailRequest
	(*EpisodeNeighbor)(nil),                     // 19: publira.v1.EpisodeNeighbor
	(*GetEpisodeDetailResponse)(nil),            // 20: publira.v1.GetEpisodeDetailResponse
	(*PublishedCreator)(nil),                    // 21: publira.v1.PublishedCreator
	(*ListPublishedCreatorsRequest)(nil),        // 22: publira.v1.ListPublishedCreatorsRequest
	(*ListPublishedCreatorsResponse)(nil),       // 23: publira.v1.ListPublishedCreatorsResponse
	(*GetPublishedCreatorDetailRequest)(nil),    // 24: publira.v1.GetPublishedCreatorDetailRequest
	(*GetPublishedCreatorDetailResponse)(nil),   // 25: publira.v1.GetPublishedCreatorDetailResponse
	(*PublishedLabel)(nil),                      // 26: publira.v1.PublishedLabel
	(*GetPublishedLabelDetailRequest)(nil),      // 27: publira.v1.GetPublishedLabelDetailRequest
	(*GetPublishedLabelDetailResponse)(nil),     // 28: publira.v1.GetPublishedLabelDetailResponse
	(*PublishedGenre)(nil),                      // 29: publira.v1.PublishedGenre
	(*ListPublishedGenresRequest)(nil),          // 30: publira.v1.ListPublishedGenresRequest
	(*ListPublishedGenresResponse)(nil),         // 31: publira.v1.ListPublishedGenresResponse
	(*PublishedTag)(nil),                        // 32: publira.v1.PublishedTag
	(*ListPublishedTagsRequest)(nil),            // 33: publira.v1.ListPublishedTagsRequest
	(*ListPublishedTagsResponse)(nil),           // 34: publira.v1.ListPublishedTagsResponse
	(*SearchPublishedSeriesRequest)(nil),        // 35: publira.v1.SearchPublishedSeriesRequest
	(*SearchPublishedSeriesResponse)(nil),       // 36: publira.v1.SearchPublishedSeriesResponse
	(*SearchPublishedCreatorsRequest)(nil),      // 37: publira.v1.SearchPublishedCreatorsRequest
	(*SearchPublishedCreatorsResponse)(nil),     // 38: publira.v1.SearchPublishedCreatorsResponse
	(*SearchPublishedLabelsRequest)(nil),        // 39: publira.v1.SearchPublishedLabelsRequest
	(*SearchPublishedLabelsResponse)(nil),       // 40: publira.v1.SearchPublishedLabelsResponse
	(*ListRecommendedSeriesRequest)(nil),        // 41: publira.v1.ListRecommendedSeriesRequest
	(*ListRecommendedSeriesResponse)(nil),       // 42: publira.v1.ListRecommendedSeriesResponse
	(*RankedSeries)(nil),                        // 43: publira.v1.RankedSeries
	(*ListRankedSeriesRequest)(nil),             // 44: publira.v1.ListRankedSeriesRequest
	(*ListRankedSeriesResponse)(nil),            // 45: publira.v1.ListRankedSeriesResponse
	(*ListRelatedSeriesRequest)(nil),            // 46: publira.v1.ListRelatedSeriesRequest
	(*ListRelatedSeriesResponse)(nil),           // 47: publira.v1.ListRelatedSeriesResponse
	(*MarkEpisodeAsReadRequest)(nil),            // 48: publira.v1.MarkEpisodeAsReadRequest
	(*MarkEpisodeAsReadResponse)(nil),           // 49: publira.v1.MarkEpisodeAsReadResponse
	(*ReadingPosition)(nil),                     // 50: publira.v1.ReadingPosition
	(*SaveReadingPositionRequest)(nil),          // 51: publira.v1.SaveReadingPositionRequest
	(*SaveReadingPositionResponse)(nil),         // 52: publira.v1.SaveReadingPositionResponse
	(*GetMyReadingPositionRequest)(nil),         // 53: publira.v1.GetMyReadingPositionRequest
	(*GetMyReadingPositionResponse)(nil),        // 54: publira.v1.GetMyReadingPositionResponse
	(*GetMySeriesProgressRequest)(nil),          // 55: publira.v1.GetMySeriesProgressRequest
	(*SeriesProgress)(nil),                      // 56: publira.v1.SeriesProgress
	(*GetMySeriesProgressResponse)(nil),         // 57: publira.v1.GetMySeriesProgressResponse
	(*RecentSeries)(nil),                        // 58: publira.v1.RecentSeries
	(*ListMyRecentSeriesRequest)(nil),           // 59: publira.v1.ListMyRecentSeriesRequest
	(*ListMyRecentSeriesResponse)(nil),          // 60: publira.v1.ListMyRecentSeriesResponse
	(*MyEpisodeRead)(nil),                       // 61: publira.v1.MyEpisodeRead
	(*ListMyEpisodeReadsRequest)(nil),           // 62: publira.v1.ListMyEpisodeReadsRequest
	(*ListMyEpisodeReadsResponse)(nil),          // 63: publira.v1.ListMyEpisodeReadsResponse
	(*FollowTarget)(nil),                        // 64: publira.v1.FollowTarget
	(*GetMyFollowStatusRequest)(nil),            // 65: publira.v1.GetMyFollowStatusRequest
	(*GetMyFollowStatusResponse)(nil),           // 66: publira.v1.GetMyFollowStatusResponse
	(*FollowRequest)(nil),                       // 67: publira.v1.FollowRequest
	(*FollowResponse)(nil),                      // 68: publira.v1.FollowResponse
	(*UnfollowRequest)(nil),                     // 69: publira.v1.UnfollowRequest
	(*UnfollowResponse)(nil),                    // 70: publira.v1.UnfollowResponse
	(*MyFollow)(nil),                            // 71: publira.v1.MyFollow
	(*ListMyFollowsRequest)(nil),                // 72: publira.v1.ListMyFollowsRequest
	(*ListMyFollowsResponse)(nil),               // 73: publira.v1.ListMyFollowsResponse
	(*FollowUpdate)(nil),                        // 74: publira.v1.FollowUpdate
	(*ListMyFollowUpdatesRequest)(nil),          // 75: publira.v1.ListMyFollowUpdatesRequest
	(*ListMyFollowUpdatesResponse)(nil),         // 76: publira.v1.ListMyFollowUpdatesResponse
	(*RateEpisodeRequest)(nil),                  // 77: publira.v1.RateEpisodeRequest
	(*RateEpisodeResponse)(nil),                 // 78: publira.v1.RateEpisodeResponse
	(*GetMyEpisodeRatingRequest)(nil),           // 79: publira.v1.GetMyEpisodeRatingRequest
	(*GetMyEpisodeRatingResponse)(nil),          // 80: publira.v1.GetMyEpisodeRatingResponse
	(*GetMySeriesRatingRequest)(nil),            // 81: publira.v1.GetMySeriesRatingRequest
	(*GetMySeriesRatingResponse)(nil),           // 82: publira.v1.GetMySeriesRatingResponse
	(*ContentViewTarget)(nil),                   // 83: publira.v1.ContentViewTarget
	(*RecordContentViewRequest)(nil),            // 84: publira.v1.RecordContentViewRequest
	(*RecordContentViewResponse)(nil),           // 85: publira.v1.RecordContentViewResponse
	(*StartEpisodeCheckoutRequest)(nil),         // 86: publira.v1.StartEpisodeCheckoutRequest
	(*StartEpisodeCheckoutResponse)(nil),        // 87: publira.v1.StartEpisodeCheckoutResponse
	(*MyPurchase)(nil),                          // 88: publira.v1.MyPurchase
	(*ListMyPurchasesRequest)(nil),              // 89: publira.v1.ListMyPurchasesRequest
	(*ListMyPurchasesResponse)(nil),             // 90: publira.v1.ListMyPurchasesResponse
	(*ProcessPaymentWebhookRequest)(nil),        // 91: publira.v1.ProcessPaymentWebhookRequest
	(*ProcessPaymentWebhookResponse)(nil),       // 92: publira.v1.ProcessPaymentWebhookResponse
	(*StartStorePurchaseRequest)(nil),           // 93: publira.v1.StartStorePurchaseRequest
	(*StartStorePurchaseResponse)(nil),          // 94: publira.v1.StartStorePurchaseResponse
	(*ConfirmStorePurchaseRequest)(nil),         // 95: publira.v1.ConfirmStorePurchaseRequest
	(*ConfirmStorePurchaseResponse)(nil),        // 96: publira.v1.ConfirmStorePurchaseResponse
	(*ProcessAppStoreNotificationRequest)(nil),  // 97: publira.v1.ProcessAppStoreNotificationRequest
	(*ProcessAppStoreNotificationResponse)(nil), // 98: publira.v1.ProcessAppStoreNotificationResponse
	nil,                              // 99: publira.v1.ProcessPaymentWebhookRequest.HeadersEntry
	(*v1.TenantContext)(nil),         // 100: publira.types.v1.TenantContext
	(v1.ClientSurface)(0),            // 101: publira.types.v1.ClientSurface
	(*v1.Label)(nil),                 // 102: publira.types.v1.Label
	(v1.SeriesStatus)(0),             // 103: publira.types.v1.SeriesStatus
	(*v1.Series)(nil),                // 104: publira.types.v1.Series
	(*v1.Episode)(nil),               // 105: publira.types.v1.Episode
	(v1.CommentMode)(0),              // 106: publira.types.v1.CommentMode
	(*v1.Creator)(nil),               // 107: publira.types.v1.Creator
	(v1.SurfaceAvailability)(0),      // 108: publira.types.v1.SurfaceAvailability
	(*v1.EpisodeImage)(nil),          // 109: publira.types.v1.EpisodeImage
	(*v1.SeriesEyeCatchVariant)(nil), // 110: publira.types.v1.SeriesEyeCatchVariant
}
var file_publira_v1_catalog_proto_depIdxs = []int32{
	100, // 0: publira.v1.ListPublishedLabelsRequest.tenant:type_name -> publira.types.v1.TenantContext
	101, // 1: publira.v1.ListPublishedLabelsRequest.surface:type_name -> publira.types.v1.ClientSurface
	102, // 2: publira.v1.ListPublishedLabelsResponse.labels:type_name -> publira.types.v1.Label
	100, // 3: publira.v1.ListPublishedSeriesRequest.tenant:type_name -> publira.types.v1.TenantContext
	0,   // 4: publira.v1.ListPublishedSeriesRequest.order:type_name -> publira.v1.SeriesOrder
	103, // 5: publira.v1.ListPublishedSeriesRequest.status:type_name -> publira.types.v1.SeriesStatus
	101, // 6: publira.v1.ListPublishedSeriesRequest.surface:type_name -> publira.types.v1.ClientSurface
	104, // 7: publira.v1.ListPublishedSeriesResponse.series:type_name -> publira.types.v1.Series
	100, // 8: publira.v1.GetSeriesDetailRequest.tenant:type_name -> publira.types.v1.TenantContext
	101, // 9: publira.v1.GetSeriesDetailRequest.surface:type_name -> publira.types.v1.ClientSurface
	104, // 10: publira.v1.GetSeriesDetailResponse.series:type_name -> publira.types.v1.Series
	105, // 11: publira.v1.GetSeriesDetailResponse.episodes:type_name -> publira.types.v1.Episode
	106, // 12: publira.v1.GetSeriesDetailResponse.comment_mode:type_name -> publira.types.v1.CommentMode
	100, // 13: publira.v1.GetSeriesEpisodeAccessRequest.tenant:type_name -> publira.types.v1.TenantContext
	101, // 14: publira.v1.GetSeriesEpisodeAccessRequest.surface:type_name -> publira.types.v1.ClientSurface
	1,   // 15: publira.v1.SeriesEpisodeAccess.access:type_name -> publira.v1.EpisodeAccess
	16,  // 16: publira.v1.GetSeriesEpisodeAccessResponse.episodes:type_name -> publira.v1.SeriesEpisodeAccess
	100, // 17: publira.v1.GetEpisodeDetailRequest.tenant:type_name -> publira.types.v1.TenantContext
	101, // 18: publira.v1.GetEpisodeDetailRequest.surface:type_name -> publira.types.v1.ClientSurface
	107, // 19: publira.v1.EpisodeNeighbor.creators:type_name -> publira.types.v1.Creator
	108, // 20: publira.v1.EpisodeNeighbor.purchase_availability:type_name -> publira.types.v1.SurfaceAvailability
	105, // 21: publira.v1.GetEpisodeDetailResponse.episode:type_name -> publira.types.v1.Episode
	104, // 22: publira.v1.GetEpisodeDetailResponse.series:type_name -> publira.types.v1.Series
	109, // 23: publira.v1.GetEpisodeDetailResponse.images:type_name -> publira.types.v1.EpisodeImage
	1,   // 24: publira.v1.GetEpisodeDetailResponse.access:type_name -> publira.v1.EpisodeAccess
	19,  // 25: publira.v1.GetEpisodeDetailResponse.previous_episode:type_name -> publira.v1.EpisodeNeighbor
	19,  // 26: publira.v1.GetEpisodeDetailResponse.next_episode:type_name -> publira.v1.EpisodeNeighbor
	100, // 27: publira.v1.ListPublishedCreatorsRequest.tenant:type_name -> publira.types.v1.TenantContext
	101, // 28: publira.v1.ListPublishedCreatorsRequest.surface:type_name -> publira.types.v1.ClientSurface
	21,  // 29: publira.v1.ListPublishedCreatorsResponse.creators:type_name -> publira.v1.PublishedCreator
	100, // 30: publira.v1.GetPublishedCreatorDetailRequest.tenant:type_name -> publira.types.v1.TenantContext
	101, // 31: publira.v1.GetPublishedCreatorDetailRequest.surface:type_name -> publira.types.v1.ClientSurface
	21,  // 32: publira.v1.GetPublishedCreatorDetailResponse.creator:type_name -> publira.v1.PublishedCreator
	104, // 33: publira.v1.GetPublishedCreatorDetailResponse.series:type_name -> publira.types.v1.Series
	110, // 34: publira.v1.PublishedLabel.eye_catch_image_variants:type_name -> publira.types.v1.SeriesEyeCatchVariant
	100, // 35: publira.v1.GetPublishedLabelDetailRequest.tenant:type_name -> publira.types.v1.TenantContext
	101, // 36: publira.v1.GetPublishedLabelDetailRequest.surface:type_name -> publira.types.v1.ClientSurface
	26,  // 37: publira.v1.GetPublishedLabelDetailResponse.label:type_name -> publira.v1.PublishedLabel
	104, // 38: publira.v1.GetPublishedLabelDetailResponse.series:type_name -> publira.types.v1.Series
	100, // 39: publira.v1.ListPublishedGenresRequest.tenant:type_name -> publira.types.v1.TenantContext
	101, // 40: publira.v1.ListPublishedGenresRequest.surface:type_name -> publira.types.v1.ClientSurface
	29,  // 41: publira.v1.ListPublishedGenresResponse.genres:type_name -> publira.v1.PublishedGenre
	100, // 42: publira.v1.ListPublishedTagsRequest.tenant:type_name -> publira.types.v1.TenantContext
	101, // 43: publira.v1.ListPublishedTagsRequest.surface:type_name -> publira.types.v1.ClientSurface
	32,  // 44: publira.v1.ListPublishedTagsResponse.tags:type_name -> publira.v1.PublishedTag
	100, // 45: publira.v1.SearchPublishedSeriesRequest.tenant:type_name -> publira.types.v1.TenantContext
	101, // 46: publira.v1.SearchPublishedSeriesRequest.surface:type_name -> publira.types.v1.ClientSurface
	104, // 47: publira.v1.SearchPublishedSeriesResponse.series:type_name -> publira.types.v1.Series
	100, // 48: publira.v1.SearchPublishedCreatorsRequest.tenant:type_name -> publira.types.v1.TenantContext
	101, // 49: publira.v1.SearchPublishedCreatorsRequest.surface:type_name -> publira.types.v1.ClientSurface
	21,  // 50: publira.v1.SearchPublishedCreatorsResponse.creators:type_name -> publira.v1.PublishedCreator
	100, // 51: publira.v1.SearchPublishedLabelsRequest.tenant:type_name -> publira.types.v1.TenantContext
	101, // 52: publira.v1.SearchPublishedLabelsRequest.surface:type_name -> publira.types.v1.ClientSurface
	102, // 53: publira.v1.SearchPublishedLabelsResponse.labels:type_name -> publira.types.v1.Label
	100, // 54: publira.v1.ListRecommendedSeriesRequest.tenant:type_name -> publira.types.v1.TenantContext
	101, // 55: publira.v1.ListRecommendedSeriesRequest.surface:type_name -> publira.types.v1.ClientSurface
	104, // 56: publira.v1.ListRecommendedSeriesResponse.series:type_name -> publira.types.v1.Series
	2,   // 57: publira.v1.ListRecommendedSeriesResponse.source:type_name -> publira.v1.RecommendationSource
	104, // 58: publira.v1.RankedSeries.series:type_name -> publira.types.v1.Series
	100, // 59: publira.v1.ListRankedSeriesRequest.tenant:type_name -> publira.types.v1.TenantContext
	3,   // 60: publira.v1.ListRankedSeriesRequest.period:type_name -> publira.v1.RankingPeriod
	101, // 61: publira.v1.ListRankedSeriesRequest.surface:type_name -> publira.types.v1.ClientSurface
	43,  // 62: publira.v1.ListRankedSeriesResponse.ranked_series:type_name -> publira.v1.RankedSeries
	100, // 63: publira.v1.ListRelatedSeriesRequest.tenant:type_name -> publira.types.v1.TenantContext
	101, // 64: publira.v1.ListRelatedSeriesRequest.surface:type_name -> publira.types.v1.ClientSurface
	104, // 65: publira.v1.ListRelatedSeriesResponse.series:type_name -> publira.types.v1.Series
	100, // 66: publira.v1.MarkEpisodeAsReadRequest.tenant:type_name -> publira.types.v1.TenantContext
	101, // 67: publira.v1.MarkEpisodeAsReadRequest.surface:type_name -> publira.types.v1.ClientSurface
	100, // 68: publira.v1.SaveReadingPositionRequest.tenant:type_name -> publira.types.v1.TenantContext
	101, // 69: publira.v1.SaveReadingPositionRequest.surface:type_name -> publira.types.v1.ClientSurface
	50,  // 70: publira.v1.SaveReadingPositionResponse.position:type_name -> publira.v1.ReadingPosition
	100, // 71: publira.v1.GetMyReadingPositionRequest.tenant:type_name -> publira.types.v1.TenantContext
	101, // 72: publira.v1.GetMyReadingPositionRequest.surface:type_name -> publira.types.v1.ClientSurface
	50,  // 73: publira.v1.GetMyReadingPositionResponse.position:type_name -> publira.v1.ReadingPosition
	100, // 74: publira.v1.GetMySeriesProgressRequest.tenant:type_name -> publira.types.v1.TenantContext
	101, // 75: publira.v1.GetMySeriesProgressRequest.surface:type_name -> publira.types.v1.ClientSurface
	105, // 76: publira.v1.SeriesProgress.episode:type_name -> publira.types.v1.Episode
	50,  // 77: publira.v1.SeriesProgress.position:type_name -> publira.v1.ReadingPosition
	56,  // 78: publira.v1.GetMySeriesProgressResponse.progress:type_name -> publira.v1.SeriesProgress
	104, // 79: publira.v1.RecentSeries.series:type_name -> publira.types.v1.Series
	105, // 80: publira.v1.RecentSeries.episode:type_name -> publira.types.v1.Episode
	50,  // 81: publira.v1.RecentSeries.position:type_name -> publira.v1.ReadingPosition
	100, // 82: publira.v1.ListMyRecentSeriesRequest.tenant:type_name -> publira.types.v1.TenantContext
	101, // 83: publira.v1.ListMyRecentSeriesRequest.surface:type_name -> publira.types.v1.ClientSurface
	58,  // 84: publira.v1.ListMyRecentSeriesResponse.series:type_name -> publira.v1.RecentSeries
	104, // 85: publira.v1.MyEpisodeRead.series:type_name -> publira.types.v1.Series
	105, // 86: publira.v1.MyEpisodeRead.episode:type_name -> publira.types.v1.Episode
	100, // 87: publira.v1.ListMyEpisodeReadsRequest.tenant:type_name -> publira.types.v1.TenantContext
	101, // 88: publira.v1.ListMyEpisodeReadsRequest.surface:type_name -> publira.types.v1.ClientSurface
	61,  // 89: publira.v1.ListMyEpisodeReadsResponse.reads:type_name -> publira.v1.MyEpisodeRead
	4,   // 90: publira.v1.FollowTarget.type:type_name -> publira.v1.FollowTargetType
	100, // 91: publira.v1.GetMyFollowStatusRequest.tenant:type_name -> publira.types.v1.TenantContext
	64,  // 92: publira.v1.GetMyFollowStatusRequest.target:type_name -> publira.v1.FollowTarget
	101, // 93: publira.v1.GetMyFollowStatusRequest.surface:type_name -> publira.types.v1.ClientSurface
	100, // 94: publira.v1.FollowRequest.tenant:type_name -> publira.types.v1.TenantContext
	64,  // 95: publira.v1.FollowRequest.target:type_name -> publira.v1.FollowTarget
	101, // 96: publira.v1.FollowRequest.surface:type_name -> publira.types.v1.ClientSurface
	100, // 97: publira.v1.UnfollowRequest.tenant:type_name -> publira.types.v1.TenantContext
	64,  // 98: publira.v1.UnfollowRequest.target:type_name -> publira.v1.FollowTarget
	101, // 99: publira.v1.UnfollowRequest.surface:type_name -> publira.types.v1.ClientSurface
	4,   // 100: publira.v1.MyFollow.target_type:type_name -> publira.v1.FollowTargetType
	100, // 101: publira.v1.ListMyFollowsRequest.tenant:type_name -> publira.types.v1.TenantContext
	101, // 102: publira.v1.ListMyFollowsRequest.surface:type_name -> publira.types.v1.ClientSurface
	71,  // 103: publira.v1.ListMyFollowsResponse.follows:type_name -> publira.v1.MyFollow
	104, // 104: publira.v1.FollowUpdate.series:type_name -> publira.types.v1.Series
	105, // 105: publira.v1.FollowUpdate.episode:type_name -> publira.types.v1.Episode
	100, // 106: publira.v1.ListMyFollowUpdatesRequest.tenant:type_name -> publira.types.v1.TenantContext
	101, // 107: publira.v1.ListMyFollowUpdatesRequest.surface:type_name -> publira.types.v1.ClientSurface
	74,  // 108: publira.v1.ListMyFollowUpdatesResponse.updates:type_name -> publira.v1.FollowUpdate
	100, // 109: publira.v1.RateEpisodeRequest.tenant:type_name -> publira.types.v1.TenantContext
	101, // 110: publira.v1.RateEpisodeRequest.surface:type_name -> publira.types.v1.ClientSurface
	5,   // 111: publira.v1.RateEpisodeResponse.mode:type_name -> publira.v1.EpisodeRatingMode
	100, // 112: publira.v1.GetMyEpisodeRatingRequest.tenant:type_name -> publira.types.v1.TenantContext
	101, // 113: publira.v1.GetMyEpisodeRatingRequest.surface:type_name -> publira.types.v1.ClientSurface
	5,   // 114: publira.v1.GetMyEpisodeRatingResponse.mode:type_name -> publira.v1.EpisodeRatingMode
	100, // 115: publira.v1.GetMySeriesRatingRequest.tenant:type_name -> publira.types.v1.TenantContext
	101, // 116: publira.v1.GetMySeriesRatingRequest.surface:type_name -> publira.types.v1.ClientSurface
	6,   // 117: publira.v1.ContentViewTarget.type:type_name -> publira.v1.ContentViewTargetType
	100, // 118: publira.v1.RecordContentViewRequest.tenant:type_name -> publira.types.v1.TenantContext
	83,  // 119: publira.v1.RecordContentViewRequest.target:type_name -> publira.v1.ContentViewTarget
	101, // 120: publira.v1.RecordContentViewRequest.surface:type_name -> publira.types.v1.ClientSurface
	100, // 121: publira.v1.StartEpisodeCheckoutRequest.tenant:type_name -> publira.types.v1.TenantContext
	8,   // 122: publira.v1.StartEpisodeCheckoutRequest.client:type_name -> publira.v1.StartEpisodeCheckoutRequest.Client
	105, // 123: publira.v1.MyPurchase.episode:type_name -> publira.types.v1.Episode
	104, // 124: publira.v1.MyPurchase.series:type_name -> publira.types.v1.Series
	100, // 125: publira.v1.ListMyPurchasesRequest.tenant:type_name -> publira.types.v1.TenantContext
	101, // 126: publira.v1.ListMyPurchasesRequest.surface:type_name -> publira.types.v1.ClientSurface
	88,  // 127: publira.v1.ListMyPurchasesResponse.purchases:type_name -> publira.v1.MyPurchase
	100, // 128: publira.v1.ProcessPaymentWebhookRequest.tenant:type_name -> publira.types.v1.TenantContext
	99,  // 129: publira.v1.ProcessPaymentWebhookRequest.headers:type_name -> publira.v1.ProcessPaymentWebhookRequest.HeadersEntry
	100, // 130: publira.v1.StartStorePurchaseRequest.tenant:type_name -> publira.types.v1.TenantContext
	7,   // 131: publira.v1.StartStorePurchaseRequest.store:type_name -> publira.v1.InAppPurchaseStore
	100, // 132: publira.v1.ConfirmStorePurchaseRequest.tenant:type_name -> publira.types.v1.TenantContext
	7,   // 133: publira.v1.ConfirmStorePurchaseRequest.store:type_name -> publira.v1.InAppPurchaseStore
	88,  // 134: publira.v1.ConfirmStorePurchaseResponse.purchase:type_name -> publira.v1.MyPurchase
	100, // 135: publira.v1.ProcessAppStoreNotificationRequest.tenant:type_name -> publira.types.v1.TenantContext
	9,   // 136: publira.v1.CatalogService.ListPublishedLabels:input_type -> publira.v1.ListPublishedLabelsRequest
	11,  // 137: publira.v1.CatalogService.ListPublishedSeries:input_type -> publira.v1.ListPublishedSeriesRequest
	13,  // 138: publira.v1.CatalogService.GetSeriesDetail:input_type -> publira.v1.GetSeriesDetailRequest
	15,  // 139: publira.v1.CatalogService.GetSeriesEpisodeAccess:input_type -> publira.v1.GetSeriesEpisodeAccessRequest
	18,  // 140: publira.v1.CatalogService.GetEpisodeDetail:input_type -> publira.v1.GetEpisodeDetailRequest
	22,  // 141: publira.v1.CatalogService.ListPublishedCreators:input_type -> publira.v1.ListPublishedCreatorsRequest
	24,  // 142: publira.v1.CatalogService.GetPublishedCreatorDetail:input_type -> publira.v1.GetPublishedCreatorDetailRequest
	27,  // 143: publira.v1.CatalogService.GetPublishedLabelDetail:input_type -> publira.v1.GetPublishedLabelDetailRequest
	30,  // 144: publira.v1.CatalogService.ListPublishedGenres:input_type -> publira.v1.ListPublishedGenresRequest
	33,  // 145: publira.v1.CatalogService.ListPublishedTags:input_type -> publira.v1.ListPublishedTagsRequest
	35,  // 146: publira.v1.CatalogService.SearchPublishedSeries:input_type -> publira.v1.SearchPublishedSeriesRequest
	37,  // 147: publira.v1.CatalogService.SearchPublishedCreators:input_type -> publira.v1.SearchPublishedCreatorsRequest
	39,  // 148: publira.v1.CatalogService.SearchPublishedLabels:input_type -> publira.v1.SearchPublishedLabelsRequest
	41,  // 149: publira.v1.CatalogService.ListRecommendedSeries:input_type -> publira.v1.ListRecommendedSeriesRequest
	44,  // 150: publira.v1.CatalogService.ListRankedSeries:input_type -> publira.v1.ListRankedSeriesRequest
	46,  // 151: publira.v1.CatalogService.ListRelatedSeries:input_type -> publira.v1.ListRelatedSeriesRequest
	48,  // 152: publira.v1.EpisodeReadService.MarkEpisodeAsRead:input_type -> publira.v1.MarkEpisodeAsReadRequest
	51,  // 153: publira.v1.EpisodeReadService.SaveReadingPosition:input_type -> publira.v1.SaveReadingPositionRequest
	53,  // 154: publira.v1.EpisodeReadService.GetMyReadingPosition:input_type -> publira.v1.GetMyReadingPositionRequest
	55,  // 155: publira.v1.EpisodeReadService.GetMySeriesProgress:input_type -> publira.v1.GetMySeriesProgressRequest
	59,  // 156: publira.v1.EpisodeReadService.ListMyRecentSeries:input_type -> publira.v1.ListMyRecentSeriesRequest
	62,  // 157: publira.v1.EpisodeReadService.ListMyEpisodeReads:input_type -> publira.v1.ListMyEpisodeReadsRequest
	65,  // 158: publira.v1.FollowService.GetMyFollowStatus:input_type -> publira.v1.GetMyFollowStatusRequest
	67,  // 159: publira.v1.FollowService.Follow:input_type -> publira.v1.FollowRequest
	69,  // 160: publira.v1.FollowService.Unfollow:input_type -> publira.v1.UnfollowRequest
	72,  // 161: publira.v1.FollowService.ListMyFollows:input_type -> publira.v1.ListMyFollowsRequest
	75,  // 162: publira.v1.FollowService.ListMyFollowUpdates:input_type -> publira.v1.ListMyFollowUpdatesRequest
	77,  // 163: publira.v1.RatingService.RateEpisode:input_type -> publira.v1.RateEpisodeRequest
	79,  // 164: publira.v1.RatingService.GetMyEpisodeRating:input_type -> publira.v1.GetMyEpisodeRatingRequest
	81,  // 165: publira.v1.RatingService.GetMySeriesRating:input_type -> publira.v1.GetMySeriesRatingRequest
	84,  // 166: publira.v1.ContentViewService.RecordContentView:input_type -> publira.v1.RecordContentViewRequest
	86,  // 167: publira.v1.PurchaseService.StartEpisodeCheckout:input_type -> publira.v1.StartEpisodeCheckoutRequest
	89,  // 168: publira.v1.PurchaseService.ListMyPurchases:input_type -> publira.v1.ListMyPurchasesRequest
	91,  // 169: publira.v1.PurchaseService.ProcessPaymentWebhook:input_type -> publira.v1.ProcessPaymentWebhookRequest
	93,  // 170: publira.v1.PurchaseService.StartStorePurchase:input_type -> publira.v1.StartStorePurchaseRequest
	95,  // 171: publira.v1.PurchaseService.ConfirmStorePurchase:input_type -> publira.v1.ConfirmStorePurchaseRequest
	97,  // 172: publira.v1.PurchaseService.ProcessAppStoreNotification:input_type -> publira.v1.ProcessAppStoreNotificationRequest
	10,  // 173: publira.v1.CatalogService.ListPublishedLabels:output_type -> publira.v1.ListPublishedLabelsResponse
	12,  // 174: publira.v1.CatalogService.ListPublishedSeries:output_type -> publira.v1.ListPublishedSeriesResponse
	14,  // 175: publira.v1.CatalogService.GetSeriesDetail:output_type -> publira.v1.GetSeriesDetailResponse
	17,  // 176: publira.v1.CatalogService.GetSeriesEpisodeAccess:output_type -> publira.v1.GetSeriesEpisodeAccessResponse
	20,  // 177: publira.v1.CatalogService.GetEpisodeDetail:output_type -> publira.v1.GetEpisodeDetailResponse
	23,  // 178: publira.v1.CatalogService.ListPublishedCreators:output_type -> publira.v1.ListPublishedCreatorsResponse
	25,  // 179: publira.v1.CatalogService.GetPublishedCreatorDetail:output_type -> publira.v1.GetPublishedCreatorDetailResponse
	28,  // 180: publira.v1.CatalogService.GetPublishedLabelDetail:output_type -> publira.v1.GetPublishedLabelDetailResponse
	31,  // 181: publira.v1.CatalogService.ListPublishedGenres:output_type -> publira.v1.ListPublishedGenresResponse
	34,  // 182: publira.v1.CatalogService.ListPublishedTags:output_type -> publira.v1.ListPublishedTagsResponse
	36,  // 183: publira.v1.CatalogService.SearchPublishedSeries:output_type -> publira.v1.SearchPublishedSeriesResponse
	38,  // 184: publira.v1.CatalogService.SearchPublishedCreators:output_type -> publira.v1.SearchPublishedCreatorsResponse
	40,  // 185: publira.v1.CatalogService.SearchPublishedLabels:output_type -> publira.v1.SearchPublishedLabelsResponse
	42,  // 186: publira.v1.CatalogService.ListRecommendedSeries:output_type -> publira.v1.ListRecommendedSeriesResponse
	45,  // 187: publira.v1.CatalogService.ListRankedSeries:output_type -> publira.v1.ListRankedSeriesResponse
	47,  // 188: publira.v1.CatalogService.ListRelatedSeries:output_type -> publira.v1.ListRelatedSeriesResponse
	49,  // 189: publira.v1.EpisodeReadService.MarkEpisodeAsRead:output_type -> publira.v1.MarkEpisodeAsReadResponse
	52,  // 190: publira.v1.EpisodeReadService.SaveReadingPosition:output_type -> publira.v1.SaveReadingPositionResponse
	54,  // 191: publira.v1.EpisodeReadService.GetMyReadingPosition:output_type -> publira.v1.GetMyReadingPositionResponse
	57,  // 192: publira.v1.EpisodeReadService.GetMySeriesProgress:output_type -> publira.v1.GetMySeriesProgressResponse
	60,  // 193: publira.v1.EpisodeReadService.ListMyRecentSeries:output_type -> publira.v1.ListMyRecentSeriesResponse
	63,  // 194: publira.v1.EpisodeReadService.ListMyEpisodeReads:output_type -> publira.v1.ListMyEpisodeReadsResponse
	66,  // 195: publira.v1.FollowService.GetMyFollowStatus:output_type -> publira.v1.GetMyFollowStatusResponse
	68,  // 196: publira.v1.FollowService.Follow:output_type -> publira.v1.FollowResponse
	70,  // 197: publira.v1.FollowService.Unfollow:output_type -> publira.v1.UnfollowResponse
	73,  // 198: publira.v1.FollowService.ListMyFollows:output_type -> publira.v1.ListMyFollowsResponse
	76,  // 199: publira.v1.FollowService.ListMyFollowUpdates:output_type -> publira.v1.ListMyFollowUpdatesResponse
	78,  // 200: publira.v1.RatingService.RateEpisode:output_type -> publira.v1.RateEpisodeResponse
	80,  // 201: publira.v1.RatingService.GetMyEpisodeRating:output_type -> publira.v1.GetMyEpisodeRatingResponse
	82,  // 202: publira.v1.RatingService.GetMySeriesRating:output_type -> publira.v1.GetMySeriesRatingResponse
	85,  // 203: publira.v1.ContentViewService.RecordContentView:output_type -> publira.v1.RecordContentViewResponse
	87,  // 204: publira.v1.PurchaseService.StartEpisodeCheckout:output_type -> publira.v1.StartEpisodeCheckoutResponse
	90,  // 205: publira.v1.PurchaseService.ListMyPurchases:output_type -> publira.v1.ListMyPurchasesResponse
	92,  // 206: publira.v1.PurchaseService.ProcessPaymentWebhook:output_type -> publira.v1.ProcessPaymentWebhookResponse
	94,  // 207: publira.v1.PurchaseService.StartStorePurchase:output_type -> publira.v1.StartStorePurchaseResponse
	96,  // 208: publira.v1.PurchaseService.ConfirmStorePurchase:output_type -> publira.v1.ConfirmStorePurchaseResponse
	98,  // 209: publira.v1.PurchaseService.ProcessAppStoreNotification:output_type -> publira.v1.ProcessAppStoreNotificationResponse
	173, // [173:210] is the sub-list for method output_type
	136, // [136:173] is the sub-list for method input_type
	136, // [136:136] is the sub-list for extension type_name
	136, // [136:136] is the sub-list for extension extendee
	0,   // [0:136] is the sub-list for field type_name
}

func init() { file_publira_v1_catalog_proto_init() }
func file_publira_v1_catalog_proto_init() {
	if File_publira_v1_catalog_proto != nil {
		return
	}
	file_publira_v1_catalog_proto_msgTypes[2].OneofWrappers = []any{}
	file_publira_v1_catalog_proto_msgTypes[34].OneofWrappers = []any{}
	type x struct{}
	out := protoimpl.TypeBuilder{
		File: protoimpl.DescBuilder{
			GoPackagePath: reflect.TypeOf(x{}).PkgPath(),
			RawDescriptor: unsafe.Slice(unsafe.StringData(file_publira_v1_catalog_proto_rawDesc), len(file_publira_v1_catalog_proto_rawDesc)),
			NumEnums:      9,
			NumMessages:   91,
			NumExtensions: 0,
			NumServices:   6,
		},
		GoTypes:           file_publira_v1_catalog_proto_goTypes,
		DependencyIndexes: file_publira_v1_catalog_proto_depIdxs,
		EnumInfos:         file_publira_v1_catalog_proto_enumTypes,
		MessageInfos:      file_publira_v1_catalog_proto_msgTypes,
	}.Build()
	File_publira_v1_catalog_proto = out.File
	file_publira_v1_catalog_proto_goTypes = nil
	file_publira_v1_catalog_proto_depIdxs = nil
}
