// Code generated by protoc-gen-go. DO NOT EDIT.
// versions:
// 	protoc-gen-go v1.36.12
// 	protoc        (unknown)
// source: publira/admin/v1/royalty.proto

package publiraadminv1

import (
	v1 "github.com/publira/publira/server/internal/proto/gen/publira/types/v1"
	protoreflect "google.golang.org/protobuf/reflect/protoreflect"
	protoimpl "google.golang.org/protobuf/runtime/protoimpl"
	reflect "reflect"
	sync "sync"
	unsafe "unsafe"
)

const (
	// Verify that this generated code is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(20 - protoimpl.MinVersion)
	// Verify that runtime/protoimpl is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(protoimpl.MaxVersion - 20)
)

type RoyaltyCloseMode int32

const (
	RoyaltyCloseMode_ROYALTY_CLOSE_MODE_UNSPECIFIED RoyaltyCloseMode = 0
	RoyaltyCloseMode_ROYALTY_CLOSE_MODE_MANUAL      RoyaltyCloseMode = 1
	RoyaltyCloseMode_ROYALTY_CLOSE_MODE_AUTOMATIC   RoyaltyCloseMode = 2
)

// Enum value maps for RoyaltyCloseMode.
var (
	RoyaltyCloseMode_name = map[int32]string{
		0: "ROYALTY_CLOSE_MODE_UNSPECIFIED",
		1: "ROYALTY_CLOSE_MODE_MANUAL",
		2: "ROYALTY_CLOSE_MODE_AUTOMATIC",
	}
	RoyaltyCloseMode_value = map[string]int32{
		"ROYALTY_CLOSE_MODE_UNSPECIFIED": 0,
		"ROYALTY_CLOSE_MODE_MANUAL":      1,
		"ROYALTY_CLOSE_MODE_AUTOMATIC":   2,
	}
)

func (x RoyaltyCloseMode) Enum() *RoyaltyCloseMode {
	p := new(RoyaltyCloseMode)
	*p = x
	return p
}

func (x RoyaltyCloseMode) String() string {
	return protoimpl.X.EnumStringOf(x.Descriptor(), protoreflect.EnumNumber(x))
}

func (RoyaltyCloseMode) Descriptor() protoreflect.EnumDescriptor {
	return file_publira_admin_v1_royalty_proto_enumTypes[0].Descriptor()
}

func (RoyaltyCloseMode) Type() protoreflect.EnumType {
	return &file_publira_admin_v1_royalty_proto_enumTypes[0]
}

func (x RoyaltyCloseMode) Number() protoreflect.EnumNumber {
	return protoreflect.EnumNumber(x)
}

// Deprecated: Use RoyaltyCloseMode.Descriptor instead.
func (RoyaltyCloseMode) EnumDescriptor() ([]byte, []int) {
	return file_publira_admin_v1_royalty_proto_rawDescGZIP(), []int{0}
}

// The tenant's policy for closing completed royalty months.
type RoyaltyConfig struct {
	state     protoimpl.MessageState `protogen:"open.v1"`
	CloseMode RoyaltyCloseMode       `protobuf:"varint,1,opt,name=close_mode,json=closeMode,proto3,enum=publira.admin.v1.RoyaltyCloseMode" json:"close_mode,omitempty"`
	// Day 1–28 of the following month when automatic closing runs. It is set
	// only when close_mode is automatic.
	AutoCloseDay *int32 `protobuf:"varint,2,opt,name=auto_close_day,json=autoCloseDay,proto3,oneof" json:"auto_close_day,omitempty"`
	// When automatic closing was most recently enabled, as RFC 3339.
	AutomaticSince string `protobuf:"bytes,3,opt,name=automatic_since,json=automaticSince,proto3" json:"automatic_since,omitempty"`
	unknownFields  protoimpl.UnknownFields
	sizeCache      protoimpl.SizeCache
}

func (x *RoyaltyConfig) Reset() {
	*x = RoyaltyConfig{}
	mi := &file_publira_admin_v1_royalty_proto_msgTypes[0]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *RoyaltyConfig) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*RoyaltyConfig) ProtoMessage() {}

func (x *RoyaltyConfig) ProtoReflect() protoreflect.Message {
	mi := &file_publira_admin_v1_royalty_proto_msgTypes[0]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use RoyaltyConfig.ProtoReflect.Descriptor instead.
func (*RoyaltyConfig) Descriptor() ([]byte, []int) {
	return file_publira_admin_v1_royalty_proto_rawDescGZIP(), []int{0}
}

func (x *RoyaltyConfig) GetCloseMode() RoyaltyCloseMode {
	if x != nil {
		return x.CloseMode
	}
	return RoyaltyCloseMode_ROYALTY_CLOSE_MODE_UNSPECIFIED
}

func (x *RoyaltyConfig) GetAutoCloseDay() int32 {
	if x != nil && x.AutoCloseDay != nil {
		return *x.AutoCloseDay
	}
	return 0
}

func (x *RoyaltyConfig) GetAutomaticSince() string {
	if x != nil {
		return x.AutomaticSince
	}
	return ""
}

type GetRoyaltyConfigRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Tenant        *v1.TenantContext      `protobuf:"bytes,1,opt,name=tenant,proto3" json:"tenant,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetRoyaltyConfigRequest) Reset() {
	*x = GetRoyaltyConfigRequest{}
	mi := &file_publira_admin_v1_royalty_proto_msgTypes[1]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetRoyaltyConfigRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetRoyaltyConfigRequest) ProtoMessage() {}

func (x *GetRoyaltyConfigRequest) ProtoReflect() protoreflect.Message {
	mi := &file_publira_admin_v1_royalty_proto_msgTypes[1]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetRoyaltyConfigRequest.ProtoReflect.Descriptor instead.
func (*GetRoyaltyConfigRequest) Descriptor() ([]byte, []int) {
	return file_publira_admin_v1_royalty_proto_rawDescGZIP(), []int{1}
}

func (x *GetRoyaltyConfigRequest) GetTenant() *v1.TenantContext {
	if x != nil {
		return x.Tenant
	}
	return nil
}

type GetRoyaltyConfigResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Config        *RoyaltyConfig         `protobuf:"bytes,1,opt,name=config,proto3" json:"config,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetRoyaltyConfigResponse) Reset() {
	*x = GetRoyaltyConfigResponse{}
	mi := &file_publira_admin_v1_royalty_proto_msgTypes[2]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetRoyaltyConfigResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetRoyaltyConfigResponse) ProtoMessage() {}

func (x *GetRoyaltyConfigResponse) ProtoReflect() protoreflect.Message {
	mi := &file_publira_admin_v1_royalty_proto_msgTypes[2]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetRoyaltyConfigResponse.ProtoReflect.Descriptor instead.
func (*GetRoyaltyConfigResponse) Descriptor() ([]byte, []int) {
	return file_publira_admin_v1_royalty_proto_rawDescGZIP(), []int{2}
}

func (x *GetRoyaltyConfigResponse) GetConfig() *RoyaltyConfig {
	if x != nil {
		return x.Config
	}
	return nil
}

type UpdateRoyaltyConfigRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Tenant        *v1.TenantContext      `protobuf:"bytes,1,opt,name=tenant,proto3" json:"tenant,omitempty"`
	CloseMode     RoyaltyCloseMode       `protobuf:"varint,2,opt,name=close_mode,json=closeMode,proto3,enum=publira.admin.v1.RoyaltyCloseMode" json:"close_mode,omitempty"`
	AutoCloseDay  *int32                 `protobuf:"varint,3,opt,name=auto_close_day,json=autoCloseDay,proto3,oneof" json:"auto_close_day,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *UpdateRoyaltyConfigRequest) Reset() {
	*x = UpdateRoyaltyConfigRequest{}
	mi := &file_publira_admin_v1_royalty_proto_msgTypes[3]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *UpdateRoyaltyConfigRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*UpdateRoyaltyConfigRequest) ProtoMessage() {}

func (x *UpdateRoyaltyConfigRequest) ProtoReflect() protoreflect.Message {
	mi := &file_publira_admin_v1_royalty_proto_msgTypes[3]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use UpdateRoyaltyConfigRequest.ProtoReflect.Descriptor instead.
func (*UpdateRoyaltyConfigRequest) Descriptor() ([]byte, []int) {
	return file_publira_admin_v1_royalty_proto_rawDescGZIP(), []int{3}
}

func (x *UpdateRoyaltyConfigRequest) GetTenant() *v1.TenantContext {
	if x != nil {
		return x.Tenant
	}
	return nil
}

func (x *UpdateRoyaltyConfigRequest) GetCloseMode() RoyaltyCloseMode {
	if x != nil {
		return x.CloseMode
	}
	return RoyaltyCloseMode_ROYALTY_CLOSE_MODE_UNSPECIFIED
}

func (x *UpdateRoyaltyConfigRequest) GetAutoCloseDay() int32 {
	if x != nil && x.AutoCloseDay != nil {
		return *x.AutoCloseDay
	}
	return 0
}

type UpdateRoyaltyConfigResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Config        *RoyaltyConfig         `protobuf:"bytes,1,opt,name=config,proto3" json:"config,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *UpdateRoyaltyConfigResponse) Reset() {
	*x = UpdateRoyaltyConfigResponse{}
	mi := &file_publira_admin_v1_royalty_proto_msgTypes[4]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *UpdateRoyaltyConfigResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*UpdateRoyaltyConfigResponse) ProtoMessage() {}

func (x *UpdateRoyaltyConfigResponse) ProtoReflect() protoreflect.Message {
	mi := &file_publira_admin_v1_royalty_proto_msgTypes[4]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use UpdateRoyaltyConfigResponse.ProtoReflect.Descriptor instead.
func (*UpdateRoyaltyConfigResponse) Descriptor() ([]byte, []int) {
	return file_publira_admin_v1_royalty_proto_rawDescGZIP(), []int{4}
}

func (x *UpdateRoyaltyConfigResponse) GetConfig() *RoyaltyConfig {
	if x != nil {
		return x.Config
	}
	return nil
}

// The figures of one month: its own sales, and what its lines pay out.
// Amounts are whole yen.
type RoyaltyStatementTotals struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// What the month sold, each sale counted once however many creators it is
	// credited to. A fully refunded sale is not a sale.
	Gross int64 `protobuf:"varint,1,opt,name=gross,proto3" json:"gross,omitempty"`
	// Partial refunds of the sales in gross.
	Refunded int64 `protobuf:"varint,2,opt,name=refunded,proto3" json:"refunded,omitempty"`
	// The sum of the lines' payouts. The rest of gross net of refunds is the
	// tenant's own.
	Payout        int64 `protobuf:"varint,3,opt,name=payout,proto3" json:"payout,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *RoyaltyStatementTotals) Reset() {
	*x = RoyaltyStatementTotals{}
	mi := &file_publira_admin_v1_royalty_proto_msgTypes[5]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *RoyaltyStatementTotals) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*RoyaltyStatementTotals) ProtoMessage() {}

func (x *RoyaltyStatementTotals) ProtoReflect() protoreflect.Message {
	mi := &file_publira_admin_v1_royalty_proto_msgTypes[5]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use RoyaltyStatementTotals.ProtoReflect.Descriptor instead.
func (*RoyaltyStatementTotals) Descriptor() ([]byte, []int) {
	return file_publira_admin_v1_royalty_proto_rawDescGZIP(), []int{5}
}

func (x *RoyaltyStatementTotals) GetGross() int64 {
	if x != nil {
		return x.Gross
	}
	return 0
}

func (x *RoyaltyStatementTotals) GetRefunded() int64 {
	if x != nil {
		return x.Refunded
	}
	return 0
}

func (x *RoyaltyStatementTotals) GetPayout() int64 {
	if x != nil {
		return x.Payout
	}
	return 0
}

// A closed month.
type RoyaltyStatement struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// The month, as YYYY-MM.
	Period string `protobuf:"bytes,1,opt,name=period,proto3" json:"period,omitempty"`
	// The IANA zone the month was cut in: the tenant's zone at close.
	TimeZone string `protobuf:"bytes,2,opt,name=time_zone,json=timeZone,proto3" json:"time_zone,omitempty"`
	ClosedAt string `protobuf:"bytes,3,opt,name=closed_at,json=closedAt,proto3" json:"closed_at,omitempty"`
	// The account that closed the month. Both empty for a close no user started
	// and once the account is deleted.
	ClosedByUserPublicId string                  `protobuf:"bytes,4,opt,name=closed_by_user_public_id,json=closedByUserPublicId,proto3" json:"closed_by_user_public_id,omitempty"`
	ClosedByUserName     string                  `protobuf:"bytes,5,opt,name=closed_by_user_name,json=closedByUserName,proto3" json:"closed_by_user_name,omitempty"`
	Totals               *RoyaltyStatementTotals `protobuf:"bytes,6,opt,name=totals,proto3" json:"totals,omitempty"`
	unknownFields        protoimpl.UnknownFields
	sizeCache            protoimpl.SizeCache
}

func (x *RoyaltyStatement) Reset() {
	*x = RoyaltyStatement{}
	mi := &file_publira_admin_v1_royalty_proto_msgTypes[6]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *RoyaltyStatement) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*RoyaltyStatement) ProtoMessage() {}

func (x *RoyaltyStatement) ProtoReflect() protoreflect.Message {
	mi := &file_publira_admin_v1_royalty_proto_msgTypes[6]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use RoyaltyStatement.ProtoReflect.Descriptor instead.
func (*RoyaltyStatement) Descriptor() ([]byte, []int) {
	return file_publira_admin_v1_royalty_proto_rawDescGZIP(), []int{6}
}

func (x *RoyaltyStatement) GetPeriod() string {
	if x != nil {
		return x.Period
	}
	return ""
}

func (x *RoyaltyStatement) GetTimeZone() string {
	if x != nil {
		return x.TimeZone
	}
	return ""
}

func (x *RoyaltyStatement) GetClosedAt() string {
	if x != nil {
		return x.ClosedAt
	}
	return ""
}

func (x *RoyaltyStatement) GetClosedByUserPublicId() string {
	if x != nil {
		return x.ClosedByUserPublicId
	}
	return ""
}

func (x *RoyaltyStatement) GetClosedByUserName() string {
	if x != nil {
		return x.ClosedByUserName
	}
	return ""
}

func (x *RoyaltyStatement) GetTotals() *RoyaltyStatementTotals {
	if x != nil {
		return x.Totals
	}
	return nil
}

// One creator's credit on one episode for one role, with that episode's sales
// in the month. Names are as they stood at close; a public ID is empty once the
// row it named has been deleted.
type RoyaltyStatementLine struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Position in the statement, from 1.
	LineNumber      int32  `protobuf:"varint,1,opt,name=line_number,json=lineNumber,proto3" json:"line_number,omitempty"`
	CreatorPublicId string `protobuf:"bytes,2,opt,name=creator_public_id,json=creatorPublicId,proto3" json:"creator_public_id,omitempty"`
	CreatorName     string `protobuf:"bytes,3,opt,name=creator_name,json=creatorName,proto3" json:"creator_name,omitempty"`
	SeriesPublicId  string `protobuf:"bytes,4,opt,name=series_public_id,json=seriesPublicId,proto3" json:"series_public_id,omitempty"`
	SeriesTitle     string `protobuf:"bytes,5,opt,name=series_title,json=seriesTitle,proto3" json:"series_title,omitempty"`
	EpisodePublicId string `protobuf:"bytes,6,opt,name=episode_public_id,json=episodePublicId,proto3" json:"episode_public_id,omitempty"`
	EpisodeTitle    string `protobuf:"bytes,7,opt,name=episode_title,json=episodeTitle,proto3" json:"episode_title,omitempty"`
	// Both empty for a credit that states no role.
	RolePublicId   string `protobuf:"bytes,8,opt,name=role_public_id,json=rolePublicId,proto3" json:"role_public_id,omitempty"`
	RoleName       string `protobuf:"bytes,9,opt,name=role_name,json=roleName,proto3" json:"role_name,omitempty"`
	SaleCount      int32  `protobuf:"varint,10,opt,name=sale_count,json=saleCount,proto3" json:"sale_count,omitempty"`
	GrossAmount    int64  `protobuf:"varint,11,opt,name=gross_amount,json=grossAmount,proto3" json:"gross_amount,omitempty"`
	RefundedAmount int64  `protobuf:"varint,12,opt,name=refunded_amount,json=refundedAmount,proto3" json:"refunded_amount,omitempty"`
	// The credit's share in basis points (10000 is all of it), as applied.
	ShareBps int32 `protobuf:"varint,13,opt,name=share_bps,json=shareBps,proto3" json:"share_bps,omitempty"`
	// (gross_amount - refunded_amount) * share_bps / 10000, floored to whole yen.
	PayoutAmount  int64 `protobuf:"varint,14,opt,name=payout_amount,json=payoutAmount,proto3" json:"payout_amount,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *RoyaltyStatementLine) Reset() {
	*x = RoyaltyStatementLine{}
	mi := &file_publira_admin_v1_royalty_proto_msgTypes[7]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *RoyaltyStatementLine) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*RoyaltyStatementLine) ProtoMessage() {}

func (x *RoyaltyStatementLine) ProtoReflect() protoreflect.Message {
	mi := &file_publira_admin_v1_royalty_proto_msgTypes[7]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use RoyaltyStatementLine.ProtoReflect.Descriptor instead.
func (*RoyaltyStatementLine) Descriptor() ([]byte, []int) {
	return file_publira_admin_v1_royalty_proto_rawDescGZIP(), []int{7}
}

func (x *RoyaltyStatementLine) GetLineNumber() int32 {
	if x != nil {
		return x.LineNumber
	}
	return 0
}

func (x *RoyaltyStatementLine) GetCreatorPublicId() string {
	if x != nil {
		return x.CreatorPublicId
	}
	return ""
}

func (x *RoyaltyStatementLine) GetCreatorName() string {
	if x != nil {
		return x.CreatorName
	}
	return ""
}

func (x *RoyaltyStatementLine) GetSeriesPublicId() string {
	if x != nil {
		return x.SeriesPublicId
	}
	return ""
}

func (x *RoyaltyStatementLine) GetSeriesTitle() string {
	if x != nil {
		return x.SeriesTitle
	}
	return ""
}

func (x *RoyaltyStatementLine) GetEpisodePublicId() string {
	if x != nil {
		return x.EpisodePublicId
	}
	return ""
}

func (x *RoyaltyStatementLine) GetEpisodeTitle() string {
	if x != nil {
		return x.EpisodeTitle
	}
	return ""
}

func (x *RoyaltyStatementLine) GetRolePublicId() string {
	if x != nil {
		return x.RolePublicId
	}
	return ""
}

func (x *RoyaltyStatementLine) GetRoleName() string {
	if x != nil {
		return x.RoleName
	}
	return ""
}

func (x *RoyaltyStatementLine) GetSaleCount() int32 {
	if x != nil {
		return x.SaleCount
	}
	return 0
}

func (x *RoyaltyStatementLine) GetGrossAmount() int64 {
	if x != nil {
		return x.GrossAmount
	}
	return 0
}

func (x *RoyaltyStatementLine) GetRefundedAmount() int64 {
	if x != nil {
		return x.RefundedAmount
	}
	return 0
}

func (x *RoyaltyStatementLine) GetShareBps() int32 {
	if x != nil {
		return x.ShareBps
	}
	return 0
}

func (x *RoyaltyStatementLine) GetPayoutAmount() int64 {
	if x != nil {
		return x.PayoutAmount
	}
	return 0
}

type PreviewRoyaltyStatementRequest struct {
	state  protoimpl.MessageState `protogen:"open.v1"`
	Tenant *v1.TenantContext      `protobuf:"bytes,1,opt,name=tenant,proto3" json:"tenant,omitempty"`
	// YYYY-MM. The current month and months already over are both accepted.
	Period        string `protobuf:"bytes,2,opt,name=period,proto3" json:"period,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *PreviewRoyaltyStatementRequest) Reset() {
	*x = PreviewRoyaltyStatementRequest{}
	mi := &file_publira_admin_v1_royalty_proto_msgTypes[8]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *PreviewRoyaltyStatementRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*PreviewRoyaltyStatementRequest) ProtoMessage() {}

func (x *PreviewRoyaltyStatementRequest) ProtoReflect() protoreflect.Message {
	mi := &file_publira_admin_v1_royalty_proto_msgTypes[8]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use PreviewRoyaltyStatementRequest.ProtoReflect.Descriptor instead.
func (*PreviewRoyaltyStatementRequest) Descriptor() ([]byte, []int) {
	return file_publira_admin_v1_royalty_proto_rawDescGZIP(), []int{8}
}

func (x *PreviewRoyaltyStatementRequest) GetTenant() *v1.TenantContext {
	if x != nil {
		return x.Tenant
	}
	return nil
}

func (x *PreviewRoyaltyStatementRequest) GetPeriod() string {
	if x != nil {
		return x.Period
	}
	return ""
}

type PreviewRoyaltyStatementResponse struct {
	state         protoimpl.MessageState  `protogen:"open.v1"`
	Period        string                  `protobuf:"bytes,1,opt,name=period,proto3" json:"period,omitempty"`
	TimeZone      string                  `protobuf:"bytes,2,opt,name=time_zone,json=timeZone,proto3" json:"time_zone,omitempty"`
	Totals        *RoyaltyStatementTotals `protobuf:"bytes,3,opt,name=totals,proto3" json:"totals,omitempty"`
	Lines         []*RoyaltyStatementLine `protobuf:"bytes,4,rep,name=lines,proto3" json:"lines,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *PreviewRoyaltyStatementResponse) Reset() {
	*x = PreviewRoyaltyStatementResponse{}
	mi := &file_publira_admin_v1_royalty_proto_msgTypes[9]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *PreviewRoyaltyStatementResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*PreviewRoyaltyStatementResponse) ProtoMessage() {}

func (x *PreviewRoyaltyStatementResponse) ProtoReflect() protoreflect.Message {
	mi := &file_publira_admin_v1_royalty_proto_msgTypes[9]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use PreviewRoyaltyStatementResponse.ProtoReflect.Descriptor instead.
func (*PreviewRoyaltyStatementResponse) Descriptor() ([]byte, []int) {
	return file_publira_admin_v1_royalty_proto_rawDescGZIP(), []int{9}
}

func (x *PreviewRoyaltyStatementResponse) GetPeriod() string {
	if x != nil {
		return x.Period
	}
	return ""
}

func (x *PreviewRoyaltyStatementResponse) GetTimeZone() string {
	if x != nil {
		return x.TimeZone
	}
	return ""
}

func (x *PreviewRoyaltyStatementResponse) GetTotals() *RoyaltyStatementTotals {
	if x != nil {
		return x.Totals
	}
	return nil
}

func (x *PreviewRoyaltyStatementResponse) GetLines() []*RoyaltyStatementLine {
	if x != nil {
		return x.Lines
	}
	return nil
}

type CloseRoyaltyStatementRequest struct {
	state  protoimpl.MessageState `protogen:"open.v1"`
	Tenant *v1.TenantContext      `protobuf:"bytes,1,opt,name=tenant,proto3" json:"tenant,omitempty"`
	// YYYY-MM of a month that is already over in the tenant's zone.
	Period        string `protobuf:"bytes,2,opt,name=period,proto3" json:"period,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *CloseRoyaltyStatementRequest) Reset() {
	*x = CloseRoyaltyStatementRequest{}
	mi := &file_publira_admin_v1_royalty_proto_msgTypes[10]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *CloseRoyaltyStatementRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*CloseRoyaltyStatementRequest) ProtoMessage() {}

func (x *CloseRoyaltyStatementRequest) ProtoReflect() protoreflect.Message {
	mi := &file_publira_admin_v1_royalty_proto_msgTypes[10]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use CloseRoyaltyStatementRequest.ProtoReflect.Descriptor instead.
func (*CloseRoyaltyStatementRequest) Descriptor() ([]byte, []int) {
	return file_publira_admin_v1_royalty_proto_rawDescGZIP(), []int{10}
}

func (x *CloseRoyaltyStatementRequest) GetTenant() *v1.TenantContext {
	if x != nil {
		return x.Tenant
	}
	return nil
}

func (x *CloseRoyaltyStatementRequest) GetPeriod() string {
	if x != nil {
		return x.Period
	}
	return ""
}

type CloseRoyaltyStatementResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Statement     *RoyaltyStatement      `protobuf:"bytes,1,opt,name=statement,proto3" json:"statement,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *CloseRoyaltyStatementResponse) Reset() {
	*x = CloseRoyaltyStatementResponse{}
	mi := &file_publira_admin_v1_royalty_proto_msgTypes[11]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *CloseRoyaltyStatementResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*CloseRoyaltyStatementResponse) ProtoMessage() {}

func (x *CloseRoyaltyStatementResponse) ProtoReflect() protoreflect.Message {
	mi := &file_publira_admin_v1_royalty_proto_msgTypes[11]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use CloseRoyaltyStatementResponse.ProtoReflect.Descriptor instead.
func (*CloseRoyaltyStatementResponse) Descriptor() ([]byte, []int) {
	return file_publira_admin_v1_royalty_proto_rawDescGZIP(), []int{11}
}

func (x *CloseRoyaltyStatementResponse) GetStatement() *RoyaltyStatement {
	if x != nil {
		return x.Statement
	}
	return nil
}

// Cursor pagination. Field shape and token rules: proto/README.md.
type ListRoyaltyStatementsRequest struct {
	state  protoimpl.MessageState `protogen:"open.v1"`
	Tenant *v1.TenantContext      `protobuf:"bytes,1,opt,name=tenant,proto3" json:"tenant,omitempty"`
	// Max items in one page. <= 0 or > 100 falls back to 20.
	Limit int32 `protobuf:"varint,2,opt,name=limit,proto3" json:"limit,omitempty"`
	// Opaque token from a previous response. Empty for the first page.
	Token         string `protobuf:"bytes,3,opt,name=token,proto3" json:"token,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ListRoyaltyStatementsRequest) Reset() {
	*x = ListRoyaltyStatementsRequest{}
	mi := &file_publira_admin_v1_royalty_proto_msgTypes[12]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ListRoyaltyStatementsRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ListRoyaltyStatementsRequest) ProtoMessage() {}

func (x *ListRoyaltyStatementsRequest) ProtoReflect() protoreflect.Message {
	mi := &file_publira_admin_v1_royalty_proto_msgTypes[12]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ListRoyaltyStatementsRequest.ProtoReflect.Descriptor instead.
func (*ListRoyaltyStatementsRequest) Descriptor() ([]byte, []int) {
	return file_publira_admin_v1_royalty_proto_rawDescGZIP(), []int{12}
}

func (x *ListRoyaltyStatementsRequest) GetTenant() *v1.TenantContext {
	if x != nil {
		return x.Tenant
	}
	return nil
}

func (x *ListRoyaltyStatementsRequest) GetLimit() int32 {
	if x != nil {
		return x.Limit
	}
	return 0
}

func (x *ListRoyaltyStatementsRequest) GetToken() string {
	if x != nil {
		return x.Token
	}
	return ""
}

type ListRoyaltyStatementsResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Statements    []*RoyaltyStatement    `protobuf:"bytes,1,rep,name=statements,proto3" json:"statements,omitempty"`
	PreviousToken string                 `protobuf:"bytes,2,opt,name=previous_token,json=previousToken,proto3" json:"previous_token,omitempty"`
	NextToken     string                 `protobuf:"bytes,3,opt,name=next_token,json=nextToken,proto3" json:"next_token,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ListRoyaltyStatementsResponse) Reset() {
	*x = ListRoyaltyStatementsResponse{}
	mi := &file_publira_admin_v1_royalty_proto_msgTypes[13]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ListRoyaltyStatementsResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ListRoyaltyStatementsResponse) ProtoMessage() {}

func (x *ListRoyaltyStatementsResponse) ProtoReflect() protoreflect.Message {
	mi := &file_publira_admin_v1_royalty_proto_msgTypes[13]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ListRoyaltyStatementsResponse.ProtoReflect.Descriptor instead.
func (*ListRoyaltyStatementsResponse) Descriptor() ([]byte, []int) {
	return file_publira_admin_v1_royalty_proto_rawDescGZIP(), []int{13}
}

func (x *ListRoyaltyStatementsResponse) GetStatements() []*RoyaltyStatement {
	if x != nil {
		return x.Statements
	}
	return nil
}

func (x *ListRoyaltyStatementsResponse) GetPreviousToken() string {
	if x != nil {
		return x.PreviousToken
	}
	return ""
}

func (x *ListRoyaltyStatementsResponse) GetNextToken() string {
	if x != nil {
		return x.NextToken
	}
	return ""
}

// Cursor pagination over the lines. Field shape and token rules:
// proto/README.md.
type GetRoyaltyStatementRequest struct {
	state  protoimpl.MessageState `protogen:"open.v1"`
	Tenant *v1.TenantContext      `protobuf:"bytes,1,opt,name=tenant,proto3" json:"tenant,omitempty"`
	// YYYY-MM.
	Period string `protobuf:"bytes,2,opt,name=period,proto3" json:"period,omitempty"`
	// Max lines in one page. <= 0 or > 100 falls back to 20.
	Limit int32 `protobuf:"varint,3,opt,name=limit,proto3" json:"limit,omitempty"`
	// Opaque token from a previous response. Empty for the first page. A token
	// issued for another period is invalid_argument.
	Token         string `protobuf:"bytes,4,opt,name=token,proto3" json:"token,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetRoyaltyStatementRequest) Reset() {
	*x = GetRoyaltyStatementRequest{}
	mi := &file_publira_admin_v1_royalty_proto_msgTypes[14]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetRoyaltyStatementRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetRoyaltyStatementRequest) ProtoMessage() {}

func (x *GetRoyaltyStatementRequest) ProtoReflect() protoreflect.Message {
	mi := &file_publira_admin_v1_royalty_proto_msgTypes[14]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetRoyaltyStatementRequest.ProtoReflect.Descriptor instead.
func (*GetRoyaltyStatementRequest) Descriptor() ([]byte, []int) {
	return file_publira_admin_v1_royalty_proto_rawDescGZIP(), []int{14}
}

func (x *GetRoyaltyStatementRequest) GetTenant() *v1.TenantContext {
	if x != nil {
		return x.Tenant
	}
	return nil
}

func (x *GetRoyaltyStatementRequest) GetPeriod() string {
	if x != nil {
		return x.Period
	}
	return ""
}

func (x *GetRoyaltyStatementRequest) GetLimit() int32 {
	if x != nil {
		return x.Limit
	}
	return 0
}

func (x *GetRoyaltyStatementRequest) GetToken() string {
	if x != nil {
		return x.Token
	}
	return ""
}

type GetRoyaltyStatementResponse struct {
	state         protoimpl.MessageState  `protogen:"open.v1"`
	Statement     *RoyaltyStatement       `protobuf:"bytes,1,opt,name=statement,proto3" json:"statement,omitempty"`
	Lines         []*RoyaltyStatementLine `protobuf:"bytes,2,rep,name=lines,proto3" json:"lines,omitempty"`
	PreviousToken string                  `protobuf:"bytes,3,opt,name=previous_token,json=previousToken,proto3" json:"previous_token,omitempty"`
	NextToken     string                  `protobuf:"bytes,4,opt,name=next_token,json=nextToken,proto3" json:"next_token,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetRoyaltyStatementResponse) Reset() {
	*x = GetRoyaltyStatementResponse{}
	mi := &file_publira_admin_v1_royalty_proto_msgTypes[15]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetRoyaltyStatementResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetRoyaltyStatementResponse) ProtoMessage() {}

func (x *GetRoyaltyStatementResponse) ProtoReflect() protoreflect.Message {
	mi := &file_publira_admin_v1_royalty_proto_msgTypes[15]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetRoyaltyStatementResponse.ProtoReflect.Descriptor instead.
func (*GetRoyaltyStatementResponse) Descriptor() ([]byte, []int) {
	return file_publira_admin_v1_royalty_proto_rawDescGZIP(), []int{15}
}

func (x *GetRoyaltyStatementResponse) GetStatement() *RoyaltyStatement {
	if x != nil {
		return x.Statement
	}
	return nil
}

func (x *GetRoyaltyStatementResponse) GetLines() []*RoyaltyStatementLine {
	if x != nil {
		return x.Lines
	}
	return nil
}

func (x *GetRoyaltyStatementResponse) GetPreviousToken() string {
	if x != nil {
		return x.PreviousToken
	}
	return ""
}

func (x *GetRoyaltyStatementResponse) GetNextToken() string {
	if x != nil {
		return x.NextToken
	}
	return ""
}

type ExportRoyaltyStatementRequest struct {
	state  protoimpl.MessageState `protogen:"open.v1"`
	Tenant *v1.TenantContext      `protobuf:"bytes,1,opt,name=tenant,proto3" json:"tenant,omitempty"`
	// YYYY-MM of a closed month.
	Period        string `protobuf:"bytes,2,opt,name=period,proto3" json:"period,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ExportRoyaltyStatementRequest) Reset() {
	*x = ExportRoyaltyStatementRequest{}
	mi := &file_publira_admin_v1_royalty_proto_msgTypes[16]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ExportRoyaltyStatementRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ExportRoyaltyStatementRequest) ProtoMessage() {}

func (x *ExportRoyaltyStatementRequest) ProtoReflect() protoreflect.Message {
	mi := &file_publira_admin_v1_royalty_proto_msgTypes[16]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ExportRoyaltyStatementRequest.ProtoReflect.Descriptor instead.
func (*ExportRoyaltyStatementRequest) Descriptor() ([]byte, []int) {
	return file_publira_admin_v1_royalty_proto_rawDescGZIP(), []int{16}
}

func (x *ExportRoyaltyStatementRequest) GetTenant() *v1.TenantContext {
	if x != nil {
		return x.Tenant
	}
	return nil
}

func (x *ExportRoyaltyStatementRequest) GetPeriod() string {
	if x != nil {
		return x.Period
	}
	return ""
}

type ExportRoyaltyStatementResponse struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// The statement as CSV: UTF-8 with a byte order mark, CRLF line endings,
	// RFC 4180 quoting. A header row, then one row per line in line order, with
	// no summary row; the totals are those of GetRoyaltyStatement. Columns:
	//
	//	period            YYYY-MM
	//	creator_id        the creator's ID, as closed
	//	creator_name      as closed
	//	role_id           the role's ID, as closed; empty for a credit with no role
	//	role_name         as closed; empty for a credit with no role
	//	series_id         the series' ID, as closed
	//	series_title      as closed
	//	episode_id        the episode's ID, as closed
	//	episode_title     as closed
	//	sale_count        integer
	//	gross_amount      whole yen, no currency symbol
	//	refunded_amount   whole yen, no currency symbol
	//	share_percent     the share with two decimals, 12.50 for 1250 bps
	//	payout_amount     whole yen, no currency symbol
	//
	// The IDs are the rows' internal IDs, not their public IDs, and they stay
	// on the line after the row is deleted.
	Csv           []byte `protobuf:"bytes,1,opt,name=csv,proto3" json:"csv,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ExportRoyaltyStatementResponse) Reset() {
	*x = ExportRoyaltyStatementResponse{}
	mi := &file_publira_admin_v1_royalty_proto_msgTypes[17]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ExportRoyaltyStatementResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ExportRoyaltyStatementResponse) ProtoMessage() {}

func (x *ExportRoyaltyStatementResponse) ProtoReflect() protoreflect.Message {
	mi := &file_publira_admin_v1_royalty_proto_msgTypes[17]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ExportRoyaltyStatementResponse.ProtoReflect.Descriptor instead.
func (*ExportRoyaltyStatementResponse) Descriptor() ([]byte, []int) {
	return file_publira_admin_v1_royalty_proto_rawDescGZIP(), []int{17}
}

func (x *ExportRoyaltyStatementResponse) GetCsv() []byte {
	if x != nil {
		return x.Csv
	}
	return nil
}

var File_publira_admin_v1_royalty_proto protoreflect.FileDescriptor

const file_publira_admin_v1_royalty_proto_rawDesc = "" +
	"\n" +
	"\x1epublira/admin/v1/royalty.proto\x12\x10publira.admin.v1\x1a\x1cpublira/types/v1/types.proto\"\xb9\x01\n" +
	"\rRoyaltyConfig\x12A\n" +
	"\n" +
	"close_mode\x18\x01 \x01(\x0e2\".publira.admin.v1.RoyaltyCloseModeR\tcloseMode\x12)\n" +
	"\x0eauto_close_day\x18\x02 \x01(\x05H\x00R\fautoCloseDay\x88\x01\x01\x12'\n" +
	"\x0fautomatic_since\x18\x03 \x01(\tR\x0eautomaticSinceB\x11\n" +
	"\x0f_auto_close_day\"R\n" +
	"\x17GetRoyaltyConfigRequest\x127\n" +
	"\x06tenant\x18\x01 \x01(\v2\x1f.publira.types.v1.TenantContextR\x06tenant\"S\n" +
	"\x18GetRoyaltyConfigResponse\x127\n" +
	"\x06config\x18\x01 \x01(\v2\x1f.publira.admin.v1.RoyaltyConfigR\x06config\"\xd6\x01\n" +
	"\x1aUpdateRoyaltyConfigRequest\x127\n" +
	"\x06tenant\x18\x01 \x01(\v2\x1f.publira.types.v1.TenantContextR\x06tenant\x12A\n" +
	"\n" +
	"close_mode\x18\x02 \x01(\x0e2\".publira.admin.v1.RoyaltyCloseModeR\tcloseMode\x12)\n" +
	"\x0eauto_close_day\x18\x03 \x01(\x05H\x00R\fautoCloseDay\x88\x01\x01B\x11\n" +
	"\x0f_auto_close_day\"V\n" +
	"\x1bUpdateRoyaltyConfigResponse\x127\n" +
	"\x06config\x18\x01 \x01(\v2\x1f.publira.admin.v1.RoyaltyConfigR\x06config\"b\n" +
	"\x16RoyaltyStatementTotals\x12\x14\n" +
	"\x05gross\x18\x01 \x01(\x03R\x05gross\x12\x1a\n" +
	"\brefunded\x18\x02 \x01(\x03R\brefunded\x12\x16\n" +
	"\x06payout\x18\x03 \x01(\x03R\x06payout\"\x8d\x02\n" +
	"\x10RoyaltyStatement\x12\x16\n" +
	"\x06period\x18\x01 \x01(\tR\x06period\x12\x1b\n" +
	"\ttime_zone\x18\x02 \x01(\tR\btimeZone\x12\x1b\n" +
	"\tclosed_at\x18\x03 \x01(\tR\bclosedAt\x126\n" +
	"\x18closed_by_user_public_id\x18\x04 \x01(\tR\x14closedByUserPublicId\x12-\n" +
	"\x13closed_by_user_name\x18\x05 \x01(\tR\x10closedByUserName\x12@\n" +
	"\x06totals\x18\x06 \x01(\v2(.publira.admin.v1.RoyaltyStatementTotalsR\x06totals\"\x94\x04\n" +
	"\x14RoyaltyStatementLine\x12\x1f\n" +
	"\vline_number\x18\x01 \x01(\x05R\n" +
	"lineNumber\x12*\n" +
	"\x11creator_public_id\x18\x02 \x01(\tR\x0fcreatorPublicId\x12!\n" +
	"\fcreator_name\x18\x03 \x01(\tR\vcreatorName\x12(\n" +
	"\x10series_public_id\x18\x04 \x01(\tR\x0eseriesPublicId\x12!\n" +
	"\fseries_title\x18\x05 \x01(\tR\vseriesTitle\x12*\n" +
	"\x11episode_public_id\x18\x06 \x01(\tR\x0fepisodePublicId\x12#\n" +
	"\repisode_title\x18\a \x01(\tR\fepisodeTitle\x12$\n" +
	"\x0erole_public_id\x18\b \x01(\tR\frolePublicId\x12\x1b\n" +
	"\trole_name\x18\t \x01(\tR\broleName\x12\x1d\n" +
	"\n" +
	"sale_count\x18\n" +
	" \x01(\x05R\tsaleCount\x12!\n" +
	"\fgross_amount\x18\v \x01(\x03R\vgrossAmount\x12'\n" +
	"\x0frefunded_amount\x18\f \x01(\x03R\x0erefundedAmount\x12\x1b\n" +
	"\tshare_bps\x18\r \x01(\x05R\bshareBps\x12#\n" +
	"\rpayout_amount\x18\x0e \x01(\x03R\fpayoutAmount\"q\n" +
	"\x1ePreviewRoyaltyStatementRequest\x127\n" +
	"\x06tenant\x18\x01 \x01(\v2\x1f.publira.types.v1.TenantContextR\x06tenant\x12\x16\n" +
	"\x06period\x18\x02 \x01(\tR\x06period\"\xd6\x01\n" +
	"\x1fPreviewRoyaltyStatementResponse\x12\x16\n" +
	"\x06period\x18\x01 \x01(\tR\x06period\x12\x1b\n" +
	"\ttime_zone\x18\x02 \x01(\tR\btimeZone\x12@\n" +
	"\x06totals\x18\x03 \x01(\v2(.publira.admin.v1.RoyaltyStatementTotalsR\x06totals\x12<\n" +
	"\x05lines\x18\x04 \x03(\v2&.publira.admin.v1.RoyaltyStatementLineR\x05lines\"o\n" +
	"\x1cCloseRoyaltyStatementRequest\x127\n" +
	"\x06tenant\x18\x01 \x01(\v2\x1f.publira.types.v1.TenantContextR\x06tenant\x12\x16\n" +
	"\x06period\x18\x02 \x01(\tR\x06period\"a\n" +
	"\x1dCloseRoyaltyStatementResponse\x12@\n" +
	"\tstatement\x18\x01 \x01(\v2\".publira.admin.v1.RoyaltyStatementR\tstatement\"\x83\x01\n" +
	"\x1cListRoyaltyStatementsRequest\x127\n" +
	"\x06tenant\x18\x01 \x01(\v2\x1f.publira.types.v1.TenantContextR\x06tenant\x12\x14\n" +
	"\x05limit\x18\x02 \x01(\x05R\x05limit\x12\x14\n" +
	"\x05token\x18\x03 \x01(\tR\x05token\"\xa9\x01\n" +
	"\x1dListRoyaltyStatementsResponse\x12B\n" +
	"\n" +
	"statements\x18\x01 \x03(\v2\".publira.admin.v1.RoyaltyStatementR\n" +
	"statements\x12%\n" +
	"\x0eprevious_token\x18\x02 \x01(\tR\rpreviousToken\x12\x1d\n" +
	"\n" +
	"next_token\x18\x03 \x01(\tR\tnextToken\"\x99\x01\n" +
	"\x1aGetRoyaltyStatementRequest\x127\n" +
	"\x06tenant\x18\x01 \x01(\v2\x1f.publira.types.v1.TenantContextR\x06tenant\x12\x16\n" +
	"\x06period\x18\x02 \x01(\tR\x06period\x12\x14\n" +
	"\x05limit\x18\x03 \x01(\x05R\x05limit\x12\x14\n" +
	"\x05token\x18\x04 \x01(\tR\x05token\"\xe3\x01\n" +
	"\x1bGetRoyaltyStatementResponse\x12@\n" +
	"\tstatement\x18\x01 \x01(\v2\".publira.admin.v1.RoyaltyStatementR\tstatement\x12<\n" +
	"\x05lines\x18\x02 \x03(\v2&.publira.admin.v1.RoyaltyStatementLineR\x05lines\x12%\n" +
	"\x0eprevious_token\x18\x03 \x01(\tR\rpreviousToken\x12\x1d\n" +
	"\n" +
	"next_token\x18\x04 \x01(\tR\tnextToken\"p\n" +
	"\x1dExportRoyaltyStatementRequest\x127\n" +
	"\x06tenant\x18\x01 \x01(\v2\x1f.publira.types.v1.TenantContextR\x06tenant\x12\x16\n" +
	"\x06period\x18\x02 \x01(\tR\x06period\"2\n" +
	"\x1eExportRoyaltyStatementResponse\x12\x10\n" +
	"\x03csv\x18\x01 \x01(\fR\x03csv*w\n" +
	"\x10RoyaltyCloseMode\x12\"\n" +
	"\x1eROYALTY_CLOSE_MODE_UNSPECIFIED\x10\x00\x12\x1d\n" +
	"\x19ROYALTY_CLOSE_MODE_MANUAL\x10\x01\x12 \n" +
	"\x1cROYALTY_CLOSE_MODE_AUTOMATIC\x10\x022\xe8\x06\n" +
	"\x13AdminRoyaltyService\x12k\n" +
	"\x10GetRoyaltyConfig\x12).publira.admin.v1.GetRoyaltyConfigRequest\x1a*.publira.admin.v1.GetRoyaltyConfigResponse\"\x00\x12t\n" +
	"\x13UpdateRoyaltyConfig\x12,.publira.admin.v1.UpdateRoyaltyConfigRequest\x1a-.publira.admin.v1.UpdateRoyaltyConfigResponse\"\x00\x12\x80\x01\n" +
	"\x17PreviewRoyaltyStatement\x120.publira.admin.v1.PreviewRoyaltyStatementRequest\x1a1.publira.admin.v1.PreviewRoyaltyStatementResponse\"\x00\x12z\n" +
	"\x15CloseRoyaltyStatement\x12..publira.admin.v1.CloseRoyaltyStatementRequest\x1a/.publira.admin.v1.CloseRoyaltyStatementResponse\"\x00\x12z\n" +
	"\x15ListRoyaltyStatements\x12..publira.admin.v1.ListRoyaltyStatementsRequest\x1a/.publira.admin.v1.ListRoyaltyStatementsResponse\"\x00\x12t\n" +
	"\x13GetRoyaltyStatement\x12,.publira.admin.v1.GetRoyaltyStatementRequest\x1a-.publira.admin.v1.GetRoyaltyStatementResponse\"\x00\x12}\n" +
	"\x16ExportRoyaltyStatement\x12/.publira.admin.v1.ExportRoyaltyStatementRequest\x1a0.publira.admin.v1.ExportRoyaltyStatementResponse\"\x00BVZTgithub.com/publira/publira/server/internal/proto/gen/publira/admin/v1;publiraadminv1b\x06proto3"

var (
	file_publira_admin_v1_royalty_proto_rawDescOnce sync.Once
	file_publira_admin_v1_royalty_proto_rawDescData []byte
)

func file_publira_admin_v1_royalty_proto_rawDescGZIP() []byte {
	file_publira_admin_v1_royalty_proto_rawDescOnce.Do(func() {
		file_publira_admin_v1_royalty_proto_rawDescData = protoimpl.X.CompressGZIP(unsafe.Slice(unsafe.StringData(file_publira_admin_v1_royalty_proto_rawDesc), len(file_publira_admin_v1_royalty_proto_rawDesc)))
	})
	return file_publira_admin_v1_royalty_proto_rawDescData
}

var file_publira_admin_v1_royalty_proto_enumTypes = make([]protoimpl.EnumInfo, 1)
var file_publira_admin_v1_royalty_proto_msgTypes = make([]protoimpl.MessageInfo, 18)
var file_publira_admin_v1_royalty_proto_goTypes = []any{
	(RoyaltyCloseMode)(0),                   // 0: publira.admin.v1.RoyaltyCloseMode
	(*RoyaltyConfig)(nil),                   // 1: publira.admin.v1.RoyaltyConfig
	(*GetRoyaltyConfigRequest)(nil),         // 2: publira.admin.v1.GetRoyaltyConfigRequest
	(*GetRoyaltyConfigResponse)(nil),        // 3: publira.admin.v1.GetRoyaltyConfigResponse
	(*UpdateRoyaltyConfigRequest)(nil),      // 4: publira.admin.v1.UpdateRoyaltyConfigRequest
	(*UpdateRoyaltyConfigResponse)(nil),     // 5: publira.admin.v1.UpdateRoyaltyConfigResponse
	(*RoyaltyStatementTotals)(nil),          // 6: publira.admin.v1.RoyaltyStatementTotals
	(*RoyaltyStatement)(nil),                // 7: publira.admin.v1.RoyaltyStatement
	(*RoyaltyStatementLine)(nil),            // 8: publira.admin.v1.RoyaltyStatementLine
	(*PreviewRoyaltyStatementRequest)(nil),  // 9: publira.admin.v1.PreviewRoyaltyStatementRequest
	(*PreviewRoyaltyStatementResponse)(nil), // 10: publira.admin.v1.PreviewRoyaltyStatementResponse
	(*CloseRoyaltyStatementRequest)(nil),    // 11: publira.admin.v1.CloseRoyaltyStatementRequest
	(*CloseRoyaltyStatementResponse)(nil),   // 12: publira.admin.v1.CloseRoyaltyStatementResponse
	(*ListRoyaltyStatementsRequest)(nil),    // 13: publira.admin.v1.ListRoyaltyStatementsRequest
	(*ListRoyaltyStatementsResponse)(nil),   // 14: publira.admin.v1.ListRoyaltyStatementsResponse
	(*GetRoyaltyStatementRequest)(nil),      // 15: publira.admin.v1.GetRoyaltyStatementRequest
	(*GetRoyaltyStatementResponse)(nil),     // 16: publira.admin.v1.GetRoyaltyStatementResponse
	(*ExportRoyaltyStatementRequest)(nil),   // 17: publira.admin.v1.ExportRoyaltyStatementRequest
	(*ExportRoyaltyStatementResponse)(nil),  // 18: publira.admin.v1.ExportRoyaltyStatementResponse
	(*v1.TenantContext)(nil),                // 19: publira.types.v1.TenantContext
}
var file_publira_admin_v1_royalty_proto_depIdxs = []int32{
	0,  // 0: publira.admin.v1.RoyaltyConfig.close_mode:type_name -> publira.admin.v1.RoyaltyCloseMode
	19, // 1: publira.admin.v1.GetRoyaltyConfigRequest.tenant:type_name -> publira.types.v1.TenantContext
	1,  // 2: publira.admin.v1.GetRoyaltyConfigResponse.config:type_name -> publira.admin.v1.RoyaltyConfig
	19, // 3: publira.admin.v1.UpdateRoyaltyConfigRequest.tenant:type_name -> publira.types.v1.TenantContext
	0,  // 4: publira.admin.v1.UpdateRoyaltyConfigRequest.close_mode:type_name -> publira.admin.v1.RoyaltyCloseMode
	1,  // 5: publira.admin.v1.UpdateRoyaltyConfigResponse.config:type_name -> publira.admin.v1.RoyaltyConfig
	6,  // 6: publira.admin.v1.RoyaltyStatement.totals:type_name -> publira.admin.v1.RoyaltyStatementTotals
	19, // 7: publira.admin.v1.PreviewRoyaltyStatementRequest.tenant:type_name -> publira.types.v1.TenantContext
	6,  // 8: publira.admin.v1.PreviewRoyaltyStatementResponse.totals:type_name -> publira.admin.v1.RoyaltyStatementTotals
	8,  // 9: publira.admin.v1.PreviewRoyaltyStatementResponse.lines:type_name -> publira.admin.v1.RoyaltyStatementLine
	19, // 10: publira.admin.v1.CloseRoyaltyStatementRequest.tenant:type_name -> publira.types.v1.TenantContext
	7,  // 11: publira.admin.v1.CloseRoyaltyStatementResponse.statement:type_name -> publira.admin.v1.RoyaltyStatement
	19, // 12: publira.admin.v1.ListRoyaltyStatementsRequest.tenant:type_name -> publira.types.v1.TenantContext
	7,  // 13: publira.admin.v1.ListRoyaltyStatementsResponse.statements:type_name -> publira.admin.v1.RoyaltyStatement
	19, // 14: publira.admin.v1.GetRoyaltyStatementRequest.tenant:type_name -> publira.types.v1.TenantContext
	7,  // 15: publira.admin.v1.GetRoyaltyStatementResponse.statement:type_name -> publira.admin.v1.RoyaltyStatement
	8,  // 16: publira.admin.v1.GetRoyaltyStatementResponse.lines:type_name -> publira.admin.v1.RoyaltyStatementLine
	19, // 17: publira.admin.v1.ExportRoyaltyStatementRequest.tenant:type_name -> publira.types.v1.TenantContext
	2,  // 18: publira.admin.v1.AdminRoyaltyService.GetRoyaltyConfig:input_type -> publira.admin.v1.GetRoyaltyConfigRequest
	4,  // 19: publira.admin.v1.AdminRoyaltyService.UpdateRoyaltyConfig:input_type -> publira.admin.v1.UpdateRoyaltyConfigRequest
	9,  // 20: publira.admin.v1.AdminRoyaltyService.PreviewRoyaltyStatement:input_type -> publira.admin.v1.PreviewRoyaltyStatementRequest
	11, // 21: publira.admin.v1.AdminRoyaltyService.CloseRoyaltyStatement:input_type -> publira.admin.v1.CloseRoyaltyStatementRequest
	13, // 22: publira.admin.v1.AdminRoyaltyService.ListRoyaltyStatements:input_type -> publira.admin.v1.ListRoyaltyStatementsRequest
	15, // 23: publira.admin.v1.AdminRoyaltyService.GetRoyaltyStatement:input_type -> publira.admin.v1.GetRoyaltyStatementRequest
	17, // 24: publira.admin.v1.AdminRoyaltyService.ExportRoyaltyStatement:input_type -> publira.admin.v1.ExportRoyaltyStatementRequest
	3,  // 25: publira.admin.v1.AdminRoyaltyService.GetRoyaltyConfig:output_type -> publira.admin.v1.GetRoyaltyConfigResponse
	5,  // 26: publira.admin.v1.AdminRoyaltyService.UpdateRoyaltyConfig:output_type -> publira.admin.v1.UpdateRoyaltyConfigResponse
	10, // 27: publira.admin.v1.AdminRoyaltyService.PreviewRoyaltyStatement:output_type -> publira.admin.v1.PreviewRoyaltyStatementResponse
	12, // 28: publira.admin.v1.AdminRoyaltyService.CloseRoyaltyStatement:output_type -> publira.admin.v1.CloseRoyaltyStatementResponse
	14, // 29: publira.admin.v1.AdminRoyaltyService.ListRoyaltyStatements:output_type -> publira.admin.v1.ListRoyaltyStatementsResponse
	16, // 30: publira.admin.v1.AdminRoyaltyService.GetRoyaltyStatement:output_type -> publira.admin.v1.GetRoyaltyStatementResponse
	18, // 31: publira.admin.v1.AdminRoyaltyService.ExportRoyaltyStatement:output_type -> publira.admin.v1.ExportRoyaltyStatementResponse
	25, // [25:32] is the sub-list for method output_type
	18, // [18:25] is the sub-list for method input_type
	18, // [18:18] is the sub-list for extension type_name
	18, // [18:18] is the sub-list for extension extendee
	0,  // [0:18] is the sub-list for field type_name
}

func init() { file_publira_admin_v1_royalty_proto_init() }
func file_publira_admin_v1_royalty_proto_init() {
	if File_publira_admin_v1_royalty_proto != nil {
		return
	}
	file_publira_admin_v1_royalty_proto_msgTypes[0].OneofWrappers = []any{}
	file_publira_admin_v1_royalty_proto_msgTypes[3].OneofWrappers = []any{}
	type x struct{}
	out := protoimpl.TypeBuilder{
		File: protoimpl.DescBuilder{
			GoPackagePath: reflect.TypeOf(x{}).PkgPath(),
			RawDescriptor: unsafe.Slice(unsafe.StringData(file_publira_admin_v1_royalty_proto_rawDesc), len(file_publira_admin_v1_royalty_proto_rawDesc)),
			NumEnums:      1,
			NumMessages:   18,
			NumExtensions: 0,
			NumServices:   1,
		},
		GoTypes:           file_publira_admin_v1_royalty_proto_goTypes,
		DependencyIndexes: file_publira_admin_v1_royalty_proto_depIdxs,
		EnumInfos:         file_publira_admin_v1_royalty_proto_enumTypes,
		MessageInfos:      file_publira_admin_v1_royalty_proto_msgTypes,
	}.Build()
	File_publira_admin_v1_royalty_proto = out.File
	file_publira_admin_v1_royalty_proto_goTypes = nil
	file_publira_admin_v1_royalty_proto_depIdxs = nil
}
