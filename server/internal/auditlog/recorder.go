...
InsertAdminAuditLog(ctx context.Context, arg dbmodels.InsertAdminAuditLogParams) error
}
...
ActorRole         string
TenantPublicID    string // empty when there is no tenant context
Action            string
TargetType        string // e.g. "series", "episode", "tenant", "operator", "user"
TargetID          string // public ID of the affected resource
...
// Record writes an audit entry. A DB error is logged but does not propagate.
func (r *Recorder) Record(ctx context.Context, e Entry) {
r.logger.InfoContext(ctx, "audit",
"actor_user_public_id", e.ActorUserPublicID,
"actor_role", e.ActorRole,
"tenant_public_id", e.TenantPublicID,
"action", e.Action,
...
if err := r.queries.InsertAdminAuditLog(ctx, dbmodels.InsertAdminAuditLogParams{
ID:                id,
ActorUserPublicID: e.ActorUserPublicID,
ActorRole:         e.ActorRole,
TenantPublicID:    sql.NullString{String: e.TenantPublicID, Valid: e.TenantPublicID != ""},
Action:            e.Action,
