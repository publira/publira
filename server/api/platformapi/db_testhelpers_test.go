...
func seedTenantMember(t *testing.T, pg *testutil.PostgresEnv, tenantID uuid.UUID, userID uuid.UUID, role string) {
	...
	ctx, cancel := context.WithTimeout(context.Background(), 10*time.Second)
	defer cancel()
	// tenant_user_roles.tenant_id has no default, so the row is inserted directly
	// rather than through CreateTenantUserRole.
	if _, err := pg.DB.ExecContext(ctx, `
		INSERT INTO tenant_user_roles (id, tenant_id, user_id, role)
		VALUES ($1, $2, $3, $4)
	`, uuid.Must(uuid.NewV7()), tenantID, userID, role); err != nil {
		t.Fatalf("insert tenant_user_roles: %v", err)
	}
}
