...
Neither may one chain reach into another's variable. A shared fallback looks harmless while both happen to run on the same connection and turns into a silent role change the day either one is repointed. `publiractl` is the one place a chain ends at `PUBLIRA_DB_URL`: a `job` subcommand's stays inside the jobs' own names before it, and a settings or provisioning command's is `PUBLIRA_PLATFORM_DB_URL` alone.
