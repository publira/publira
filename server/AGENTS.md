...
3. **Every batch job is a subcommand of `cmd/batch`**, one binary and one image, selected by the first argument. A new batch is a new entry in that command's subcommand table, never a new `cmd/` directory: per-batch directories multiply the Docker matrix and the registrations in `infra/docker/Taskfile.yaml` and `scripts/ci-plan-jobs.sh`. Keep each subcommand's own lifecycle: a job that has to act the moment a stored instant passes is a ticker (`publish-episodes`, `apply-free-windows`), and a job that rebuilds or purges a period of data is a one-shot an external schedule invokes. Resolve `service.name` as `publira-<subcommand>`.
...
Neither may one chain reach into another's variable. A shared fallback looks harmless while both happen to run on the same connection and turns into a silent role change the day either one is repointed. The one-shot `cmd/batch` subcommands are the one place a chain runs several variables deep, and it stays inside the batches' own names before ending at `PUBLIRA_DB_URL`. The three tickers are not part of that: they share `PUBLIRA_TICKER_DB_URL` and end on `publira_ticker`'s development URL the way every server does, because they write to the catalog on every pass rather than rebuilding a day of derived rows on demand.
