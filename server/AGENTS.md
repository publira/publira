...
| Path | Role |
| --- | --- |
| `cmd/` | Thin entrypoints only (`api-server`, image servers, `worker` for the long-lived background jobs, and `batch` for the one-shot batch jobs) |
...
3. **The shape of a job follows when it has to run.** A job that rebuilds or purges a period of data is a one-shot an external schedule invokes, and it is a subcommand of `cmd/batch` — one binary and one image, selected by the first argument, resolving `service.name` as `publira-<subcommand>`. A new one is a new entry in that command's subcommand table, never a new `cmd/` directory: per-batch directories multiply the Docker matrix and the registrations in `infra/docker/Taskfile.yaml` and `scripts/ci-plan-jobs.sh`. The work itself belongs to `internal/maintenance`, which both that subcommand and the worker's own River kind (`internal/maintenancejobs`) invoke, so a new job is a settings type and a `Run` method there before it is an entry in either table — never an implementation inside `cmd/`.
4. **A job that has to act the moment a stored instant passes is a River periodic job the `worker` process runs**, never a process on a ticker of its own. `worker` is the one long-lived background process: it drains the Outbox and hosts every River-backed job, so a new class of background work joins it rather than bringing a second River client and a second set of maintenance services. Its workers and schedule live in `internal/tickerjobs` like any other logic, and `cmd/worker` only wires them into the River client, per rule 2. The worker is already a long-lived River client, so the schedule, the overlap guard, and a record of each due run come with it instead of being a deployment to supervise. Give such a job `UniqueOpts` over River's in-flight states — every one of them spans every tenant, so two at once would write the same rows twice — and a `service.name` of its own through `tracing.Setup`, so the runs stay apart in a trace UI now that they share a process. The jobs connect as `publira_ticker`, not on the worker's own pool: that login owns River's schema and holds `CREATE ON SCHEMA public`, which is the privilege they must not have.
...
Neither may one chain reach into another's variable. A shared fallback looks harmless while both happen to run on the same connection and turns into a silent role change the day either one is repointed. The one-shot `cmd/batch` subcommands are the one place a chain runs several variables deep, and it stays inside the batches' own names before ending at `PUBLIRA_DB_URL`.

The rule is about roles rather than about processes, and two processes show why. `worker` opens a second pool for its periodic jobs (`PUBLIRA_TICKER_DB_URL`) because the login that owns River's schema is not the one those jobs should write the catalog with. And `api-server` serves all three Connect namespaces and therefore opens a pool per login — `PUBLIRA_PUBLIC_DB_URL`, `PUBLIRA_ADMIN_DB_URL`, `PUBLIRA_PLATFORM_DB_URL` — picking the pool by the namespace the procedure path names. Three chains in one process are still three chains; sharing a pool between namespaces would hand `publira.v1` the `BYPASSRLS` reach of `publira_platform`.
