...
A mail limit below `1`, or one that is not a whole number, stops the server at startup rather than taking effect: a limit of zero would refuse every one of those forms, and that is better caught before the server serves anything than by the first person who cannot get their password reset.
