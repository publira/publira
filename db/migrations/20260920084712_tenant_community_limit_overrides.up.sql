...
CREATE TABLE tenant_community_limit_overrides (
    ...
    CONSTRAINT tenant_community_limit_overrides_comment_post_check CHECK ((comment_post_limit_per_minute IS NULL) = (comment_post_limit_per_day IS NULL) AND (comment_post_limit_per_minute IS NULL OR (comment_post_limit_per_minute >= 1 AND comment_post_limit_per_day >= comment_post_limit_per_minute))),
    CONSTRAINT tenant_community_limit_overrides_comment_report_check CHECK ((comment_report_limit_per_minute IS NULL) = (comment_report_limit_per_day IS NULL) AND (comment_report_limit_per_minute IS NULL OR (comment_report_limit_per_minute >= 1 AND comment_report_limit_per_day >= comment_report_limit_per_minute))),
    ...
    CONSTRAINT tenant_community_limit_overrides_episode_rating_check CHECK ((episode_rating_limit_per_minute IS NULL) = (episode_rating_limit_per_day IS NULL) AND (episode_rating_limit_per_minute IS NULL OR (episode_rating_limit_per_minute >= 1 AND episode_rating_limit_per_day >= episode_rating_limit_per_minute))),
    CONSTRAINT tenant_community_limit_overrides_contact_account_check CHECK ((contact_message_limit_per_account_per_hour IS NULL) = (contact_message_limit_per_account_per_day IS NULL) AND (contact_message_limit_per_account_per_hour IS NULL OR (contact_message_limit_per_account_per_hour >= 1 AND contact_message_limit_per_account_per_day >= contact_message_limit_per_account_per_hour))),
    CONSTRAINT tenant_community_limit_overrides_contact_client_check CHECK ((contact_message_limit_per_client_per_hour IS NULL) = (contact_message_limit_per_client_per_day IS NULL) AND (contact_message_limit_per_client_per_hour IS NULL OR (contact_message_limit_per_client_per_hour >= 1 AND contact_message_limit_per_client_per_day >= contact_message_limit_per_client_per_hour))),
    CONSTRAINT tenant_community_limit_overrides_viewer_preferences_check CHECK ((viewer_preferences_limit_per_minute IS NULL) = (viewer_preferences_limit_per_day IS NULL) AND (viewer_preferences_limit_per_minute IS NULL OR (viewer_preferences_limit_per_minute >= 1 AND viewer_preferences_limit_per_day >= viewer_preferences_limit_per_minute)))
);
