...
SET
    ...
    payload = CASE
        WHEN attempts + 1 >= sqlc.arg('max_attempts') THEN payload - '{token,password_hash,email,name,birth_date}'::text[]
        ELSE payload
    END,
    available_at = NOW(),
    updated_at = NOW()
