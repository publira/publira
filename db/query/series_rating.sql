...
WITH prior AS (
    ...
    WHERE cds.tenant_id = sqlc.arg('tenant_id')
        AND cds.entity_type = 'series'
        AND cds.entity_id = sqlc.arg('series_id')
), tenant_totals AS (
    SELECT
        COALESCE((
            SELECT trt.points
            FROM tenant_rating_totals trt
            WHERE trt.tenant_id = sqlc.arg('tenant_id')
        ), 0)::numeric AS points,
        COALESCE((
    ...
    SELECT COALESCE((
        ...
        WHERE src.tenant_id = sqlc.arg('tenant_id')
            AND src.series_id = sqlc.arg('series_id')
    ), 0)::bigint AS count
), rated AS (
