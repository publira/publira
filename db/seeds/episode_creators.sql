-- The credits of the episodes a seed wrote, copied from their series.
--
...
-- copied into each, so the copy has one place to be read and changed. The
-- including seed names its tenant with `\set seed_tenant <public_id>`, so
-- re-applying it never reads an episode another spec is deleting.
--
