-- Scenario: member notification list pagination (#717)
--
-- Gives the dev seed member (`member@example.com` on Host `localhost`) more
-- notifications than one page of the public list holds, so E2E can page across
-- the cursor boundary. The dev seed itself creates no notifications at all.
--
-- 45 rows at `NOTIFICATIONS_PAGE_SIZE` 20 means three pages: 20 / 20 / 5.
--
