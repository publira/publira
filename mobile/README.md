...
- A signed-out reader holds no credential, and a free body is what makes that second source reachable. The API puts a media token on every free page's URL that is issued for the episode and a 24-hour rotation window rather than for a reader, so every reader of that episode inside one window is handed the same token and derives the same key. Its `sub` names nobody (`anonymous-free-episode`) and unlocks nothing; the app reads it out of the token the way it reads a reader's, which is why the free path needs no derivation of its own. A signed-in reader keeps decrypting the same free page with their bearer, because that is the material image-server resolved first
