...
- **Host processes:** the API server (its edge-facing and internal listeners carry all three Connect namespaces), email-renderer, the outbox worker (which also runs the periodic jobs that promote due episodes, apply free window boundaries, and roll tenant days, on the intervals `E2E_PUBLISH_EPISODES_INTERVAL_SEC`, `E2E_FREE_WINDOW_INTERVAL_SEC`, and `E2E_TENANT_DAY_INTERVAL_SEC` shorten to seconds), image-server, and standalone `web-host`, `web-admin`, and `web-platform` (`node server.js`).
