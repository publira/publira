...
- **Host processes:** the API server (its edge-facing and internal listeners carry all three Connect namespaces), batch `publish-episodes`, email-renderer, outbox worker, image-server, and standalone `web-host`, `web-admin`, and `web-platform` (`node server.js`).
