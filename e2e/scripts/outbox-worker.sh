...
READY_TIMEOUT_SEC="${PUBLIRA_E2E_OUTBOX_WORKER_READY_TIMEOUT_SEC:-60}"

outbox_worker_readyz_url() {
  printf 'http://127.0.0.1:%s/readyz' "${PUBLIRA_E2E_OUTBOX_WORKER_PORT}"
}
...
start_outbox_worker() {
  ...
  e2e_log "starting outbox-worker (addr :${PUBLIRA_E2E_OUTBOX_WORKER_PORT}, ticker intervals ${PUBLIRA_E2E_PUBLISH_EPISODES_INTERVAL_SEC}s/${PUBLIRA_E2E_FREE_WINDOW_INTERVAL_SEC}s/${PUBLIRA_E2E_TENANT_DAY_INTERVAL_SEC}s)"
  (
    cd "${REPO_ROOT}/server"
    exec env \
      PUBLIRA_WORKER_DB_URL="${PUBLIRA_WORKER_DB_URL}" \
      PUBLIRA_TICKER_DB_URL="${PUBLIRA_TICKER_DB_URL}" \
      PUBLIRA_WORKER_ADDR=":${PUBLIRA_E2E_OUTBOX_WORKER_PORT}" \
