#!/usr/bin/env bash
# Start/stop only the platform-api-server process (:8102).
set -euo pipefail
...
start_platform_api_server() {
  ...
  e2e_log "starting platform-api-server (:${E2E_PLATFORM_API_GRPC_PORT})"
  (
    cd "${REPO_ROOT}/server"
    exec env \
      ...
      PUBLIRA_MAIL_REQUEST_LIMIT_PER_SOURCE_PER_HOUR="${PUBLIRA_MAIL_REQUEST_LIMIT_PER_SOURCE_PER_HOUR:-}" \
      PUBLIRA_MAIL_REQUEST_LIMIT_PER_SOURCE_PER_DAY="${PUBLIRA_MAIL_REQUEST_LIMIT_PER_SOURCE_PER_DAY:-}" \
      "${api_bin}"
