...
# PNCH_REDIS_URL never reaches them they fall back to redis://localhost:6379,
...
if [[ -z "${clients}" ]] || ((clients < 2)); then
  bootstrap_fail "no app is connected to the bootstrap Redis (connected_clients=${clients:-unknown}); PNCH_REDIS_URL is probably not reaching the dev tasks"
fi
