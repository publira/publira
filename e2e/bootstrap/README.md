...
データストアのポートは Playwright E2E（`5433` / `6380` / `9003`）とずらしてあるので、同時に動かせる。一方 `task dev` のポートは固定なので、**開発用の `task dev` を動かしたままでは phase 4 が走らない**（起動前にポート衝突を検出して落とす）。
...
`task dev` に渡す環境変数（`PUBLIRA_DB_URL` / `PUBLIRA_*_DB_URL` / `PUBLIRA_REDIS_URL` / `PUBLIRA_S3_*` / `AWS_*` / `PUBLIRA_AUTH_SECRET` / `PUBLIRA_AUTH_JWT_SECRET`）は `scripts/lib.sh` が bootstrap 用スタックを指すよう export する。2 つの鍵だけはスタックを指す値ではなく、Next.js アプリが必須とするセッション暗号鍵と Go サーバーが必須とするアクセストークン署名鍵をチェック用に与えるもの（`task dev` はホスト側で走るので Dev Container の compose が渡す値は届かない）。ストレージは Dev Container と同じく path-style で、`PUBLIRA_S3_ENDPOINT` は bootstrap 用 RustFS（`http://127.0.0.1:${BOOTSTRAP_RUSTFS_PORT}`）に固定する。Go サーバーと Next.js アプリの API 向き先は既定値（`localhost` + 標準ポート）のままなので上書きしない。
