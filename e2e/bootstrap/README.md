...
| All services from `task dev` | `3000` `4000` `4100` `8000` `8100` `8200` `8201` | **Cannot change**; Next.js ports are fixed in `apps/*/package.json` `dev` commands. |
...
| 4 | Run `task dev`. | Three Go servers (the API server's edge and internal listeners, image, and admin image) and three Next.js apps return 200 from `/livez` and `/readyz`; all 7 ports listen; the bootstrap Redis has application connections. |
