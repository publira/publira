...
- The calling surface (`ClientSurface`) decides which rows a list holds as well, so every token of a list it narrows — the catalog's, and the reader's own follows, history, and library — carries it too, as a leading `surface:<name>` key ahead of the list's own keys, and a token presented from another surface is `invalid_argument`. A web URL opened in the app is where one would otherwise cross over.
